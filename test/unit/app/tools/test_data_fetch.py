...
def test_uri_get_records_applied_transform(test_http_server, body, expected_transform):
    url = test_http_server.get_url(remote_url="https://example.org/transform-test", body=body)
    with _execute_context(allow_localhost=True) as execute_context:
        request = {
            "targets": [
                {
