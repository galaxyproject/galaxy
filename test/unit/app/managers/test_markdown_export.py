...
class BaseExportTestCase(BaseTestCase):
    ...
    def _expect_get_history(self, history):
        self.app.history_manager.get_accessible.return_value = history  # type: ignore[union-attr]
        yield
        self.app.history_manager.get_accessible.assert_called_once_with(history.id, self.trans.user)  # type: ignore[union-attr]

    @contextmanager
    def _expect_get_hda(self, hda, hda_id=1):
        self.app.hda_manager.get_accessible.return_value = hda  # type: ignore[union-attr]
        yield
        self.app.hda_manager.get_accessible.assert_called_once_with(hda.id, self.trans.user)  # type: ignore[union-attr]
...
"""
    ...
    def test_history_collection_paired(self):
        ...
        self.trans.app.dataset_collection_manager.get_dataset_collection_instance.return_value = hdca  # type: ignore[union-attr]
        example = """# Example
```galaxy
...
"""
    ...
    def test_workflow_export(self):
        ...
        self.trans.app.workflow_manager.get_stored_accessible_workflow.return_value = stored_workflow  # type: ignore[union-attr]
        example = """# Example
```galaxy
...
"""
        ...
        self.app.workflow_manager.get_invocation.side_effect = [invocation]  # type: ignore[union-attr]
        result = self._to_basic(example)
...
```
"""
        self.app.hda_manager.get_accessible.side_effect = [hda, hda2]  # type: ignore[union-attr]
        export_markdown, extra_data = self._ready_export(example)
        assert "history_datasets" in extra_data
