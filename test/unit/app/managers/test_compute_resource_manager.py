...
class TestComputeResourceManager(BaseTestCase):
    def set_up_managers(self):
        ...
        self.compute_resource_manager = ComputeResourceManager(
    ...
    def test_returns_none_for_anonymous(self):
        assert self.compute_resource_manager.get_active_for(None) is None
    ...
    def test_returns_none_when_user_has_no_resources(self):
        ...
        assert self.compute_resource_manager.get_active_for(user) is None
    ...
    def test_returns_active_resource(self):
        ...
        result = self.compute_resource_manager.get_active_for(user)
    ...
    def test_scopes_to_requesting_user(self):
        ...
        result_a = self.compute_resource_manager.get_active_for(user_a)
        result_b = self.compute_resource_manager.get_active_for(user_b)
    ...
    def test_start_registration_persists_token(self):
        ...
        fetched = self.trans.sa_session.scalars(
            select(ComputeResourceRegistration).where(ComputeResourceRegistration.token == ticket.bootstrap_token)
        ).first()
    ...
    def test_complete_registration_rejects_expired_bootstrap_token(self):
        ...
        with pytest.raises(RegistrationTokenExpired):
            self.compute_resource_manager.complete_registration(
                bootstrap_token=ticket.bootstrap_token,
                refresh_token="RT",
                relay_url="https://relay.example.test",
    ...
    def test_delete_transitions_to_disabled(self):
        ...
        returned = self.compute_resource_manager.delete(user, resource.id)
    ...
    def test_delete_refuses_cross_user(self):
        ...
        assert self.compute_resource_manager.delete(intruder, resource.id) is None
        self.trans.sa_session.refresh(resource)
    ...
    def test_start_registration_rate_limits(self):
        ...
        for _ in range(RATE_LIMIT_PER_HOUR):
            self.compute_resource_manager.start_registration(user)
    ...
    def test_start_registration_reaps_expired_tokens(self):
        ...
        # rolling window).
        fresh = self.compute_resource_manager.start_registration(user)
    ...
    def test_list_for_excludes_deleted(self):
        ...
        rows = self.compute_resource_manager.list_for(user)
    ...
    def _seed_vault_token(self, user, resource_id: int, value: str = "RT-OLD"):
        ...
        UserVaultWrapper(self.app.vault, user).write_secret(
            f"compute_resource/{resource_id}/relay_refresh_token", value
        )

    def _read_vault_token(self, user, resource_id: int):
        from galaxy.security.vault import UserVaultWrapper

        return UserVaultWrapper(self.app.vault, user).read_secret(f"compute_resource/{resource_id}/relay_refresh_token")
    ...
    def test_purge_clears_vault_and_transitions_to_deleted(self):
        ...
        returned = self.compute_resource_manager.purge(user, resource.id)
        assert returned is not None
    ...
    def test_purge_refuses_cross_user(self):
        ...
        assert self.compute_resource_manager.purge(intruder, resource.id) is None
    ...
    def test_purge_refuses_while_running_jobs_exist(self):
        ...
        job.destination_params = {"compute_resource_id": str(resource.id)}
        self.trans.sa_session.add(job)
        self.trans.sa_session.commit()

        with pytest.raises(ResourceHasRunningJobs):
            self.compute_resource_manager.purge(user, resource.id)
