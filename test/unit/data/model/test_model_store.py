...
def test_export_simple_invocation_to_ro_crate(tmp_path):
    # Mock the app, which includes a mock toolbox
    app = _mock_app()

    # Mock the toolbox behavior if needed
    mock_tool = MagicMock()
    mock_tool.id = "test_tool"
    mock_tool.version = "1.0"
    app.toolbox.get_tool.return_value = mock_tool  # Simulate fetching a tool from the toolbox

    # Set up a simple workflow invocation
    workflow_invocation = _setup_simple_invocation(app)

    # Create a directory to export the RO-Crate to
    crate_directory = tmp_path / "crate"

    # Export the workflow invocation to the RO-Crate
    with store.ROCrateModelExportStore(crate_directory, app=app) as export_store:
        export_store.export_workflow_invocation(workflow_invocation)

    # Validate the exported crate
    validate_invocation_crate_directory(crate_directory)
...
class TestApp(GalaxyDataTestApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Add a mock toolbox to the test app
