...
# Deferred sources are realized through the stock file sources, which include the base64
# handler, so the fixture bytes travel in the URI rather than over the network.
BED_URI = base64_uri(CONTENTS_2_BED.encode())
SIMPLE_LINE_URI = base64_uri(CONTENTS_SIMPLE_LINE.encode())
