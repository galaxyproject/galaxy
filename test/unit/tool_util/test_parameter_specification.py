...
def _assert_workflow_step_validates(parameters: ToolParameterBundleModel, workflow_step: RawStateDict) -> None:
    try:
        ...
        raise AssertionError(f"Parameter {parameter} failed to validate workflow step {workflow_step}. {e}")
...
def _assert_workflow_step_invalid(parameters: ToolParameterBundleModel, workflow_step: RawStateDict) -> None:
    ...
    assert (
        exc is not None
    ), f"Parameter {parameter} didn't result in validation error on workflow step {workflow_step} as expected."


def _assert_workflow_step_linked_validates(parameters: ToolParameterBundleModel, workflow_step_linked: RawStateDict) -> None:
    try:
        validate_workflow_step_linked(parameters, workflow_step_linked)
    except RequestParameterInvalidException as e:
        raise AssertionError(
            f"Parameter {parameter} failed to validate linked workflow step {workflow_step_linked}. {e}"
        )


def _assert_workflow_step_linked_invalid(parameters: ToolParameterBundleModel, workflow_step_linked: RawStateDict) -> None:
    exc = None
    try:
        validate_workflow_step_linked(parameters, workflow_step_linked)
    except RequestParameterInvalidException as e:
    ...
    assert (
        exc is not None
    ), f"Parameter {parameter} didn't result in validation error on linked workflow step {workflow_step_linked} as expected."
