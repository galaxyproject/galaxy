...
define([ "test-app", "mvc/upload/upload-view", "QUnit"
    ...
    QUnit.test( "test adding/removing paste/fetch upload item", function(assert) {
        ...
        assert.ok($('#btn-new').find('i').hasClass('fa-edit'), 'Paste/fetch icon changed');
        assert.ok($('#btn-start').hasClass('btn-primary'), 'Start button should be enabled/highlighted.');
        assert.ok($('#upload-row-0').find('.upload-symbol').hasClass('fa-trash-o'), 'Should show regular trash icon.');
        $('#upload-row-0').find('.upload-settings').trigger('click');
        assert.ok($('#upload-row-0').find('.upload-settings-cover').css('display') == 'none', 'Settings should be enabled.');
        $('#upload-row-0').find('.popover-close').trigger('click');
        $('#btn-start').trigger('click');
        assert.ok($('#upload-row-0').find('.upload-symbol').hasClass('fa-exclamation-triangle'), 'Upload attempt should have failed.');
        assert.ok($('#upload-row-0').find('.upload-settings').trigger('click'));
        assert.ok($('#upload-row-0').find('.upload-settings-cover').css('display') == 'block', 'Settings should be disabled.');
