...
define([ "test-app", "mvc/upload/upload-view"
    ...
    test( "test adding/removing paste/fetch upload item", function() {
        ...
        ok($('#upload-item-0').find('#symbol').hasClass('fa-trash-o'), 'Should show regular trash icon.');
        $('#upload-item-0').find('#settings').trigger('click');
        ok($('#upload-item-0').find('.upload-settings-cover').css('display') == 'none', 'Settings should be enabled.');
        $('#upload-item-0').find('.popover-close').trigger('click');
        $('#btn-start').trigger('click');
        ok($('#upload-item-0').find('#symbol').hasClass('fa-exclamation-triangle'), 'Upload attempt should have failed.');
        ok($('#upload-item-0').find('#settings').trigger('click'));
        ok($('#upload-item-0').find('.upload-settings-cover').css('display') == 'block', 'Settings should be disabled.');
        $('#upload-item-0').find('#symbol').trigger('click');
