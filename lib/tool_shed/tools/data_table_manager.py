...
class ToolDataTableManager(object):
    ...
    def generate_repository_info_elem_from_repository(self,
    ...
    def get_tool_index_sample_files(self, sample_files):
        ...
        for s in sample_files:
            ...
            if s.endswith('.loc.sample') or s.endswith(
                    '.xml.sample') or s.endswith('.txt.sample'):
                tool_index_sample_files.append(str(s))
        return tool_index_sample_files

    def handle_missing_data_table_entry(self, relative_install_dir, tool_path,
        ...
        if missing_data_table_entry:
            ...
            repository_tool = self.app.toolbox.load_tool(
    ...
    def get_target_install_dir(self, tool_shed_repository):
        ...
        target_dir = os.path.join(self.app.config.shed_tool_data_path,
                                  relative_target_dir)
    ...
    def install_tool_data_tables(self, tool_shed_repository,
        ...
        tool_data_table_conf_filename = os.path.join(target_dir,
                                                     TOOL_DATA_TABLE_FILE_NAME)
        elems = []
                ...
                for file_elem in elem.findall('file'):
                    ...
                    if path:
                        file_elem.set(
                            'path',
                            os.path.normpath(
                                os.path.join(target_dir, os.path.split(path)[
                                    1])))
