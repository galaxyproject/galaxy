...
class RepositoryReviewController(BaseUIController, ratings_util.ItemRatings):
    ...
    def browse_components(self, trans, **kwd):
        if "operation" in kwd:
            ...
            if operation == "create":
                return trans.response.send_redirect(
                    web.url_for(controller="repository_review", action="create_component", **kwd)
                )
        ...
        if kwd.get("create_component_button", False):
            if not name or not description:
                ...
                trans.response.send_redirect(
                    web.url_for(
                        controller="repository_review", action="manage_components", message=message, status=status
                    )
                )
        return trans.fill_template(
            ...
            if changeset_revision:
                ...
                if review_util.get_review_by_repository_id_changeset_revision_user_id(
                    ...
                    if (
                        ...
                        and review_util.has_previous_repository_reviews(trans.app, repository, changeset_revision)
                    ):
                        return trans.response.send_redirect(
                ...
                return trans.response.send_redirect(
                    web.url_for(controller="repository_review", action="manage_repository_reviews", **kwd)
                )
        return trans.response.send_redirect(
            web.url_for(controller="repository_review", action="view_or_manage_repository", **kwd)
        )
        ...
        if save_button_clicked:
            ...
            for component_name in components_dict.keys():
                ...
                if flushed:
                    ...
                    average_rating = (
                        trans.sa_session.query(func.avg(trans.model.ComponentReview.table.c.rating))
                        .filter(
                            and_(
                                trans.model.ComponentReview.table.c.repository_review_id == review.id,
    ...
    def manage_components(self, trans, **kwd):
        if "operation" in kwd:
            ...
            if operation == "create":
                ...
                return trans.response.send_redirect(
                    web.url_for(controller="repository_review", action="edit_component", **kwd)
                )
    ...
    def manage_repositories_ready_for_review(self, trans, **kwd):
        ...
        if "operation" in kwd:
            ...
            if operation == "inspect repository revisions":
                return trans.response.send_redirect(
                    web.url_for(controller="repository_review", action="create_review", **kwd)
                )
    ...
    def manage_repository_reviews(self, trans, mine=False, **kwd):
        ...
        if repository_id:
            ...
            metadata_revision_hashes = [
                metadata_revision.changeset_revision for metadata_revision in repository.metadata_revisions
            ]
                ...
                if changeset_revision in metadata_revision_hashes or changeset_revision in reviewed_revision_hashes:
                    ...
                    if changeset_revision in reviewed_revision_hashes:
                        ...
                        can_add_review = trans.user not in [
                            repository_review.user for repository_review in repository_reviews
                        ]
                        repository_metadata = metadata_util.get_repository_metadata_by_changeset_revision(
                            trans.app, repository_id, changeset_revision
                        )
