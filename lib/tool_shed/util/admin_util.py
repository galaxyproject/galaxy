...
class Admin(object):
    ...
    def index(self, trans, **kwd):
        ...
        return trans.fill_template(
            '/webapps/tool_shed/admin/index.mako',
            message=message,
            status=status)
    ...
    def center(self, trans, **kwd):
        ...
        status = kwd.get('status', 'done')
        return trans.fill_template(
            '/webapps/tool_shed/admin/center.mako',
            message=message,
            status=status)
        ...
        if trans.webapp.name == 'galaxy' and len(role.dataset_actions) < 25:
            ...
            for dp in role.dataset_actions:
                for ldda in trans.sa_session.query(trans.app.model.LibraryDatasetDatasetAssociation) \
                    ...
                    try:
                        library_dataset_actions[library][folder_path].append(
                            dp.action)
                    except:
                        library_dataset_actions[library][folder_path] = [
                            dp.action
                        ]
        else:
