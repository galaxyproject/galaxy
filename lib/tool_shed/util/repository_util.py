...
def build_allow_push_select_field(trans,
                                  current_push_list,
                                  selected_value='none'):
    options = []
...
def create_or_update_tool_shed_repository(app,
    ...
    if status in [
            app.install_model.ToolShedRepository.installation_status.
            DEACTIVATED
    ]:
        deleted = True
        uninstalled = False
...
def create_repository(app,
    ...
    dir = os.path.join(app.config.file_path,
                       *util.directory_hash_id(repository.id))
    ...
    hg_util.get_repo_for_repository(
        app, repository=None, repo_path=repository_path, create=True)
...
def generate_sharable_link_for_repository_in_tool_shed(
    ...
    sharable_url = '%s://%s/view/%s/%s' % (protocol, base,
...
def generate_tool_shed_repository_install_dir(repository_clone_url,
                                              changeset_revision):
    """
...
def get_installed_repository(app,
    ...
    query = app.install_model.context.query(
        app.install_model.ToolShedRepository)
    if repository_id:
        clause_list = [
            app.install_model.ToolShedRepository.table.c.id == repository_id
        ]
    else:
        clause_list = [
            app.install_model.ToolShedRepository.table.c.tool_shed ==
            tool_shed,
    ...
    if installed_changeset_revision is not None:
        clause_list.append(
            app.install_model.ToolShedRepository.table.c.
            installed_changeset_revision == installed_changeset_revision)
    ...
    for repo_info_dict in repo_info_dicts:
        ...
        if repository:
            ...
            if encoded_repository_id in tsr_ids:
                ...
                prior_import_or_install_required_dict[
                    encoded_repository_id] = prior_import_or_install_ids
...
def get_repositories_by_category(app, category_id, installable=False):
    ...
    for row in resultset.repositories:
        repository_dict = row.repository.to_dict(
        ...
        for changeset, changehash in row.repository.installable_revisions(app):
            ...
            repository_dict['metadata']['%s:%s' %
                                        (changeset,
                                         changehash)] = metadata.to_dict(
                                             value_mapper=default_value_mapper)
        if installable:
...
def get_repository_and_repository_dependencies_from_repo_info_dict(
    ...
    if hasattr(app, "install_model"):
        ...
        repository = get_repository_for_dependency_relationship(
            app, tool_shed, repository_name, repository_owner,
            changeset_revision)
    else:
        # We're in the tool shed.
        repository = get_repository_by_name_and_owner(app, repository_name,
...
def get_repository_by_id(app, id):
    ...
    if is_tool_shed_client(app):
        return app.install_model.context.query(
            app.install_model.ToolShedRepository).get(
                app.security.decode_id(id))
    else:
        sa_session = app.model.context.current
        return sa_session.query(app.model.Repository).get(
            app.security.decode_id(id))
...
def get_role_by_id(app, role_id):
    ...
    sa_session = app.model.context.current
    return sa_session.query(app.model.Role).get(
        app.security.decode_id(role_id))
...
def get_tool_shed_repository_status_label(app,
    ...
    if tool_shed_repository:
        ...
        if tool_shed_repository.status in [
                app.install_model.ToolShedRepository.installation_status.
                CLONING, app.install_model.ToolShedRepository.
            ...
            if tool_shed_repository.repository_dependencies_being_installed:
                bgcolor = app.install_model.ToolShedRepository.states.WARNING
                status_label = '%s, %s' % (
                    status_label, app.install_model.ToolShedRepository.
    ...
    return '<div class="count-box state-color-%s">%s</div>' % (bgcolor,
                                                               status_label)
...
def set_repository_attributes(app,
                              repository,
                              status,
                              error_message,
                              deleted,
