...
def accumulate_tool_dependencies(tool_shed_accessible, tool_dependencies,
...
def check_for_missing_tools(app, tool_panel_configs,
    ...
    tools_xml_file_path = os.path.abspath(
        os.path.join('scripts', 'migrate_tools', '%04d_tools.xml' %
                     latest_tool_migration_script_number))
...
def check_tool_tag_set(elem, migrated_tool_configs_dict,
                       missing_tool_configs_dict):
...
def generate_clone_url_for_repository_in_tool_shed(user, repository):
    ...
    if user:
        ...
        return '%s://%s%s/repos/%s/%s' % (protocol, username, base,
                                          repository.user.username,
                                          repository.name)
    else:
        return '%s/repos/%s/%s' % (base_url, repository.user.username,
                                   repository.name)
...
def get_repository_dependencies(app, tool_shed_url, repository_name,
    ...
    try:
        ...
        log.warning("The URL\n%s\nraised the exception:\n%s\n",
                    util.build_url(
                        tool_shed_url, pathspec=pathspec, params=params), e)
    if tool_shed_accessible:
        if len(raw_text) > 2:
...
def parse_repository_dependency_tuple(repository_dependency_tuple,
