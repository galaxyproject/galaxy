...
def get_all_dependencies(app, metadata_entry, processed_dependency_links=[]):
    ...
    value_mapper = {
        'repository_id': encoder,
        'id': encoder,
        'user_id': encoder
    }
    metadata = metadata_entry.to_dict(
    ...
    for dependency_metadata in required_metadata:
        ...
        dependency_dict['repository'] = repository.to_dict(
        ...
        if dependency_dict['includes_tool_dependencies']:
            dependency_dict[
                'tool_dependencies'] = repository.get_tool_dependencies(
...
def get_dependencies_for_metadata_revision(app, metadata):
    ...
    for shed, name, owner, changeset, prior, _ in metadata[
        ...
        required_repository = tool_shed.util.repository_util.get_repository_by_name_and_owner(
            app, name, owner)
        repo = hg_util.get_repo_for_repository(
            app, repository=required_repository, repo_path=None, create=False)
        updated_changeset = get_next_downloadable_changeset_revision(
            required_repository, repo, changeset)
...
def get_latest_changeset_revision(app, repository, repo):
    ...
    changeset_revisions = [
        revision[1] for revision in get_metadata_revisions(repository, repo)
    ]
...
def get_latest_repository_metadata(app,
                                   decoded_repository_id,
                                   downloadable=False):
    """Get last metadata defined for a specified repository from the database."""
    sa_session = app.model.context.current
    repository = sa_session.query(
        app.model.Repository).get(decoded_repository_id)
    repo = hg_util.get_repo_for_repository(
        app, repository=repository, repo_path=None, create=False)
    if downloadable:
        changeset_revision = get_latest_downloadable_changeset_revision(
            app, repository, repo)
    else:
        changeset_revision = get_latest_changeset_revision(
            app, repository, repo)
...
def get_metadata_by_id(app, metadata_id):
    sa_session = app.model.context.current
    return sa_session.query(app.model.RepositoryMetadata).filter(
        app.model.RepositoryMetadata.table.c.id == metadata_id).one()
...
def get_metadata_revisions(repository,
                           repo,
                           sort_revisions=True,
                           reverse=False,
                           downloadable=True):
...
def get_next_downloadable_changeset_revision(repository, repo,
...
def get_repository_dependencies(app, metadata_id):
    ...
    return sa_session.query(app.model.RepositoryDependency).filter(
        app.model.RepositoryDependency.table.c.parent_metadata_id ==
        metadata_id).all()
    ...
    if reviewed:
        ...
        for review in repository.reviews:
            if review.changeset_revision in metadata_changeset_revision_hashes:
                rmcr_hashes = [
                    rmr.changeset_revision
                    for rmr in repository_metadata_revisions
                ]
                if review.changeset_revision not in rmcr_hashes:
                    repository_metadata_revisions.append(
                        review.repository_metadata)
    else:
        for review in repository.reviews:
