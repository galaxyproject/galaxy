...
def build_approved_select_field(trans,
                                name,
                                selected_value=None,
                                for_component=True):
    ...
    for option_tup in options:
        ...
        select_field.add_option(
            option_tup[0], option_tup[1], selected=selected)
    return select_field
...
def get_latest_downloadable_repository_metadata_if_it_includes_tools(
    ...
    and tool_dependency_definition.
    """
    repository_metadata = get_latest_downloadable_repository_metadata(
        trans, repository)
...
def get_latest_repository_metadata(trans, repository):
    ...
    try:
        ...
        latest_downloadable_revision = metadata_util.get_previous_metadata_changeset_revision(
            repository, repo, tip_ctx, downloadable=False)
        if latest_downloadable_revision == hg_util.INITIAL_CHANGELOG_HASH:
            return None
        repository_metadata = metadata_util.get_repository_metadata_by_changeset_revision(
            trans.app, encoded_repository_id, latest_downloadable_revision)
...
def get_latest_repository_metadata_if_it_includes_invalid_tools(
        trans, repository):
    """
