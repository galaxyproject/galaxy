...
from tool_shed.util import (basic_util, common_util, hg_util, metadata_util,
                            readme_util, repository_util, shed_util_common as
                            suc, tool_dependency_util, tool_util, xml_util)
...
class MetadataGenerator(object):
    def __init__(self,
        ...
        if self.app.name == 'galaxy':
            ...
            if repository_files_dir is None and self.repository is not None:
                repository_files_dir = self.repository.repo_files_directory(
                    self.app)
    ...
    def generate_data_manager_metadata(self,
                                       repo_dir,
        ...
        try:
            # Galaxy Side.
            repo_files_directory = self.repository.repo_files_directory(
                self.app)
        ...
        if data_manager_tool_path:
            relative_data_manager_dir = os.path.join(relative_data_manager_dir,
                                                     data_manager_tool_path)
        for i, data_manager_elem in enumerate(root.findall('data_manager')):
            ...
            if data_manager_id is None:
                ...
                invalid_data_managers.append({
                    ...
                    i,
                    'error_message':
                    'Data Manager entry is missing id attribute'
                })
                continue
            # FIXME: default behavior is to fall back to tool.name.
            data_manager_name = data_manager_elem.get('name', data_manager_id)
            version = data_manager_elem.get('version',
                                            DataManager.DEFAULT_VERSION)
            guid = self.generate_guid_for_object(DataManager.GUID_TYPE,
                                                 data_manager_id, version)
            data_tables = []
            if tool_file is None:
                ...
                invalid_data_managers.append({
                    ...
                    i,
                    'error_message':
                    'Data Manager entry is missing tool_file attribute'
                })
                continue
            else:
                ...
                for data_table_elem in data_manager_elem.findall('data_table'):
                    ...
                    if data_table_name is None:
                        ...
                        invalid_data_managers.append({
                            ...
                            i,
                            'error_message':
                            'Data Manager entry is missing name attribute'
                        })
                        bad_data_table = True
                        break
                    else:
            ...
            if tool is None:
                log.error("Unable to determine tools metadata for '%s'." %
                          (data_manager_metadata_tool_file))
                invalid_data_managers.append({
                    'index':
                    i,
                    'error_message':
                    'Unable to determine tools metadata'
                })
                continue
    ...
    def generate_datatypes_metadata(self, tv, repository_files_dir,
                                    datatypes_config, metadata_dict):
        ...
        if registration is not None:
            for elem in registration.findall('datatype'):
                ...
                if max_optional_metadata_filesize:
                    datatypes_dict[
                        'max_optional_metadata_filesize'] = max_optional_metadata_filesize
    ...
    def generate_environment_dependency_metadata(self, elem,
    ...
    def generate_metadata_for_changeset_revision(self):
        ...
        readme_file_names = readme_util.get_readme_file_names(
            str(self.repository.name))
        if self.app.name == 'galaxy':
            # Shed related tool panel configs are only relevant to Galaxy.
            metadata_dict = {
                'shed_config_filename':
                self.shed_config_dict.get('config_filename')
            }
        else:
            metadata_dict = {}
        readme_files = []
        ...
        if self.resetting_all_metadata_on_repository:
            ...
            if self.shed_config_dict.get('tool_path'):
                files_dir = os.path.join(self.shed_config_dict['tool_path'],
                                         files_dir)
        ...
        for sample_file in sample_file_copy_paths:
            tool_util.copy_sample_file(
                self.app, sample_file, dest_path=work_dir)
                ...
                for name in files:
                    ...
                    if name == rt_util.REPOSITORY_DEPENDENCY_DEFINITION_FILENAME:
                        ...
                        metadata_dict, error_message = \
                            self.generate_repository_dependency_metadata(path_to_repository_dependencies_config,
                                                                         metadata_dict)
                        if error_message:
                            self.invalid_file_tups.append((name,
                                                           error_message))
                    # See if we have one or more READ_ME files.
                        ...
                        relative_path_to_readme = self.get_relative_path_to_repository_file(
                            root, name, self.relative_install_dir, work_dir,
                            self.shed_config_dict)
                    ...
                    # See if we have a tool config.
                        ...
                        full_path = str(
                            os.path.abspath(os.path.join(
                                root, name)))  # why the str, seems very odd
                        element_tree, error_message = xml_util.parse_xml(
                            full_path)
                            ...
                            if tool is None:
                                ...
                                if can_set_metadata:
                                    ...
                                    metadata_dict = self.generate_tool_metadata(
                                        relative_path_to_tool_config, tool,
                                        metadata_dict)
                                else:
                                    for tup in invalid_files_and_errors_tups:
                        ...
                        if os.path.getsize(os.path.abspath(relative_path)) > 0:
                            ...
                            if valid_exported_galaxy_workflow and \
                                ...
                                metadata_dict = self.generate_workflow_metadata(
                                    relative_path, exported_workflow_dict,
                                    metadata_dict)
    ...
    def generate_package_dependency_metadata(self, elem,
                                             valid_tool_dependencies_dict,
                            ...
                            if in_actions_group:
                                ...
                                for action_elem in actions_elem:
                                    if action_elem.tag == 'package':
                                        ...
                                        # </package>
                                        ae_package_name = action_elem.get(
                                            'name', None)
        ...
        if requirements_dict:
            ...
            if repository_dependency_is_valid:
                valid_tool_dependencies_dict[
                    dependency_key] = requirements_dict
            else:
        ...
        if xml_is_valid:
            ...
            if valid_repository_dependency_tups:
                valid_repository_dependencies_dict[
                    'repository_dependencies'] = valid_repository_dependency_tups
                metadata_dict[
                    'repository_dependencies'] = valid_repository_dependencies_dict
    ...
    def generate_tool_metadata(self, tool_config, tool, metadata_dict):
        ...
        for tool_requirement in tool.requirements:
            ...
            version = str(
                tool_requirement.version) if tool_requirement.version else None
        ...
        if tool.tests:
            for ttb in tool.tests:
                ...
                for output in ttb.outputs:
                    ...
                    outputs.append((name, basic_util.strip_path(file_name)
    ...
    def generate_tool_dependency_metadata(self,
                ...
                if elem.tag == 'package':
                    ...
                    if repository_dependency_is_valid:
                        ...
                        if repository_dependency_tup and repository_dependency_tup not in invalid_repository_dependency_tups:
                            ...
                            invalid_repository_dependency_tups.append(
                                repository_dependency_tup)
                            error_messages.append('%s  %s' % (error_message,
                                                              message))
            ...
            if original_valid_tool_dependencies_dict:
                ...
                updated_tool_dependency_names, deleted_tool_dependency_names = \
                    irm.handle_existing_tool_dependencies_that_changed_in_update(self.repository,
                                                                                 ...
                                                                                 rvs.valid_tool_dependencies_dict)
            metadata_dict[
                'tool_dependencies'] = rvs.valid_tool_dependencies_dict
        if rvs.invalid_tool_dependencies_dict:
            metadata_dict[
                'invalid_tool_dependencies'] = rvs.invalid_tool_dependencies_dict
        if valid_repository_dependency_tups:
            metadata_dict = \
    ...
    def generate_workflow_metadata(self, relative_path, exported_workflow_dict,
                                   metadata_dict):
        """
        ...
        if 'workflows' in metadata_dict:
            metadata_dict['workflows'].append((relative_path,
                                               exported_workflow_dict))
        else:
            metadata_dict['workflows'] = [(relative_path,
                                           exported_workflow_dict)]
        return metadata_dict
    ...
    def get_relative_path_to_repository_file(self, root, name,
                                             relative_install_dir, work_dir,
                                             shed_config_dict):
        if self.resetting_all_metadata_on_repository:
            ...
            relative_path_to_file = os.path.join(relative_install_dir,
                                                 stripped_path_to_file)
        else:
    ...
    def get_sample_files_from_disk(self,
                            ...
                            if tool_path and relative_install_dir:
                                if relative_path_to_sample_file.startswith(
                                    ...
                                    relative_path_to_sample_file = relative_path_to_sample_file[
                                        len(tool_path) + 1:]
                        sample_file_metadata_paths.append(
                            relative_path_to_sample_file)
        return sample_file_metadata_paths, sample_file_copy_paths

    def handle_repository_elem(self,
                               repository_elem,
                               only_if_compiling_contained_td=False):
        """
        ...
        if self.app.name == 'galaxy':
            if self.updating_installed_repository:
                ...
                if toolshed is None or name is None or owner is None or changeset_revision is None:
                    ...
                    error_message += xml_util.xml_to_string(
                        repository_elem, use_indent=True)
    ...
    def update_repository_dependencies_metadata(
        ...
        if repository_dependencies_dict:
            if is_valid:
                metadata[
                    'repository_dependencies'] = repository_dependencies_dict
            else:
                metadata[
                    'invalid_repository_dependencies'] = repository_dependencies_dict
        return metadata
