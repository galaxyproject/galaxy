...
class CompressedFile(object):
    ...
    def extract(self, path):
        ...
        if len(contents) == 1:
            ...
            if len(common_prefix) >= 1 and not common_prefix.endswith(
                    os.sep) and self.isdir(self.getmember(common_prefix)):
        ...
        if self.file_type == 'zip':
            for zipped_file in contents:
                ...
                external_attributes = self.archive.getinfo(
                    filename).external_attr
...
class Download(object):
    def url_download(self,
        ...
        try:
            download_to_file(
                download_url, file_path, chunk_size=basic_util.CHUNK_SIZE)
        except Exception as e:
            err_msg = 'Error downloading from URL %s : %s' % (
                str(download_url), str(e))
            raise Exception(err_msg)

        if 'sha256sum' in checksums or '#sha256#' in download_url:
            downloaded_checksum = hashlib.sha256(
                open(file_path, 'rb').read()).hexdigest().lower()
        ...
        if 'md5sum' in checksums or '#md5#' in download_url or '#md5=' in download_url:
            downloaded_checksum = hashlib.md5(
                open(file_path, 'rb').read()).hexdigest().lower()
        ...
        if extract:
            if tarfile.is_tarfile(file_path) or (
                    ...
                    and not file_path.endswith('.jar')):
...
class RecipeStep(object):
    ...
    def prepare_step(self, tool_dependency, action_elem, action_dict,
...
class AssertDirectoryExecutable(RecipeStep):
    ...
    def execute_step(self,
        ...
        if not self.assert_directory_executable(full_path=full_path):
            ...
            error_message = 'The path %s is not a directory or is not executable by the owner.' % str(
                full_path)
...
class AssertFileExecutable(RecipeStep):
    ...
    def execute_step(self,
        ...
        if not self.assert_file_executable(full_path=full_path):
            status = self.app.install_model.ToolDependency.installation_status.ERROR
            error_message = 'The path %s is not a file or is not executable by the owner.' % str(
                full_path)
            tool_dependency = tool_dependency_util.set_tool_dependency_attributes(
                self.app,
                tool_dependency,
...
class ChangeDirectory(RecipeStep):
    ...
    def execute_step(self,
        ...
        target_dir = os.path.realpath(
            os.path.normpath(action_dict['directory']))
        if dir_valid(target_dir):
            ...
            target_dir = os.path.realpath(
                os.path.normpath(
                    os.path.join(current_dir, action_dict['directory'])))
            if dir_valid(target_dir):
                dir = target_dir
            else:
                log.debug(
...
class Chmod(RecipeStep):
    ...
    def execute_step(self,
        ...
        for target_file, mode in action_dict['change_modes']:
            if os.path.exists(target_file):
                ...
                log.debug('Invalid file %s specified, ignoring %s action.',
                          target_file, self.type)
        return tool_dependency, None, None
    ...
    def prepare_step(self, tool_dependency, action_elem, action_dict,
        ...
        for file_elem in file_elems:
            ...
            file = basic_util.evaluate_template(file_elem.text,
                                                install_environment)
...
class DownloadBinary(Download, RecipeStep):
    ...
    def execute_step(self,
        ...
        try:
            ...
            log.debug('Attempting to download from %s to %s', url,
        ...
        if downloaded_filename and os.path.exists(
            ...
            if target_directory:
                target_directory = os.path.realpath(
                    os.path.normpath(
                        os.path.join(install_environment.install_dir,
                                     target_directory)))
    ...
    def prepare_step(self, tool_dependency, action_elem, action_dict,
        ...
        action_dict['url'] = Template(
            url_template_elem.text.strip()).safe_substitute(platform_info_dict)
        action_dict['target_directory'] = action_elem.get(
...
class DownloadByUrl(Download, RecipeStep):
    ...
    def execute_step(self,
        ...
        if is_binary:
            ...
            logfile.write(
                'Successfully downloaded from url: %s\n' % action_dict['url'])
            logfile.close()
...
class MoveDirectoryFiles(RecipeStep):
    ...
    def move_directory_files(self, current_dir, source_dir, destination_dir):
        source_directory = os.path.abspath(
            os.path.join(current_dir, source_dir))
        ...
        for dir_entry in os.listdir(source_directory):
            ...
            if os.path.islink(source_entry):
                destination_entry = os.path.join(destination_directory,
                                                 dir_entry)
    ...
    def execute_step(self,
        ...
        with open(filename, 'r') as haystack:
            for line in haystack:
                altered_text, replacement_count = re.subn(
                    regex, replacement, line)
        ...
        log_text = log_text % (action_dict['regex'],
                               action_dict['replacement'], filename,
                               total_replacements)
        log.debug(log_text)
...
class SetEnvironment(RecipeStep):
    ...
    def execute_step(self,
        ...
        root_dir_dict = dict(
            action='set_to',
            name='%s_ROOT_DIR' % re.sub(r"[^A-Z0-9_]", "_",
                                        tool_dependency.name.upper()),
    ...
    def handle_environment_variables(self, install_environment,
                                     tool_dependency, env_var_dict,
    ...
    def prepare_step(self, tool_dependency, action_elem, action_dict,
        ...
        for env_elem in action_elem:
            if env_elem.tag == 'repository':
                env_shell_file_paths = env_manager.get_env_shell_file_paths(
                    env_elem)
    ...
    def execute_step(self,
        ...
        log.debug(
            ...
            % (str(tool_dependency.name),
               str(install_environment.env_shell_file_paths)))
        dir = os.path.curdir
            ...
            with settings(warn_only=True):
                ...
                for perl_package_dict in perl_packages:
                    ...
                    if perl_package.find('://') != -1:
                        ...
                        if os.path.exists(
                                os.path.join(tmp_work_dir, 'Makefile.PL')):
                            cmd += '''perl Makefile.PL INSTALL_BASE=$INSTALL_DIR && make && make install'''
                        elif os.path.exists(
                                os.path.join(tmp_work_dir, 'Build.PL')):
                        ...
                        cmd += '''cpanm --local-lib=$INSTALL_DIR %s''' % (
                            perl_package)
                        cmd = install_environment.build_command(
                            basic_util.evaluate_template(
                                cmd, install_environment))
                ...
                env_file_builder.append_line(
                    name="PERL5LIB",
                    action="prepend_to",
                    value=os.path.join(install_environment.install_dir, 'lib',
                                       'perl5'))
                env_file_builder.append_line(
                    name="PATH",
                    action="prepend_to",
                ...
                for ruby_package_tup_dict in ruby_package_tups:
                    ...
                    if os.path.isfile(gem):
                        ...
                        self.url_download(
                            ...
                            checksums=checksums)
                        cmd = '''PATH=$PATH:$RUBY_HOME/bin; export PATH; GEM_HOME=$INSTALL_DIR; export GEM_HOME;
                                gem install --local %s %s''' % (gem_name,
                                                                gem_parameters)
                    else:
            ...
            with settings(warn_only=True):
                ...
                for python_package_tup_dict in python_package_tups:
                    ...
                    if os.path.isfile(package_path):
                        ...
                        checksums = python_package_tup_dict.get(
                            'checksums', {})
                        self.url_download(
                            work_dir,
                            package_name,
                            ...
                            extract=False,
                            checksums=checksums)
                        package_to_install = os.path.join(
...
class SetupVirtualEnv(Download, RecipeStep):
    ...
    def execute_step(self,
        ...
        venv_src_directory = os.path.abspath(
            os.path.join(self.app.config.tool_dependency_dir,
                         '__virtualenv_src'))
        if not self.install_virtualenv(install_environment,
                                       venv_src_directory):
            log.debug('Unable to install virtualenv')
            return tool_dependency, None, None
        requirements = action_dict['requirements']
        if os.path.exists(
                os.path.join(install_environment.install_dir, requirements)):
            # requirements specified as path to a file
            requirements_path = requirements
        else:
            # requirements specified directly in XML, create a file with these for pip.
            requirements_path = os.path.join(install_environment.install_dir,
                                             "requirements.txt")
        ...
        setup_command = "%s %s/virtualenv.py --no-site-packages '%s'" % (
            python_cmd, venv_src_directory, venv_directory)
                ...
                while True:
                    ...
                    if line:
                        ...
                        if not install_command:
                            ...
                            install_command = "%s && %s" % (
                                install_command, line_install_command)
        full_setup_command = "%s; %s; %s" % (setup_command, activate_command,
                                             install_command)
        return_code = install_environment.handle_command(
            tool_dependency=tool_dependency,
            ...
            job_name=package_name)
        if return_code:
            log.error("Failed to do setup_virtualenv install, exit code='%s'",
                      return_code)
        ...
        env_file_builder.append_line(
            name="PATH",
            action="prepend_to",
            value=os.path.join(venv_directory, "bin"))
    ...
    def __get_site_packages_directory(self, install_environment, app,
                                      tool_dependency, python_cmd,
                                      venv_directory):
...
class TemplateCommand(RecipeStep):
    ...
    def prepare_step(self, tool_dependency, action_elem, action_dict,
        ...
        if language == 'cheetah':
            ...
            log.debug("Unsupported template language '%s'. Not proceeding." %
                      str(language))
            raise Exception(
