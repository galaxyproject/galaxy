...
class ToolMigrationManager(object):
                ...
                if tool_shed_accessible:
                    ...
                    if missing_tool_configs_dict:
                        for proprietary_tool_conf in self.proprietary_tool_confs:
                            ...
                            shutil.copy(proprietary_tool_conf,
                                        '%s-pre-stage-%04d' %
        ...
        for proprietary_tool_conf in self.proprietary_tool_confs:
            ...
            if persist_required:
                fh = tempfile.NamedTemporaryFile(
                    'wb', prefix="tmp-toolshed-fapptpc")
                tmp_filename = fh.name
                fh.close()
                fh = open(tmp_filename, 'wb')
                tree.write(
                    tmp_filename, encoding='utf-8', xml_declaration=True)
                fh.close()
                shutil.move(tmp_filename,
                            os.path.abspath(proprietary_tool_conf))
    ...
    def get_guid(self, repository_clone_url, relative_install_dir,
                 tool_config):
            ...
            if root.find('.hg') < 0 and root.find('hgrc') < 0:
                ...
                for name in files:
                    ...
                    if filename == tool_config_filename:
                        ...
                        tool = self.toolbox.load_tool(
                            full_path, use_cached=False)
                        return suc.generate_tool_guid(repository_clone_url,
                                                      tool)
    ...
    def order_repositories_for_installation(self, tool_shed_repositories,
                                            repository_dependencies_dict):
        """
    ...
    def __is_repository_dependency(self, name, changeset_revision,
                                   tool_shed_repository):
