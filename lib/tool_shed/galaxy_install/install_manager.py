...
class InstallToolDependencyManager(object):
    def __init__(self, app):
        ...
        self.INSTALL_ACTIONS = [
            'download_binary', 'download_by_url', 'download_file',
    ...
    def install_and_build_package(self, install_environment, tool_dependency,
                                  actions_dict):
                    ...
                    for action_tup in filtered_actions:
                        ...
                        with lcd(current_dir):
                            ...
                            if tool_dependency.status in [
                                    self.install_model.ToolDependency.
    ...
    def install_and_build_package_via_fabric(self, install_environment,
    ...
    def install_specified_tool_dependencies(self,
                                            tool_shed_repository,
        ...
        """
        attr_tups_of_dependencies_for_install = [(td.name, td.version, td.type)
        ...
        for elem in elems:
            ...
            if (name and type == 'set_environment') or (name and version):
                ...
                try:
                    index = attr_tups_of_dependencies_for_install.index(
                        attr_tup)
                except ValueError:
                    index = None
                if index is not None:
                    ...
                    if (tool_dependency.type == 'package'
                            and proceed_with_install):
                        try:
                            tool_dependency = self.install_package(
                                elem,
                                tool_shed_repository,
                                tool_dependencies=tool_dependencies,
                                from_tool_migration_manager=
                                from_tool_migration_manager)
                        except Exception as e:
    ...
    def install_via_fabric(self,
        ...
        if actions_elem is not None:
            elems = actions_elem
            if elems.get('os') is not None and elems.get(
        ...
        if custom_fabfile_path is not None:
            ...
            tool_dependency = self.install_and_build_package_via_fabric(
                install_environment, tool_shed_repository, tool_dependency,
                actions_dict)
        return tool_dependency
    ...
    def install_package(self,
        ...
        if tool_dependencies and package_name and package_version:
            ...
            for tool_dependency in tool_dependencies:
                if package_name == str(
                        tool_dependency.name) and package_version == str(
    ...
    def mark_tool_dependency_installed(self, tool_dependency):
        if tool_dependency.status not in [
            ...
            log.debug('Changing status for tool dependency %s from %s to %s.' %
                      (str(tool_dependency.name), str(tool_dependency.status),
                       str(self.install_model.ToolDependency.
                           installation_status.INSTALLED)))
...
class InstallRepositoryManager(object):
    ...
    def get_repository_components_for_installation(
            self, encoded_tsr_id, encoded_tsr_ids, repo_info_dicts,
    ...
    def __get_install_info_from_tool_shed(self, tool_shed_url, name, owner,
                                          changeset_revision):
        params = dict(
            name=str(name),
            owner=str(owner),
    ...
    def __handle_repository_contents(self,
                                     tool_shed_repository,
                                     tool_path,
        ...
        if 'tools' in irmm_metadata_dict:
            ...
            self.update_tool_shed_repository_status(
                tool_shed_repository, self.install_model.ToolShedRepository.
                installation_status.SETTING_TOOL_VERSIONS)
            tool_version_dicts = fetch_tool_versions(self.app,
                                                     tool_shed_repository)
            ...
            if repository_tools_tups:
                ...
                repository_tools_tups, sample_files_copied = tool_util.handle_missing_index_file(
                    self.app, tool_path, sample_files, repository_tools_tups,
                    sample_files_copied)
        ...
        if 'data_manager' in irmm_metadata_dict:
            ...
            dmh.install_data_managers(
                ...
                irmm_metadata_dict, shed_config_dict, relative_install_dir,
                tool_shed_repository, repository_tools_tups)
        if 'datatypes' in irmm_metadata_dict:
            self.update_tool_shed_repository_status(
                tool_shed_repository, self.install_model.ToolShedRepository.
                installation_status.LOADING_PROPRIETARY_DATATYPES)
            ...
            if shed_config_dict.get('tool_path'):
                files_dir = os.path.join(shed_config_dict['tool_path'],
                                         files_dir)
    ...
    def initiate_repository_installation(self, installation_dict):
        ...
        filtered_repo_info_dicts = installation_dict[
            'filtered_repo_info_dicts']
        has_repository_dependencies = installation_dict[
            'has_repository_dependencies']
        includes_tool_dependencies = installation_dict[
        ...
        install_tool_dependencies = installation_dict[
        ...
        if includes_tools_for_display_in_tool_panel and (
            ...
            for tool_guid in tool_panel_section_mapping:
                if tool_panel_section_mapping[tool_guid]['action'] == 'create':
                    new_tool_panel_section_name = tool_panel_section_mapping[
                        tool_guid]['tool_panel_section']
                    log.debug('Creating tool panel section "%s" for tool %s' %
                              (new_tool_panel_section_name, tool_guid))
                    self.tpm.handle_tool_panel_section(
                        self.app.toolbox, None, tool_panel_section_mapping[
                            tool_guid]['tool_panel_section'])
        ...
        for tsr_id in tsr_ids:
            tsr = self.install_model.context.query(
                self.install_model.ToolShedRepository).get(tsr_id)
        ...
        query = self.install_model.context.query(
            self.install_model.ToolShedRepository).filter(or_(*clause_list))
        return encoded_kwd, query, tool_shed_repositories, encoded_repository_ids

    def install(self, tool_shed_url, name, owner, changeset_revision,
                install_options):
        ...
        installed_tool_shed_repositories = self.__initiate_and_install_repositories(
            tool_shed_url, repository_revision_dict, repo_info_dicts,
            install_options)
        return installed_tool_shed_repositories

    def __initiate_and_install_repositories(self, tool_shed_url,
                                            repository_revision_dict,
                                            repo_info_dicts, install_options):
        try:
        ...
        if shed_tool_conf:
            ...
            try:
                shed_config_dict = self.app.toolbox.dynamic_confs(
                    include_migrated_tool_conf=False)[0]
            except IndexError:
        ...
        if created_or_updated_tool_shed_repositories:
            ...
            return self.install_repositories(
                tsr_ids, decoded_kwd, reinstalling=False)
    ...
    def install_repositories(self, tsr_ids, decoded_kwd, reinstalling):
        ...
        tool_panel_section_keys = util.listify(
            decoded_kwd['tool_panel_section_keys'])
        tool_panel_section_mapping = decoded_kwd.get(
            'tool_panel_section_mapping', {})
        repo_info_dicts = util.listify(decoded_kwd['repo_info_dicts'])
        install_resolver_dependencies = decoded_kwd[
            'install_resolver_dependencies']
        ...
        for tsr_id in ordered_tsr_ids:
            ...
            if repository.status in [
                    self.install_model.ToolShedRepository.installation_status.
                    NEW, self.install_model.ToolShedRepository.
        ...
        if repositories_for_installation:
            for tool_shed_repository, repo_info_dict, tool_panel_section_key in zip(
                    repositories_for_installation, filtered_repo_info_dicts,
                    filtered_tool_panel_section_keys):
    ...
    def install_tool_shed_repository(self,
                                     tool_shed_repository,
                                     repo_info_dict,
        ...
        if tool_panel_section_key:
            _, tool_section = self.app.toolbox.get_section(
                tool_panel_section_key)
        ...
        if cloned_ok:
            if reinstalling:
                ...
                if current_ctx_rev != ctx_rev:
                    repo = hg_util.get_repo_for_repository(
                        ...
                        create=False)
                    hg_util.pull_repository(repo, repository_clone_url,
            ...
            repository_util.set_repository_attributes(
                self.app,
                tool_shed_repository,
                status=self.install_model.ToolShedRepository.
                installation_status.ERROR,
                error_message=error_message,
                deleted=False,
                uninstalled=False,
    ...
    def order_components_for_installation(self, tsr_ids, repo_info_dicts,
        ...
        processed_tsr_ids = []
        while len(processed_tsr_ids) != len(
                prior_install_required_dict.keys()):
            ...
            if tsr_id not in ordered_tsr_ids:
                prior_install_required_ids = prior_install_required_dict[
                    tsr_id]
    ...
    def update_tool_shed_repository_status(self,
                                           tool_shed_repository,
                                           status,
                                           error_message=None):
        """
...
class RepositoriesInstalledException(
...
def fetch_tool_versions(app, tool_shed_repository):
    ...
    try:
        tool_shed_url = common_util.get_tool_shed_url_from_tool_shed_registry(
            app, str(tool_shed_repository.tool_shed))
        params = dict(
            name=str(tool_shed_repository.name),
        ...
        if text:
            ...
            raise Exception(
                "No content returned from Tool Shed repository version request to %s"
                % url)
    except Exception:
        log.exception(
            "Failed to fetch tool version information for Tool Shed repository."
        )
        raise
