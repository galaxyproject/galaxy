...
class InstalledRepositoryManager(object):
    ...
    def activate_repository(self, repository):
        ...
        if repository.includes_tools_for_display_in_tool_panel:
            ...
            if repository.includes_data_managers:
                ...
                dmh.install_data_managers(
                    ...
                    data_manager_relative_install_dir, repository,
                    repository_tools_tups)
        ...
        if repository.includes_datatypes:
            ...
            if installed_repository_dict:
                converter_path = installed_repository_dict.get(
                    'converter_path')
    ...
    def add_entry_to_installed_repository_dependencies_of_installed_repositories(
        ...
        if repository_tup not in self.installed_repository_dependencies_of_installed_repositories:
            ...
            log.debug(debug_msg)
            self.installed_repository_dependencies_of_installed_repositories[
                repository_tup] = repository_dependency_tups
        ...
        for required_repository_tup in repository_dependency_tups:
            ...
            if required_repository_tup in self.installed_dependent_repositories_of_installed_repositories:
                self.installed_dependent_repositories_of_installed_repositories[
                    required_repository_tup].append(repository_tup)
            else:
                self.installed_dependent_repositories_of_installed_repositories[
                    required_repository_tup] = [repository_tup]
    ...
    def add_entry_to_installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies(
        ...
        if tool_dependency_tup not in self.installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies:
            ...
            debug_msg = "Adding an entry for version %s of %s %s " % (version,
                                                                      type,
                                                                      name)
    ...
    def add_entry_to_installed_tool_dependencies_of_installed_repositories(
        ...
        if repository_tup not in self.installed_tool_dependencies_of_installed_repositories:
            ...
            self.installed_tool_dependencies_of_installed_repositories[
                repository_tup] = installed_tool_dependency_tups
    ...
    def add_entry_to_tool_dependencies_of_installed_repositories(
        ...
        if repository_tup not in self.tool_dependencies_of_installed_repositories:
            ...
            self.tool_dependencies_of_installed_repositories[
                repository_tup] = tool_dependency_tups

    def get_containing_repository_for_tool_dependency(self,
                                                      tool_dependency_tup):
        tool_shed_repository_id, name, version, type = tool_dependency_tup
        return self.app.install_model.context.query(
            self.app.install_model.ToolShedRepository).get(
                tool_shed_repository_id)

    def get_dependencies_for_repository(self,
                                        tool_shed_url,
                                        repo_info_dict,
        ...
        if repository_dependencies:
            ...
            if not repository:
                repository = repository_util.get_repository_for_dependency_relationship(
                    self.app, tool_shed_url, name, repository_owner,
                    changeset_revision)
            ...
            required_repo_info_dicts = all_repo_info_dict.get(
                'all_repo_info_dicts', [])
        ...
        if has_repository_dependencies:
            ...
            for tsr in repository.repository_dependencies:
                ...
                only_if_compiling_contained_td = self.set_only_if_compiling_contained_td(
                    repository, tsr)
                rd_tup = [
                    tsr.tool_shed, tsr.name, tsr.owner, tsr.changeset_revision,
                    prior_installation_required,
                    only_if_compiling_contained_td, tsr.id, tsr.status
                ]
            ...
            if installed_rd_tups or missing_rd_tups:
                ...
                repository_dependencies = metadata.get(
                ...
                root_key = container_util.generate_repository_dependencies_key_for_repository(
                    repository.tool_shed, repository.name, repository.owner,
                    repository.installed_changeset_revision,
                    prior_installation_required,
                ...
                for rd_tup in rd_tups:
                    ...
                    if repository:
                        new_rd_tup = [
                            ...
                            prior_installation_required,
                            only_if_compiling_contained_td, repository.id,
                            repository.status
                        ]
                        ...
                        new_rd_tup = [
                            tool_shed, name, owner, changeset_revision,
                            prior_installation_required,
                            only_if_compiling_contained_td, None,
                            'Never installed'
                        ]
                        if not util.asbool(only_if_compiling_contained_td):
            ...
            for td_key, val in tmp_tool_dependencies_dict.items():
                ...
                if td_key == 'set_environment':
                    ...
                    for requirement_dict in val:
                        ...
                        if tool_dependency_status in [
                            ...
                            if td_key in installed_tool_dependencies:
                                installed_tool_dependencies[td_key].extend(
                                    new_val)
                            else:
                                installed_tool_dependencies[td_key] = new_val
                        else:
                            if td_key in missing_tool_dependencies:
                                missing_tool_dependencies[td_key].extend(
                                    new_val)
                            else:
                    ...
                    tool_dependency = tool_dependency_util.get_tool_dependency_by_name_version_type(
                        self.app,
                        val.get('name', None),
                        val.get('version', None), val.get('type', 'package'))
                    if tool_dependency:
                        tool_dependency_status = tool_dependency.status
                    val['status'] = tool_dependency_status
                if tool_dependency_status in [
                        self.install_model.ToolDependency.installation_status.
                        INSTALLED
                ]:
    ...
    def get_repository_tuple_for_installed_repository_manager(
            self, repository):
        return (str(repository.tool_shed), str(repository.name),
    ...
    def get_runtime_dependent_tool_dependency_tuples(self,
                                                     tool_dependency,
                                                     status=None):
        """
        ...
        """
        runtime_dependent_tool_dependency_tups = []
        required_env_shell_file_path = tool_dependency.get_env_shell_file_path(
            self.app)
        if required_env_shell_file_path:
            required_env_shell_file_path = os.path.abspath(
                required_env_shell_file_path)
                    ...
                    if env_shell_file_path is not None:
                        try:
                            ...
                            log.debug(
                                ...
                                % (str(env_shell_file_path), str(td.name),
                                   str(tool_dependency.name), str(e)))
                        if contents is not None and contents.find(
                                required_env_shell_file_path) >= 0:
                            td_tuple = self.get_tool_dependency_tuple_for_installed_repository_manager(
                                td)
                            runtime_dependent_tool_dependency_tups.append(
                                td_tuple)
    ...
    def get_tool_dependency_tuple_for_installed_repository_manager(
        ...
        return (tool_dependency.tool_shed_repository_id,
                str(tool_dependency.name), str(tool_dependency.version), type)
        ...
        for original_dependency_key, original_dependency_val_dict in original_dependency_dict.items(
            ...
            if original_dependency_key not in new_dependency_dict:
                updated_tool_dependency = self.update_existing_tool_dependency(
                    repository, original_dependency_val_dict,
                    new_dependency_dict)
                if updated_tool_dependency:
                    updated_tool_dependency_names.append(
                        updated_tool_dependency.name)
                else:
    ...
    def handle_repository_install(self, repository):
        ...
        self.add_entry_to_tool_dependencies_of_installed_repositories(
            repository)
    ...
    def handle_repository_uninstall(self, repository):
        ...
        for tool_dependency in repository.tool_dependencies:
            ...
            for (
                    td_tup, installed_runtime_dependent_tool_dependency_tups
            ) in self.installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies.items(
            ):
        ...
        self.remove_entry_from_tool_dependencies_of_installed_repositories(
            repository)
    ...
    def handle_tool_dependency_install(self, repository, tool_dependency):
        ...
        if tool_dependency.is_installed:
            ...
            if repository_tup in self.installed_tool_dependencies_of_installed_repositories:
                self.installed_tool_dependencies_of_installed_repositories[
                    repository_tup].append(tool_dependency_tup)
            else:
                self.installed_tool_dependencies_of_installed_repositories[
                    repository_tup] = [tool_dependency_tup]
    ...
    def load_proprietary_datatypes(self):
        ...
        for tool_shed_repository in self.context.query(self.install_model.ToolShedRepository) \
            ...
            relative_install_dir = self.get_repository_install_dir(
                tool_shed_repository)
            if relative_install_dir:
                installed_repository_dict = cdl.load_installed_datatypes(
                    tool_shed_repository, relative_install_dir)
                if installed_repository_dict:
                    self.installed_repository_dicts.append(
        ...
        if remove_from_disk:
            try:
                ...
                if isinstance(e, OSError
                              ) and not os.path.exists(repository_install_dir):
                    removed = True
                    log.debug(
    ...
    def remove_entry_from_installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies(
        ...
        if tool_dependency_tup in self.installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies:
            tool_shed_repository_id, name, version, type = tool_dependency_tup
            debug_msg = "Removing entry for version %s of %s %s " % (version,
                                                                     type,
                                                                     name)
    ...
    def remove_entry_from_repository_dependencies_of_installed_repositories(
        ...
        if repository_tup in self.repository_dependencies_of_installed_repositories:
            ...
            log.debug(debug_msg)
            del self.repository_dependencies_of_installed_repositories[
                repository_tup]
    ...
    def remove_entry_from_tool_dependencies_of_installed_repositories(
            self, repository):
        ...
        if repository_tup in self.tool_dependencies_of_installed_repositories:
            ...
            debug_msg = "Removing entry for revision %s of repository %s owned by %s from tool_dependencies_of_installed_repositories." % \
                (installed_changeset_revision, name, owner)
            log.debug(debug_msg)
            del self.tool_dependencies_of_installed_repositories[
                repository_tup]
    ...
    def set_only_if_compiling_contained_td(self, repository,
                                           required_repository):
        """
        ...
        required_repository_tup = [
            required_repository.tool_shed, required_repository.name,
            required_repository.owner, required_repository.changeset_revision
        ]
    ...
    def update_existing_tool_dependency(
        ...
        if tool_dependency and tool_dependency.can_update:
            dependency_install_dir = tool_dependency.installation_directory(
                self.app)
            ...
            if removed_from_disk:
                ...
                for new_dependency_key, new_dependency_val_dict in new_dependencies_dict.items(
                    ...
                    if original_name == new_dependency_val_dict['name']:
                        ...
                        new_dependency_version = new_dependency_val_dict[
                            'version']
                        break
                if new_dependency_name and new_dependency_type and new_dependency_version:
                    ...
                    log.debug(
                        ...
                        (str(tool_dependency.version),
                         str(tool_dependency.type), str(tool_dependency.name),
