...
class DataManagerHandler(object):
    ...
    def install_data_managers(self, shed_data_manager_conf_filename,
                              metadata_dict, shed_config_dict,
                              relative_install_dir, repository,
        ...
        if 'data_manager' in metadata_dict:
            ...
            repo_data_manager_conf_filename = metadata_dict[
                'data_manager'].get('config_filename', None)
            ...
            repo_data_manager_conf_filename = os.path.join(
                relative_repo_data_manager_dir,
                repo_data_manager_conf_filename)
            tree, error_message = xml_util.parse_xml(
                repo_data_manager_conf_filename)
            if tree is None:
                return rval
            ...
            for elem in root:
                if elem.tag == 'data_manager':
                    ...
                    if data_manager_id is None:
                        ...
                        continue
                    data_manager_dict = metadata_dict['data_manager'].get(
                        'data_managers', {}).get(data_manager_id, None)
                    ...
                    if tool is None:
                        log.error(
                            ...
                            % (tool_guid, data_manager_id))
                        continue
                    tool_config_filename = tool_dict.get(
                        'tool_config_filename', None)
                    if tool_config_filename is None:
                        log.error(
                            "Data manager metadata is missing 'tool_config_file' for '%s'."
                            % (data_manager_id))
                        continue
                    elem.set('shed_conf_file',
                             shed_config_dict['config_filename'])
                    if elem.get('tool_file', None) is not None:
                        del elem.attrib[
                            'tool_file']  # remove old tool_file info
                    tool_elem = tpm.generate_tool_elem(
                        repository.tool_shed, repository.name,
                    ...
                    log.warning("Encountered unexpected element '%s':\n%s" %
                                (elem.tag, xml_util.xml_to_string(elem)))
        ...
        if metadata_dict and 'data_manager' in metadata_dict:
            ...
            if tree:
                ...
                guids = [
                    ...
                    for data_manager_dict in metadata_dict.get(
                        'data_manager', {}).get('data_managers', {}).values()
