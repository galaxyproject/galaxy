...
class LintContext:
    ...
    def lint(
        ...
        try:
            ...
            # Attributed to the parse, not to whichever linter happened to touch the
            # unparseable part of the tool source first.
            self._report_failure(f"Tool could not be parsed: {unicodify(e)}", "ToolParse")
        except Exception as e:
            self._report_failure(f"Linter {name} failed: {unicodify(e)}", name)
