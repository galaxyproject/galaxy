...
from ._util import is_valid_cheetah_placeholder, node_props_factory
...
def lint_output(tool_xml, lint_ctx):
    ...
    if len(outputs) == 0:
        lint_ctx.warn("Tool contains no outputs section, most tools should produce outputs.", **node_props(tool_node))
        return
    if len(outputs) > 1:
        lint_ctx.warn("Tool contains multiple output sections, behavior undefined.", **node_props(outputs[1]))
    num_outputs = 0
    for output in list(outputs[0]):
        if output.tag not in ["data", "collection"]:
            lint_ctx.warn(f"Unknown element found in outputs [{output.tag}]", **node_props(output))
            continue
        num_outputs += 1
        if "name" not in output.attrib:
            lint_ctx.warn("Tool output doesn't define a name - this is likely a problem.", **node_props(output))
            ...
            lint_ctx.warn(f'Tool output name [{output.attrib["name"]}] is not a valid Cheetah placeholder.', **node_props(output))

        format_set = False
        if __check_format(output, lint_ctx, node_props):
            format_set = True
        if output.tag == "data":
            ...
            if "type" not in output.attrib:
                lint_ctx.warn("Collection output with undefined 'type' found.", **node_props(output))
            if "structured_like" in output.attrib and "inherit_format" in output.attrib:
                format_set = True
        for sub in output:
            if __check_pattern(sub):
                format_set = True
            elif __check_format(sub, lint_ctx, node_props, allow_ext=True):
                format_set = True

        if not format_set:
            lint_ctx.warn(f"Tool {output.tag} output {output.attrib.get('name', 'with missing name')} doesn't define an output format.", **node_props(output))
...
def __check_format(node, lint_ctx, node_props, allow_ext=False):
    """
    ...
    if "format_source" in node.attrib and ("ext" in node.attrib or "format" in node.attrib):
        lint_ctx.warn(f"Tool {node.tag} output '{node.attrib.get('name', 'with missing name')}' should use either format_source or format/ext", **node_props(node))
    ...
    if fmt == "input":
        lint_ctx.warn(f"Using format='input' on {node.tag}, format_source attribute is less ambiguous and should be used instead.", **node_props(node))
