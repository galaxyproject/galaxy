    ...
    class EmailColumn(grids.GridColumn):
        def filter(self, trans, user, query, column_filter):
            if column_filter == 'All':
                return query
            return query.filter(
                and_(model.StoredWorkflow.table.c.user_id == model.User.table.c.id, model.User.table.c.email == column_filter))
...
class Workflows(BaseUIController, ReportQueryBuilder):
    ...
    def specified_date_handler(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if operation == "workflow_per_month":
                ...
                return trans.response.send_redirect(web.url_for(controller='workflows', action='workflow_per_month', **kwd))
    ...
    def per_user(self, trans, **kwd):
        ...
        workflows = []
        q = sa.select(
            (model.User.table.c.email.label('user_email'), sa.func.count(model.StoredWorkflow.table.c.id).label('total_workflows')),
            from_obj=[sa.outerjoin(model.StoredWorkflow.table, model.User.table)],
            group_by=['user_email'],
            order_by=[_order],
            offset=offset,
        ...
        all_workflows_per_user = sa.select(
            (model.User.table.c.email.label('user_email'), self.select_day(model.StoredWorkflow.table.c.create_time).label('date'),
             model.StoredWorkflow.table.c.id),
            from_obj=[sa.outerjoin(model.StoredWorkflow.table, model.User.table)])
    ...
    def user_per_month(self, trans, **kwd):
        ...
        workflows = []
        for row in q.execute():
            workflows.append((row.date.strftime("%Y-%m"), row.total_workflows, row.date.strftime("%B"), row.date.strftime("%Y")))
        return trans.fill_template(
    ...
    def per_workflow(self, trans, **kwd):
        ...
        q = sa.select(
            (model.Workflow.table.c.id.label('workflow_id'), sa.func.min(model.Workflow.table.c.name).label('workflow_name'),
        ...
        all_runs_per_workflow = sa.select(
            (model.Workflow.table.c.id.label('workflow_id'), model.Workflow.table.c.name.label('workflow_name'),
