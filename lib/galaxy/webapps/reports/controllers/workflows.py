    ...
    class EmailColumn(grids.GridColumn):
        def filter(self, trans, user, query, column_filter):
            if column_filter == 'All':
                return query
            return query.filter(
                and_(model.StoredWorkflow.table.c.user_id == model.User.table.
                     c.id, model.User.table.c.email == column_filter))
    ...
    class SpecifiedDateColumn(grids.GridColumn):
        def filter(self, trans, user, query, column_filter):
            ...
            if column_filter.count('-') == 1:
                ...
                end_date = start_date + timedelta(days=calendar.monthrange(
                    year, month)[1])
                return query.filter(
                    and_(self.model_class.table.c.create_time >= start_date,
                         self.model_class.table.c.create_time < end_date))
...
class Workflows(BaseUIController, ReportQueryBuilder):
    ...
    def specified_date_handler(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if operation == "workflow_per_month":
                ...
                return trans.response.send_redirect(
                    web.url_for(
                        controller='workflows', action='user_per_month', **
                        kwd))
    ...
    def per_month_all(self, trans, **kwd):
        ...
        q = sa.select(
            (self.select_month(model.StoredWorkflow.table.c.create_time)
             .label('date'), sa.func.count(
        ...
        all_workflows = sa.select(
            (self.select_day(model.StoredWorkflow.table.c.create_time)
             .label('date'), model.StoredWorkflow.table.c.id))
    ...
    def per_user(self, trans, **kwd):
        ...
        workflows = []
        q = sa.select(
            (model.User.table.c.email.label('user_email'), sa.func.count(
                model.StoredWorkflow.table.c.id).label('total_workflows')),
            from_obj=[
                sa.outerjoin(model.StoredWorkflow.table, model.User.table)
            ],
        ...
        all_workflows_per_user = sa.select(
            (model.User.table.c.email.label('user_email'), self.select_day(
