...
class Users(BaseUIController, ReportQueryBuilder):
    ...
    def specified_date(self, trans, **kwd):
        ...
        year_label = start_date.strftime("%Y")
        q = sa.select(
            (self.select_day(galaxy.model.User.table.c.create_time)
             .label('date'), galaxy.model.User.table.c.email),
            whereclause=sa.and_(
                galaxy.model.User.table.c.create_time >= start_date,
                galaxy.model.User.table.c.create_time < end_date),
            from_obj=[galaxy.model.User.table],
    ...
    def history_per_user(self, trans, **kwd):
        ...
        histories = [(_.username
                      if _.username is not None else "Unknown", _.history)
