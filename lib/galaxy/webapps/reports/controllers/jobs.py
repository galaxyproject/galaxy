...
class Jobs( BaseController ):
    ...
    def specified_month_all( self, trans, **kwd ):
        ...
        month = params.get( 'id', year_month )
        ...
        return trans.fill_template( 'jobs_specified_month_all.mako', month=month, jobs=jobs, msg=msg )
    @web.expose
    def specified_month_in_error( self, trans, **kwd ):
        ...
        job_rows = engine.text( s ).execute().fetchall()
        for job in job_rows:
            jobs.append( ( job.day_label, job.year_month_day, job.num_jobs ) )
        return trans.fill_template( 'jobs_specified_month_in_error.mako', month=month, jobs=jobs, msg=msg )
    @web.expose
    def specified_date_in_error( self, trans, **kwd ):
        ...
        specified_date = params.get( 'id', today )
        s = """
        SELECT
            job.id AS id,
            job.state AS state,
        ...
        return trans.fill_template( 'jobs_specified_date_in_error.mako', specified_date=specified_date, jobs=jobs, msg=msg )
    @web.expose
        ...
        for job in job_rows:
            ...
            if job.remote_address:
                try:
                    ...
                    pass
            jobs.append( ( job.state, job.id, job.create_time, job.update_time, job.session_id, job.tool_id, job.user_email, remote_host, job.command_line, job.stderr, job.stack_trace, job.info ) )
        return trans.fill_template( 'jobs_specified_date_all.mako', specified_date=specified_date, jobs=jobs, msg=msg )
    @web.expose
    ...
    def per_month_in_error( self, trans, **kwd ):
        ...
        job_rows = engine.text( s ).execute().fetchall()
        for job in job_rows:
            jobs.append( ( job.year_month, job.num_jobs ) )
    ...
    def user_per_month( self, trans, **kwd ):
        ...
        engine = galaxy.model.mapping.metadata.engine
        jobs = []
        email = params.get( 'id', 'monitor@bx.psu.edu' )
        ...
        for job in job_rows:
            ...
            if job.remote_address:
                try:
                    ...
                    pass
            jobs.append( ( job.state, job.id, job.create_time, job.update_time, job.session_id, job.tool_id, job.user_email, remote_host, job.command_line, job.stderr, job.stack_trace, job.info ) )
        return trans.fill_template( 'jobs_user_for_month.mako', email=email, month=month, jobs=jobs, msg=msg )
    @web.expose
    ...
    def tool_per_month( self, trans, **kwd ):
        ...
        engine = galaxy.model.mapping.metadata.engine
        jobs = []
        tool_id = params.get( 'id', 'Add a column1' )
        s = """
        SELECT
            count(id) AS num_jobs,
            substr(create_time, 1, 7) AS year_month
        ...
        for job in job_rows:
            ...
            if job.remote_address:
                try:
                    ...
                    pass
            jobs.append( ( job.state, job.id, job.create_time, job.update_time, job.session_id, job.tool_id, job.user_email, remote_host, job.command_line, job.stderr, job.stack_trace, job.info ) )
        return trans.fill_template( 'jobs_tool_for_month.mako', tool_id=tool_id, month=month, jobs=jobs, msg=msg )
