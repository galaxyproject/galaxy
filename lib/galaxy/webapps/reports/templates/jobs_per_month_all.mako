...
<%def name="main_body()">
  <div class="reportBody">
    ...
    <h4 align="center">Click Year-Month to view the number of jobs for each day of that month</h4>
    %if msg:
      ...
      %else:
        ...
        %for job in jobs:
          ...
          %else:
            <tr class="tr">
          %endif
            <td><a href="${h.url_for( controller='jobs', action='specified_month_all', id=job[0] )}">${job[0]}</a></td>
            <td>${job[1]}</td>
            <td>${job[2]}</td>
            <td>${job[3]}</td>
          </tr>
