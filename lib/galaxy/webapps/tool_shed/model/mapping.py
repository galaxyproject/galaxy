...
APIKeys.table = Table("api_keys", metadata,
                      ...
                      Column(
                          ...
                          index=True),
                      Column(
                          "key", TrimmedString(32), index=True, unique=True))
...
GalaxySession.table = Table(
    ...
    Column("referer", TEXT),
    Column("session_key", TrimmedString(255), index=True,
           unique=True),  # unique 128 bit random number coerced to a string
...
ComponentReview.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey("repository_review.id"),
        index=True),
...
mapper(
    ...
    properties=dict(
        active_repositories=relation(
            Repository,
            primaryjoin=((Repository.table.c.user_id == User.table.c.id) &
        ...
        api_keys=relation(
            APIKeys,
            backref="user",
            order_by=desc(APIKeys.table.c.create_time))))
...
mapper(
    Group, Group.table, properties=dict(users=relation(UserGroupAssociation)))
...
mapper(
    ...
    properties=dict(
        user=relation(User, backref="groups"),
...
mapper(
    ...
    properties=dict(
        group=relation(Group, backref="roles"), role=relation(Role)))

mapper(
    GalaxySession, GalaxySession.table, properties=dict(user=relation(User)))

mapper(
    Tag,
    Tag.table,
    properties=dict(children=relation(
        Tag, backref=backref('parent', remote_side=[Tag.table.c.id]))))
...
mapper(
    ...
    properties=dict(repositories=relation(
        ...
        primaryjoin=(Category.table.c.id ==
                     RepositoryCategoryAssociation.table.c.category_id),
        secondaryjoin=(RepositoryCategoryAssociation.table.c.repository_id ==
                       Repository.table.c.id))))
...
mapper(
    ...
    properties=dict(
        ...
        downloadable_revisions=relation(
            RepositoryMetadata,
            primaryjoin=((Repository.table.c.id ==
        ...
        reviewers=relation(
            ...
            primaryjoin=(Repository.table.c.id ==
                         RepositoryReview.table.c.repository_id),
            secondaryjoin=(
                RepositoryReview.table.c.user_id == User.table.c.id))))
...
mapper(
    ...
    properties=dict(
        ...
        reviews=relation(
            ...
            primaryjoin=((RepositoryMetadata.table.c.repository_id ==
                          RepositoryReview.table.c.repository_id) &
                         (RepositoryMetadata.table.c.changeset_revision ==
...
mapper(
    ...
    properties=dict(
        category=relation(Category), repository=relation(Repository)))
