    ...
    def upload(self, trans, **kwd):
        ...
        if kwd.get('upload_button', False):
            if file_data == '' and url == '':
                ...
                try:
                    commands.clone(hg_util.get_configured_ui(), repo_url,
                                   uploaded_directory)
                except Exception as e:
                    message = 'Error uploading via mercurial clone: %s' % basic_util.to_html_string(
                        str(e))
                    status = 'error'
                    ...
                    if ok:
                        ...
                        if ok:
                            ...
                            if full_path.endswith(
                                ...
                                # dictionary.
                                error, error_message = tdtm.handle_sample_tool_data_table_conf_file(
        ...
        for root, dirs, files in os.walk(uploaded_directory):
            for uploaded_file in files:
                relative_path = os.path.normpath(
                    os.path.join(
                        os.path.relpath(root, uploaded_directory),
                        uploaded_file))
