...
class RepositoryReviewController(BaseUIController, ratings_util.ItemRatings):
    ...
    def browse_components(self, trans, **kwd):
        if 'operation' in kwd:
            ...
            if operation == "create":
                return trans.response.send_redirect(web.url_for(controller='repository_review', action='create_component', **kwd))
        ...
        if kwd.get('create_component_button', False):
            if not name or not description:
                ...
                trans.response.send_redirect(
                    web.url_for(controller='repository_review', action='manage_components', message=message, status=status))
        return trans.fill_template(
        ...
        if repository_id:
            if changeset_revision:
                ...
                if review_util.get_review_by_repository_id_changeset_revision_user_id(
                    ...
                    message = "You have already created a review for revision <b>%s</b> of repository <b>%s</b>." % (
                    ...
                    if not create_without_copying and \
                        ...
                        return trans.response.send_redirect(
                            web.url_for(controller='repository_review', action='select_previous_review', **kwd))
                    ...
                    if repository_metadata:
                        ...
                        if metadata:
                            ...
                            trans.response.send_redirect(
                                web.url_for(
                                    controller='repository_review', action='edit_review', id=review_id, message=message, status=status))
                    else:
                ...
                return trans.response.send_redirect(web.url_for(controller='repository_review', action='manage_repository_reviews', **kwd))
        return trans.response.send_redirect(web.url_for(controller='repository_review', action='view_or_manage_repository', **kwd))
        ...
        if kwd.get('edit_component_button', False):
            ...
            if component.description != new_description:
                ...
                return trans.response.send_redirect(
                    web.url_for(controller='repository_review', action='manage_components', message=message, status=status))
        return trans.fill_template(
            '/webapps/tool_shed/repository_review/edit_component.mako', component=component, message=message, status=status)
    ...
    def edit_review(self, trans, **kwd):
                ...
                if component_review:
                    ...
                    if comment or private or approved != trans.model.ComponentReview.approved_states.NO or rating:
                        component_review = trans.model.ComponentReview(
                            repository_review_id=review.id, component_id=component.id, comment=comment, approved=approved, rating=rating)
                        trans.sa_session.add(component_review)
                        trans.sa_session.flush()
                        flushed = True
                        saved_component_names.append(component_name)
        ...
        revision_approved_select_field = grids_util.build_approved_select_field(
            trans, name='revision_approved', selected_value=selected_value, for_component=False)
        rev, changeset_revision_label = hg_util.get_rev_label_from_changeset_revision(repo, review.changeset_revision)
        return trans.fill_template(
    ...
    def manage_components(self, trans, **kwd):
        if 'operation' in kwd:
            ...
            if operation == "create":
                ...
                return trans.response.send_redirect(web.url_for(controller='repository_review', action='edit_component', **kwd))
    ...
    def manage_repositories_ready_for_review(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if operation == "inspect repository revisions":
                return trans.response.send_redirect(web.url_for(controller='repository_review', action='create_review', **kwd))
    ...
    def manage_repositories_reviewed_by_me(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            kwd['mine'] = True
            return trans.response.send_redirect(
                web.url_for(controller='repository_review', action='manage_repositories_with_reviews', **kwd))
    ...
    def manage_repository_reviews_of_revision(self, trans, **kwd):
        ...
        rev, changeset_revision_label = hg_util.get_rev_label_from_changeset_revision(repo, changeset_revision)
        reviews = review_util.get_reviews_by_repository_id_changeset_revision(trans.app, repository_id, changeset_revision)
        return trans.fill_template(
    ...
    def select_previous_review(self, trans, **kwd):
        ...
        changeset_revision = kwd.get('changeset_revision', None)
        repo = hg_util.get_repo_for_repository(trans.app, repository=repository, repo_path=None, create=False)
        previous_reviews_dict = review_util.get_previous_repository_reviews(trans.app, repository, changeset_revision)
        rev, changeset_revision_label = hg_util.get_rev_label_from_changeset_revision(repo, changeset_revision)
        return trans.fill_template(
    ...
    def view_or_manage_repository(self, trans, **kwd):
        ...
        if trans.user_is_admin() or repository.user == trans.user:
            return trans.response.send_redirect(web.url_for(controller='repository', action='manage_repository', **kwd))
        else:
            return trans.response.send_redirect(web.url_for(controller='repository', action='view_repository', **kwd))
