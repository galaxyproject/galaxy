...
class DatasetsController(BaseAPIController, UsesVisualizationMixin):
    ...
    def show(self, trans, id, hda_ldda='hda', data_type=None, provider=None, **kwd):
        ...
        try:
            if data_type == 'state':
                ...
                rval = self._converted_datasets_state(trans, dataset, kwd.get('chrom', None), is_true(kwd.get('retry', False)))
            elif data_type == 'data':
                ...
                if hda_ldda == 'hda':
                    return self.hda_serializer.serialize_to_view(dataset, view=kwd.get('view', 'detailed'), user=trans.user, trans=trans)
                else:
        ...
        if isinstance(data_provider, (SamDataProvider, BamDataProvider)):
            ...
            if dataset.dbkey:
                ...
                region = self.app.genomes.reference(
                    trans, dbkey=dataset.dbkey, chrom=chrom, low=(max(0, int(low) - 1000000)), high=(int(high) + 1000000))
    ...
    def display(self, trans, history_content_id, history_id, preview=False, filename=None, to_ext=None, raw=False, **kwd):
        """
            ...
            if raw:
                if filename and filename != 'index':
                    file_path = trans.app.object_store.get_filename(
                        hda.dataset, extra_dir=('dataset_%s_files' % hda.dataset.id), alt_name=filename)
                else:
