...
class DatasetsController(BaseAPIController, UsesVisualizationMixin):
    ...
    def show(self,
        ...
        try:
            dataset = self.get_hda_or_ldda(
                trans, hda_ldda=hda_ldda, dataset_id=id)
        ...
        try:
            if data_type == 'state':
                ...
                rval = self._converted_datasets_state(
                    trans, dataset,
                    kwd.get('chrom', None), is_true(kwd.get('retry', False)))
            elif data_type == 'data':
    ...
    def _converted_datasets_state(self,
                                  trans,
                                  dataset,
                                  chrom=None,
                                  retry=False):
        ...
        return {
            "status": dataset.conversion_messages.DATA,
            "valid_chroms": None
        }
    ...
    def _data(self,
        ...
        if mode == "Auto":
            ...
            features_per_pixel = stats['data']['max'] * float(
                kwargs['resolution'])
        ...
        result = data_provider.get_data(
            ...
            **kwargs)
        result.update({
            'dataset_type': data_provider.dataset_type,
    ...
    def display(self,
        ...
        try:
            ...
            trans.response.status = 500
            rval = (
                "Could not get display data for dataset: " + str(exception))
        ...
        try:
            ...
            trans.response.headers[
                "Content-Disposition"] = 'attachment; filename="Galaxy%s-[%s].%s"' % (
                    hda.hid, fname, file_ext)
            return open(hda.metadata.get(metadata_file).file_name)
        except Exception as exception:
            log.error(
                "Error getting metadata_file (%s) for dataset (%s) from history (%s): %s",
                metadata_file,
                history_content_id,
                history_id,
                str(exception),
    ...
    def converted(self, trans, dataset_id, ext, **kwargs):
        ...
        *existing* converted datasets.
        ...
        return self.hda_serializer.serialize_converted_datasets(
            hda, 'converted')
    ...
    def _get_or_create_converted(self, trans, original, target_ext, **kwargs):
        try:
            ...
            exc_data = dict(
                source=original.ext,
                target=target_ext,
                available=original.get_converter_types().keys())
