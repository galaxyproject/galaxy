...
async def job_lock_status(job_manager=Depends(get_job_manager), admin_user=Depends(get_admin_user)) -> JobLock:
    """Get job lock status."""
    return job_manager.job_lock()


@router.put('/api/job_lock')
async def update_job_lock(job_manager=Depends(get_job_manager), admin_user=Depends(get_admin_user), job_lock: JobLock = Body(...)) -> JobLock:
    """Set job lock status."""
