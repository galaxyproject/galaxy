                ...
                for filename in filenames:
                    ...
                    response.append(
                        dict(
                            ...
                            ctime=time.strftime("%m/%d/%Y %I:%M:%S %p",
                                                time.localtime(
                                                    statinfo.st_ctime))))
        else:
            ...
            for (dirpath, dirnames, filenames) in os.walk(directory):
                for dirname in dirnames:
                    ...
                    dir_path_hash = hashlib.sha1(
                        unicodify(dir_path).encode('utf-8')).hexdigest()
                    disabled = True if disable == 'folders' else False
                    jstree_paths.append(
                        jstree.Path(dir_path, dir_path_hash, {
                            'type': 'folder',
                            'state': {
                                'disabled': disabled
                            },
                ...
                for filename in filenames:
                    ...
                    file_path_hash = hashlib.sha1(
                        unicodify(file_path).encode('utf-8')).hexdigest()
                    disabled = True if disable == 'files' else False
                    jstree_paths.append(
                        jstree.Path(file_path, file_path_hash, {
                            'type': 'file',
                            'state': {
                                'disabled': disabled
                            },
