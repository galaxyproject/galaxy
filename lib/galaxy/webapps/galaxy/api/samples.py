...
class SamplesAPIController(BaseAPIController):
    update_types = Bunch(
        SAMPLE=['sample_state', 'run_details'],
    ...
    def update(self, trans, id, payload, **kwd):
        ...
        try:
            ...
            trans.response.status = 400
            return "Malformed sample_id (%s) specified, unable to decode." % str(
                sample_id)
        try:
            sample = trans.sa_session.query(
                trans.app.model.Sample).get(decoded_sample_id)
        except:
            sample = None
        if not sample:
            trans.response.status = 400
        ...
        if update_type == 'run_details':
            ...
            if deferred_plugin:
                try:
                    trans.app.job_manager.deferred_job_queue.plugins[
                        deferred_plugin].create_job(
                            trans, sample=sample, **payload)
                except:
                    log.exception(
                        'update() called with a deferred job plugin (%s) but creating the deferred job failed:'
                        % deferred_plugin)
            status, output = requests_admin_controller.edit_template_info(
                trans,
                cntrller='api',
                item_type='sample',
            ...
            return self.__update_sample_state(trans, sample, sample_id,
                                              **payload)
    ...
    def __update_sample_state(self, trans, sample, encoded_sample_id,
                              **payload):
        ...
        status, output = requests_common_cntrller.update_sample_state(
            trans=trans,
            cntrller='api',
    ...
    def __update_sample_dataset_status(self, trans, **payload):
        ...
        status, output = requests_admin_cntrller.update_sample_dataset_status(
            trans=trans,
            cntrller='api',
