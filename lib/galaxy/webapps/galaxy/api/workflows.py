...
from galaxy.web.base.controller import (BaseAPIController, SharableMixin,
...
class WorkflowsAPIController(BaseAPIController, UsesStoredWorkflowMixin,
    ...
    def get_workflows_list(self, trans, kwd):
        ...
        """
        show_published = util.string_as_bool(
            kwd.get('show_published', 'False'))
        ...
        for wf in trans.sa_session.query(trans.app.model.StoredWorkflow).filter(
                filter1,
                trans.app.model.StoredWorkflow.table.c.deleted == false()
        ).order_by(desc(
                trans.app.model.StoredWorkflow.table.c.update_time)).all():
        ...
        for wf_sa in trans.sa_session.query(
            ...
            item['number_of_steps'] = len(
                wf_sa.stored_workflow.latest_workflow.steps)
            item['show_in_tool_panel'] = False
            for x in user.stored_workflow_menu_entries:
    ...
    def show(self, trans, id, **kwd):
        ...
        return self.workflow_contents_manager.workflow_to_dict(
            trans, stored_workflow, style=style)
    ...
    def create(self, trans, payload, **kwd):
        ...
        if 'installed_repository_file' in payload:
            ...
            result = workflow_controller.import_workflow(
                trans=trans, cntrller='api', **payload)
            return result
        ...
        invocation_response = self.__encode_invocation(
            trans, invocation, step_details=kwd.get('step_details', False))
    ...
    def update(self, trans, id, payload, **kwds):
        ...
        * PUT /api/workflows/{id}
        ...
        stored_workflow = self.__get_stored_workflow(trans, id)
        if 'workflow' in payload:
            stored_workflow.name = sanitize_html(payload['name']) if (
                'name' in payload) else stored_workflow.name

            if 'annotation' in payload:
                newAnnotation = sanitize_html(payload['annotation'])
                self.add_item_annotation(trans.sa_session,
                                         trans.get_user(), stored_workflow,
                                         newAnnotation)
    ...
    def _get_tool(self, id, tool_version=None, user=None):
        ...
        if not tool or not tool.allow_user_access(user):
            raise exceptions.ObjectNotFound(
                "Could not find tool with id '%s'" % id)
        return tool
    ...
    def index_invocations(self, trans, workflow_id, **kwd):
        ...
        results = self.workflow_manager.build_invocations_query(
            trans, stored_workflow.id)
        out = []
        for r in results:
    ...
    def show_invocation(self, trans, workflow_id, invocation_id, **kwd):
        ...
        if workflow_invocation:
            return self.__encode_invocation(
                trans,
                workflow_invocation,
                step_details=kwd.get('step_details', False))
        return None
    ...
    @expose_api
    def invocation_step(self, trans, workflow_id, invocation_id, step_id,
                        **kwd):
        """
    ...
    @expose_api
    def update_invocation_step(self, trans, workflow_id, invocation_id,
                               step_id, payload, **kwd):
        """
    ...
    def __encode_invocation(self,
        ...
        return self.encode_all_ids(trans,
                                   invocation.to_dict(
                                       view, step_details=step_details), True)
