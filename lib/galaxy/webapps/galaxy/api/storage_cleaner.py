...
def discarded_histories_summary(
    ...
    return service.get_discarded_summary(trans, stored_item_type="history")
...
def discarded_histories(
    ...
    return service.get_discarded(trans, "history", offset, limit, order)
...
def cleanup_histories(
    trans: ProvidesHistoryContext = DependsOnTrans,
    payload: CleanupStorageItemsRequest = Body(...),
    service: StorageCleanerService = depends(StorageCleanerService),
) -> StorageItemsCleanupResult:
    """
    **Warning**: This operation cannot be undone. All objects will be deleted permanently from the disk.
    """
    return service.cleanup_items(trans, stored_item_type="history", item_ids=set(payload.item_ids))
...
def discarded_datasets_summary(
    ...
    return service.get_discarded_summary(trans, stored_item_type="dataset")
...
def discarded_datasets(
    ...
    return service.get_discarded(trans, "dataset", offset, limit, order)
...
def cleanup_datasets(
    ...
    **Warning**: This operation cannot be undone. All objects will be deleted permanently from the disk.
    """
    return service.cleanup_items(trans, stored_item_type="dataset", item_ids=set(payload.item_ids))
...
def archived_histories_summary(
    ...
    return service.get_archived_summary(trans, stored_item_type="history")
...
def archived_histories(
    ...
    return service.get_archived(trans, "history", offset, limit, order)
