...
class GroupRolesAPIController(BaseAPIController):
    ...
    def index(self, trans, group_id, **kwd):
        ...
        try:
            for gra in group.roles:
                ...
                rval.append(dict(id=encoded_id, name=role.name, url=url_for(
                    'group_role',
                    group_id=group_id,
                    id=encoded_id, )))
    ...
    def show(self, trans, id, group_id, **kwd):
        ...
        try:
            ...
            for gra in group.roles:
                if gra.role == role:
                    item = dict(id=role_id, name=role.name, url=url_for('group_role', group_id=group_id, id=role_id))  # TODO Fix This
            if not item:
    ...
    def update(self, trans, id, group_id, **kwd):
        ...
        try:
            ...
            for gra in group.roles:
                if gra.role == role:
                    item = dict(id=role_id, name=role.name, url=url_for('group_role', group_id=group_id, id=role_id))
            if not item:
