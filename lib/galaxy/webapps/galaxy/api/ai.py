"""
OpenAI adapter for Galaxy.

This module provides a narrowly scoped adapter for proxying OpenAI-compatible
chat completion requests on behalf of Galaxy-managed clients.

It is intentionally not a generic OpenAI proxy. The adapter exists to support
specific Galaxy integrations (for example Jupyternaut) while enforcing Galaxy
configuration, authentication, rate limiting, and request normalization.

The design goal is to expose a controlled, auditable interface rather than a
general-purpose AI service endpoint.
"""
...
class FastAPIAI:
    ...
    @router.post("/api/ai/plugins/{plugin_name}/chat/completions", unstable=True)
    @limiter.limit("30/minute")
    async def ai_plugins_adapter(
        self,
        request: Request,
