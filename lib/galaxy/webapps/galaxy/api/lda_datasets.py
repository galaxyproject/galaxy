...
class LibraryDatasetsController(BaseAPIController, UsesVisualizationMixin):
        ...
        * GET /api/libraries/datasets/{encoded_dataset_id}
        ...
        try:
            library_dataset = self.get_library_dataset(
                trans, id=id, check_ownership=False, check_accessible=True)
        except Exception:
            raise exceptions.ObjectNotFound(
                'Requested library_dataset was not found.')
        ...
        dataset_item = (trans.security.encode_id(library_dataset.id),
        ...
        rval['file_size'] = util.nice_size(
            int(library_dataset.library_dataset_dataset_association.get_size())
        )
        rval[
            'date_uploaded'] = library_dataset.library_dataset_dataset_association.create_time.strftime(
                "%Y-%m-%d %I:%M %p")
        rval[
            'can_user_modify'] = trans.app.security_agent.can_modify_library_item(
                current_user_roles, library_dataset) or trans.user_is_admin()
        ...
        rval['tags'] = tag_manager.get_tags_str(
            library_dataset.library_dataset_dataset_association.tags)
    ...
    def _get_current_roles(self, trans, library_dataset):
        ...
        access_dataset_role_list = [(access_role.name,
                                     trans.security.encode_id(access_role.id))
                                    for access_role in access_roles]
        manage_dataset_role_list = [(manage_role.name,
                                     trans.security.encode_id(manage_role.id))
                                    for manage_role in manage_roles]
        modify_item_role_list = [(modify_role.name,
        ...
        if action is None:
            ...
            if new_access_roles_ids is None:
                ...
                for role_id in new_access_roles_ids:
                    role = self.role_manager.get(trans,
                                                 self.__decode_id(
                                                     trans, role_id, 'role'))
    ...
    def delete(self, trans, encoded_dataset_id, **kwd):
        ...
        * DELETE /api/libraries/datasets/{encoded_dataset_id}
        ...
        try:
            dataset = self.get_library_dataset(
                trans,
                id=encoded_dataset_id,
                check_ownership=False,
                check_accessible=False)
        ...
        * POST /api/libraries/datasets
        ...
        if source in ["userdir_file", "importdir_file"]:
            ...
            abspath_datasets.append(
                trans.webapp.controllers['library_common']
                .make_library_uploaded_dataset(trans, 'api', kwd,
                ...
                if is_composite:  # need to add all the components from the extra_files_path to the zip
                    ...
                    try:
                        if format == 'zip':
                            ...
                            archive.add(
                                ldda.dataset.file_name, path, check_file=True)
                    except IOError:
                        log.exception(
    ...
    def _build_path(self, trans, folder):
        ...
        if folder.parent_id is None:
            path_to_root.append(('F' + trans.security.encode_id(folder.id),
                                 folder.name))
        else:
            # We add the current folder and traverse up one folder.
            path_to_root.append(('F' + trans.security.encode_id(folder.id),
                                 folder.name))
            upper_folder = trans.sa_session.query(
                trans.app.model.LibraryFolder).get(folder.parent_id)
    ...
    def __decode_id(self, trans, encoded_id, object_name=None):
        ...
        try:
            ...
            raise exceptions.MalformedId(
                ...
                if object_name is not None else '')
        except ValueError:
            raise exceptions.MalformedId(
