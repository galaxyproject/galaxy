...
class DatasetInterface(BaseUIController, UsesAnnotations, UsesItemRatings,
    ...
    def _get_job_for_dataset(self, trans, dataset_id):
        ...
        '''
        hda = trans.sa_session.query(trans.app.model.HistoryDatasetAssociation
                                     ).get(self.decode_id(dataset_id))
        assert hda and self._can_access_dataset(trans, hda)
        return hda.creating_job

    def _can_access_dataset(self,
                            trans,
                            dataset_association,
                            allow_admin=True,
        ...
        return (allow_admin and trans.user_is_admin()
    ...
    def get_metadata_file(self, trans, hda_id, metadata_name):
        ...
        file_ext = data.metadata.spec.get(metadata_name).get(
            "file_ext", metadata_name)
    ...
    def _check_dataset(self, trans, hda_id):
        ...
        if data.deleted and not (trans.user_is_admin() or
                                 (data.history
                                  and trans.get_user() == data.history.user)):
            return trans.show_error_message(
    ...
    def transfer_status(self, trans, dataset_id, filename=None):
        ...
        if isinstance(data, string_types):
            return data
        log.debug(
            "Checking transfer status for dataset %s..." % data.dataset.id)
    ...
    def edit(self, trans, dataset_id=None, filename=None, hid=None, **kwd):
        ...
        if hid is not None:
            ...
            data = trans.sa_session.query(
                self.app.model.HistoryDatasetAssociation).get(id)
        else:
            trans.log_event(
                "dataset_id and hid are both None, cannot load a dataset to edit"
            )
            return {
                status: 'error',
        ...
        if data is None:
            trans.log_event(
                "Problem retrieving dataset (encoded: %s, decoded: %s) with history id %s."
                % (str(dataset_id), str(id), str(hid)))
            return {status: 'error', message: "History dataset id is invalid."}
        if dataset_id is not None and data.history.user is not None and data.history.user != trans.user:
            trans.log_event(
                "User attempted to edit an HDA they do not own (encoded: %s, decoded: %s)."
                % (dataset_id, id))
            # Do not reveal the dataset's existence
            return {status: 'error', message: "History dataset id is invalid."}
        current_user_roles = trans.get_current_user_roles()
        if data.history.user and not data.dataset.has_manage_permissions_roles(
                trans):
        ...
        if self._can_access_dataset(trans, data):
            ...
            if params.change:
                ...
                if data.datatype.allow_datatype_change and trans.app.datatypes_registry.get_datatype_by_extension(
                        params.datatype).allow_datatype_change:
                    ...
                    if not __ok_to_edit_metadata(data.id):
                        ...
                        message = "Changed the type of dataset %s to %s." % (
                            to_unicode(data.name), params.datatype)
                else:
                    message = "You are unable to change datatypes in this manner. Changing %s to %s is not allowed." % (
                        data.extension, params.datatype)
                    error = True
            elif params.save:
                ...
                if __ok_to_edit_metadata(data.id):
                    ...
                    for name, spec in data.datatype.metadata_spec.items():
                        ...
                        setattr(data.metadata, name,
                                spec.unwrap(params.get(name, None)))
                    data.datatype.after_setting_metadata(data)
                    # Sanitize annotation before adding it.
                    if params.annotation:
                        annotation = sanitize_html(params.annotation, 'utf-8',
                                                   'text/html')
                ...
                if target_type:
                    message = data.datatype.convert_dataset(
                        trans, data, target_type)
                ...
                if not trans.user:
                    return {
                        ...
                        'error',
                        message:
                        "You must be logged in if you want to change permissions."
                    }
                if trans.app.security_agent.can_manage_dataset(
                    ...
                    if error:
                        ...
                        permissions[
                            access_action] = data.dataset.get_access_roles(
                                trans)
                        status = 'error'
                    else:
            ...
            ldatatypes = [
                ...
                for dtype_name, dtype_value in trans.app.datatypes_registry.
                datatypes_by_extension.iteritems()
                if dtype_value.allow_datatype_change
            ]
            ldatatypes.sort()
            all_roles = trans.app.security_agent.get_legitimate_roles(
                trans, data.dataset, 'root')
            data_metadata = [(name, spec)
                             for name, spec in data.metadata.spec.items()]
            converters_collection = [
                (key, value.name)
                for key, value in data.get_converter_types().items()
            ]
            ...
            edit_attributes_inputs.append({
                ...
                'Annotation',
                'value':
                self.get_item_annotation_str(trans.sa_session, trans.user,
                                             data),
                'help':
            ...
            for name, spec in data_metadata:
                if spec.visible:
                    attributes = data.metadata.get_metadata_parameter(
                        name, trans=trans)
                    if type(attributes) is form_builder.SelectField:
                        edit_attributes_inputs.append({
                            ...
                            'value':
                            attributes.value
                            if attributes.multiple else [attributes.value]
                        })
            ...
            convert_datatype_inputs.append({
                ...
                'options': [(ext_name, ext_id)
                            for ext_id, ext_name in ldatatypes],
                'value': [
                    ext_id for ext_id, ext_name in ldatatypes
                    if ext_id == data.ext
                ]
            })
            ...
            if can_manage_dataset:
                ...
                for index, action in permitted_actions:
                    ...
                    permission_inputs.append({
                        ...
                        'value':
                        saved_role_ids[action.action]
                        if action.action in saved_role_ids else []
                    })
            elif trans.user:
    ...
    def list(self, trans, **kwargs):
        ...
        if 'operation' in kwargs:
            ...
            if hdas:
                if operation == "switch" or operation == "switch_history":
                    ...
                    status, message = trans.webapp.controllers[
                        'history']._list_switch(trans, histories)

                    # Current history changed, refresh history frame; if switching to a dataset, set hda seek.
                    kwargs['refresh_frames'] = ['history']
                    if operation == "switch":
                        hda_ids = [
                            trans.security.encode_id(hda.id) for hda in hdas
                        ]
        ...
        return self.stored_list_grid(
            trans, status=status, message=message, **kwargs)
    ...
    def imp(self, trans, dataset_id=None, **kwd):
        ...
        if not dataset_id:
            return trans.show_error_message(
                "You must specify a dataset to import. You can %s." %
                referer_message,
                use_panels=True)
        # Do import.
        ...
        message = "Dataset imported. <br>You can <a href='%s'>start using the dataset</a> or %s." % (
    ...
    @web.expose
    def display_by_username_and_slug(self,
                                     trans,
                                     username,
                                     slug,
                                     filename=None,
        ...
        if dataset:
            ...
            if isinstance(dataset.datatype,
                          datatypes.binary.Binary) or isinstance(
                              dataset.datatype,
                              datatypes.images.Image) or isinstance(
                                  dataset.datatype, datatypes.text.Html):
    ...
    def annotate_async(self, trans, id, new_annotation=None, **kwargs):
        ...
        if dataset and new_annotation:
            ...
            new_annotation = sanitize_html(new_annotation, 'utf-8',
                                           'text/html')
            self.add_item_annotation(trans.sa_session,
                                     trans.get_user(), dataset, new_annotation)
            trans.sa_session.flush()
    ...
    def get_annotation_async(self, trans, id):
        ...
        if not dataset:
            web.httpexceptions.HTTPNotFound()
        annotation = self.get_item_annotation_str(trans.sa_session, trans.user,
                                                  dataset)
        if annotation and isinstance(annotation, text_type):
            annotation = annotation.encode('ascii',
                                           'replace')  # paste needs ascii here
        return annotation
    ...
    def display_at(self, trans, dataset_id, filename=None, **kwd):
        ...
        if trans.app.security_agent.dataset_is_public(data.dataset):
            return trans.response.send_redirect(
                redirect_url)  # anon access already permitted by rbac
        if self._can_access_dataset(trans, data):
    ...
    def display_application(self,
        ...
        app_kwds = {}
        for name, value in dict(kwds).iteritems(
        ):  # clone kwds because we remove stuff as we go.
            if name.startswith("app_"):
                app_kwds[name[len("app_"):]] = value
                del kwds[name]
        if kwds:
            log.debug("Unexpected Keywords passed to display_application: %s" %
                      kwds)  # route memory?
        # decode ids
        ...
        if self._can_access_dataset(trans, data, additional_roles=user_roles):
            ...
            display_app = trans.app.datatypes_registry.display_applications.get(
                app_name)
            if not display_app:
                log.debug("Unknown display application has been requested: %s",
                          app_name)
            ...
            if not display_link:
                log.debug("Unknown display link has been requested: %s",
                          link_name)
                return paste.httpexceptions.HTTPNotFound(
            ...
            if data.state == data.states.ERROR:
                ...
                if display_link.display_ready():
                    if app_action in ['data', 'param']:
                        ...
                        if value.parameter.type == 'data':
                            try:
                                if action_param_extra:
                                    assert value.parameter.allow_extra_files_access, "Extra file content requested (%s), but allow_extra_files_access is False." % (
                                        action_param_extra)
                        ...
                        msg.append(('Invalid action provided: %s' % app_action,
                                    'error'))
                else:
    ...
    def _delete(self, trans, dataset_id):
        ...
        try:
            ...
            msg = 'HDA deletion failed (encoded: %s, decoded: %s)' % (
                dataset_id, id)
    ...
    def _undelete(self, trans, dataset_id):
        ...
        try:
            ...
            msg = 'HDA undeletion failed (encoded: %s, decoded: %s)' % (
                dataset_id, id)
            log.exception(msg)
            trans.log_event(msg)
    ...
    def _purge(self, trans, dataset_id):
        ...
        try:
            ...
            msg = 'HDA purge failed (encoded: %s, decoded: %s): %s' % (
                dataset_id, id, exc)
            log.exception(msg)
            trans.log_event(msg)
    ...
    def unhide(self, trans, dataset_id, filename):
        if self._unhide(trans, dataset_id):
            return trans.response.send_redirect(
                web.url_for(
                    controller='root', action='history', show_hidden=True))
        raise Exception("Error unhiding")

    @web.expose
    def purge(self, trans, dataset_id, filename,
              show_deleted_on_refresh=False):
    ...
    def copy_datasets(self,
        ...
        if source_content_ids:
            ...
            encoded_dataset_collection_ids = [
                s[len("dataset_collection|"):] for s in source_content_ids
                if s.startswith("dataset_collection|")
            ]
            encoded_dataset_ids = [
                s[len("dataset|"):] for s in source_content_ids
        ...
        if do_copy:
            invalid_contents = 0
            if not (decoded_dataset_ids or decoded_dataset_collection_ids
                    ) or not (target_history_ids or new_history_name):
                ...
                if len(target_histories) != len(target_history_ids):
                    error_msg = error_msg + "You do not have permission to add datasets to %i requested histories.  " % (
                ...
                source_contents.extend(
                    map(
                        ...
                        ).get, decoded_dataset_collection_ids))
                ...
                done_msg = "%i %s copied to %i %s: %s." % (
                    num_source, inflector.cond_plural(num_source, "dataset"),
                    num_target, inflector.cond_plural(num_target, "history"),
                    hist_names_str)
