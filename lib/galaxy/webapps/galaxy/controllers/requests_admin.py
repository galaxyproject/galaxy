...
class AdminRequestsGrid(RequestsGrid):
    ...
    global_actions = [
        grids.GridAction("Create new request",
                         dict(
...
class RequestsAdmin(BaseUIController, UsesFormDefinitionsMixin):
    ...
    def reject_request(self, trans, **kwd):
        ...
        if not comment:
            ...
            return trans.fill_template(
                '/admin/requests/reject.mako',
                cntrller='requests_admin',
                request=request,
                status=status,
                message=message)
        # Create an event with state 'Rejected' for this request
        event_comment = "Sequencing request marked rejected by %s. Reason: %s " % (
            trans.user.email, comment)
        event = trans.model.RequestEvent(request, request.states.REJECTED,
                                         event_comment)
    ...
    def manage_datasets(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if operation == "view":
                ...
                message = '%i datasets have been deleted.' % (
    ...
    def rename_datasets(self, trans, **kwd):
        ...
        for sample_dataset_id in id_list:
            sample_dataset = trans.sa_session.query(
                trans.app.model.SampleDataset).get(
                    trans.security.decode_id(sample_dataset_id))
        ...
        if params.get('rename_datasets_button', False):
            ...
            if len(sample_datasets) == len(incorrect_dataset_names):
                ...
                status = 'done'
                message = 'Changes saved successfully. The following datasets were renamed incorrectly: %s.' % str(
                    incorrect_dataset_names)
            else:
    ...
    def __ensure_library_add_permission(self, trans, target_library,
                                        target_folder):
        """
        ...
        current_user_private_role = trans.app.security_agent.get_private_user_role(
            trans.user)
        flush_needed = False
        if not trans.app.security_agent.can_add_library_item(
                current_user_roles, target_library):
            lp = trans.model.LibraryPermissions(
                trans.app.security_agent.permitted_actions.LIBRARY_ADD.action,
                target_library, current_user_private_role)
            trans.sa_session.add(lp)
            flush_needed = True
        if not trans.app.security_agent.can_add_library_item(
                current_user_roles, target_folder):
            lfp = trans.model.LibraryFolderPermissions(
                trans.app.security_agent.permitted_actions.LIBRARY_ADD.action,
                target_folder, current_user_private_role)
    ...
    def initiate_data_transfer(self,
        ...
        self.__ensure_library_add_permission(trans, sample.library,
                                             sample.folder)
        ...
        if trans.app.config.enable_beta_job_managers:
            ...
            if sample_datasets:
                external_service_type_id = sample_datasets[
                    0].external_service.external_service_type_id
                ...
                scp_configs = external_service.data_transfer[
                    trans.model.ExternalService.data_transfer_protocol.SCP]
                if not scp_configs['automatic_transfer']:
                    deferred_plugin = 'ManualDataTransferPlugin'
                else:
                    raise Exception(
                        "Automatic data transfer using scp is not yet supported."
                    )
    ...
    @web.expose
    def update_sample_dataset_status(self,
                                     trans,
                                     cntrller,
                                     sample_dataset_ids,
                                     new_status,
        ...
        possible_status_list = [
            v[1]
            for v in trans.app.model.SampleDataset.transfer_status.items()
        ]
        ...
        for id in util.listify(sample_dataset_ids):
            try:
                ...
                sample_dataset = trans.sa_session.query(
                    trans.app.model.SampleDataset).get(sd_id)
            except:
                trans.response.status = 400
                return 400, "Invalid sample dataset id ( %s ) specified." % str(
                    id)
...
def build_rename_datasets_for_sample_select_field(trans,
                                                  sample_dataset,
                                                  selected_value='none'):
    options = []
    for option_index, option in enumerate(
            sample_dataset.file_path.split(os.sep)[:-1]):
        option = option.strip()
        if option:
    ...
    return build_select_field(
        ...
        objs=options,
        label_attr='self',
        select_field_name='rename_datasets_for_sample_%s' %
        trans.security.encode_id(sample_dataset.id),
        selected_value=selected_value,
        refresh_on_change=False)
