...
class TagsController(BaseUIController, UsesTagsMixin):
    ...
    def get_tagging_elt_async(self, trans, item_id, item_class,
                              elt_context=""):
        """
        Returns HTML for editing an item's tags.
        """
        item = self._get_item(trans, item_class,
                              trans.security.decode_id(item_id))
        if not item:
            return trans.show_error_message("No item of class %s with id %s " %
                                            (item_class, item_id))
    ...
    def add_tag_async(self,
        ...
        item = self._get_item(trans, item_class,
                              trans.security.decode_id(item_id))
        user = trans.user
        self.get_tag_handler(trans).apply_item_tags(user, item,
    ...
    def remove_tag_async(self,
        ...
        item = self._get_item(trans, item_class,
                              trans.security.decode_id(item_id))
        user = trans.user
        self.get_tag_handler(trans).remove_item_tag(user, item,
    ...
    def tag_autocomplete_data(self,
                              trans,
                              q=None,
                              limit=None,
                              timestamp=None,
        ...
        q = q.encode('utf-8')
        if q.find(":") == -1:
            return self._get_tag_autocomplete_names(trans, q, limit, timestamp,
                                                    user, item, item_class)
        else:
            return self._get_tag_autocomplete_values(
                trans, q, limit, timestamp, user, item, item_class)

    def _get_tag_autocomplete_names(self,
                                    trans,
                                    q,
                                    limit,
                                    timestamp,
        ...
        for row in result_set:
            ...
            if (item is
                    not None) and (self.get_tag_handler(trans).item_has_tag(
                        trans.user, item, tag)):
                continue
    ...
    def _get_tag_autocomplete_values(self,
                                     trans,
                                     q,
                                     limit,
                                     timestamp,
        ...
        tag_uname = self._get_usernames_for_tag(
            trans, trans.user, tag, item_class, item_tag_assoc_class)[0]
        for row in result_set:
            ac_data += tag_uname + ":" + row[0] + "|" + row[0] + "\n"
        return ac_data

    def _get_usernames_for_tag(self, trans, user, tag, item_class,
        ...
        # Build select stmt.
        cols_to_select = [
            item_tag_assoc_class.table.c.user_tname,
            func.count('*')
        ]
    ...
    def _get_item(self, trans, item_class_name, id):
        ...
        """
        item_class = self.get_tag_handler(trans).item_tag_assoc_info[
            item_class_name].item_class
        item = trans.sa_session.query(item_class).filter(
            item_class.id == id)[0]
        return item
