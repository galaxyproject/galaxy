...
class ExternalServiceGrid(grids.Grid):
    ...
    global_actions = [
        grids.GridAction("Reload external service types",
                         dict(
                             controller='external_service',
                             action='reload_external_service_types')),
        grids.GridAction("Create new external service",
                         dict(
                             controller='external_service',
...
class ExternalService(BaseUIController, UsesFormDefinitionsMixin):
    ...
    def create_external_service(self, trans, **kwd):
        ...
        widgets = self.__build_external_service_widgets(
            trans, external_service=None, **kwd)
        ...
        if not trans.app.external_service_types.visible_external_service_types:
            ...
            external_service_type = self.get_external_service_type(
                trans, external_service_type_id)
            widgets.extend(
                external_service_type.form_definition.get_widgets(
                    trans.user, **kwd))
        if error:
            return trans.response.send_redirect(
                web.url_for(
    ...
    def __save_external_service(self, trans, **kwd):
        ...
        description = util.restore_text(
            params.get('external_service_description', ''))
        ...
        if external_service_id:
            ...
            external_service.form_values.content = self.get_form_values(
                trans, trans.user, external_service.form_definition, **kwd)
            ...
            external_service_type = self.get_external_service_type(
                trans, external_service_type_id)
            external_service = trans.model.ExternalService(
                name, description, external_service_type_id, version)
            ...
            values = self.get_form_values(
                trans, trans.user, external_service.form_definition, **kwd)
            external_service.form_values = trans.model.FormValues(
                external_service.form_definition, values)
            trans.sa_session.add(external_service)
    ...
    def edit_external_service_form_definition(self, trans, **kwd):
        ...
        vars = dict(
            id=trans.security.encode_id(
                external_service.form_definition.form_definition_current_id),
    ...
    def delete_external_service(self, trans, **kwd):
        ...
        for external_service_id in external_service_id_list:
            ...
            trans.sa_session.add(external_service)
            trans.sa_session.flush()
        message = '%i external services has been deleted' % len(
            external_service_id_list)
        return trans.response.send_redirect(
            web.url_for(
                controller='external_service',
                action='browse_external_services',
    ...
    def undelete_external_service(self, trans, **kwd):
        ...
        for external_service_id in external_service_id_list:
            ...
            trans.sa_session.flush()
        status = 'done'
        message = '%i external services have been undeleted' % len(
            external_service_id_list)
        return trans.response.send_redirect(
            web.url_for(
                controller='external_service',
    ...
    def get_external_service_type(self,
                                  trans,
                                  external_service_type_id,
                                  action='browse_external_services'):
        try:
    ...
    def __build_external_service_widgets(self,
                                         trans,
        ...
        if external_service:
            ...
            description = util.restore_text(
                params.get('external_service_description', ''))
            version = util.restore_text(
                params.get('external_service_version', ''))
            ...
            if selected_seq_type in trans.app.external_service_types.all_external_service_types:
                seq_type = trans.app.external_service_types.all_external_service_types[
                    selected_seq_type].id
            else:
                seq_type = 'none'
        widgets = [
            dict(
            ...
            dict(
                label='Description',
                widget=TextField('external_service_description', 40,
                                 description),
                helptext=''),
            dict(
                label='Version',
        ...
        if not external_service:
            widgets.append(
                dict(
                    ...
                    widget=self.__build_external_service_type_select_field(
                        trans,
                        seq_type,
                        visible_external_service_types_only=True),
                    helptext=''))
        return widgets
    ...
    def __build_external_service_type_select_field(
        ...
        if visible_external_service_types_only:
            objs_list = [
                external_service_types[seq_type_id]
                for seq_type_id in
        ...
        for seq_type in objs_list:
            ...
            if selected_value == seq_type.id:
                select_external_service_type.add_option(
                    option_name, seq_type.id, selected=True)
            else:
                select_external_service_type.add_option(
