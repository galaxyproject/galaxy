...
class RequestTypeGrid(grids.Grid):
    ...
    global_actions = [
        grids.GridAction("Create new request type",
                         dict(
...
class RequestType(BaseUIController, UsesFormDefinitionsMixin):
            ...
            if operation == "view_form_definition":
                ...
                return trans.response.send_redirect(
                    web.url_for(
                        ...
                        form_type=trans.model.FormDefinition.types.
    ...
    def create_request_type(self, trans, **kwd):
        ...
        rt_info_widgets, rt_states_widgets = self.__get_populated_request_type_widgets(
            trans, **kwd)
        ...
        if params.get('add_state_button', False):
            ...
            self.__save_request_type(
                trans, action='create_request_type', **kwd)
    ...
    def edit_request_type(self, trans, **kwd):
        ...
        # get all the external services selected till now
        external_service_select_fields_list = []
        external_services_list = self.__get_external_services(
            trans, request_type, **kwd)
        if params.get('edit_request_type_button', False):
            request_type = self.__save_request_type(
                trans, action='edit_request_type', **kwd)
            ...
            index = int(
                kwd['remove_external_service_button'].split(' ')[3]) - 1
    ...
    def __save_request_type(self, trans, action, **kwd):
        ...
        try:
            request_form = trans.sa_session.query(
                trans.model.FormDefinition).get(
        ...
        if request_type_id:
            ...
            for sample_state in request_type.states:
                ...
                name = util.restore_text(
                    params.get('state_name_%s' % sample_state_id, ''))
                desc = util.restore_text(
                    params.get('state_desc_%s' % sample_state_id, ''))
            ...
            request_type = trans.model.RequestType(
                name=name,
                desc=desc,
            ...
            while True:
                if 'state_name_%i' % i in kwd:
                    name = util.restore_text(
                        params.get('state_name_%i' % i, ''))
                    desc = util.restore_text(
                        params.get('state_desc_%i' % i, ''))
                    sample_state = trans.model.SampleState(
                        name, desc, request_type)
                    trans.sa_session.add(sample_state)
                    trans.sa_session.flush()
                    i += 1
                else:
        ...
        for external_service in external_services_list:
            request_type.add_external_service_association(
                trans, external_service)
        return request_type
    ...
    def delete_request_type(self, trans, **kwd):
        ...
        for request_type_id in request_type_id_list:
            ...
            trans.sa_session.add(request_type)
            trans.sa_session.flush()
        message = '%i request types has been deleted' % len(
    ...
    def undelete_request_type(self, trans, **kwd):
        ...
        for request_type_id in request_type_id_list:
            ...
            trans.sa_session.flush()
        status = 'done'
        message = '%i request types have been undeleted' % len(
            request_type_id_list)
        return trans.response.send_redirect(
            web.url_for(
                controller='request_type',
    ...
    def request_type_permissions(self, trans, **kwd):
        ...
        if params.get('update_roles_button', False):
            ...
            for k, v in trans.model.RequestType.permitted_actions.items():
                in_roles = [
                    trans.sa_session.query(trans.model.Role).get(x)
        ...
        return trans.fill_template(
            '/admin/request_type/request_type_permissions.mako',
            request_type=request_type,
            roles=roles,
            status=status,
            message=message)
    ...
    def view_form_definition(self, trans, **kwd):
        ...
        try:
            ...
            return invalid_id_redirect(
                ...
                action='browse_request_types')
        return trans.fill_template(
            '/admin/forms/view_form_definition.mako',
            form_definition=form_definition)

    # ===== Methods for building SelectFields used on various admin_requests forms
    def __build_external_service_select_field(self,
                                              trans,
                                              select_field_name,
        ...
        if external_service:
            ...
            selected_value = 'none'
        all_external_services = trans.sa_session.query(
            trans.model.ExternalService).filter(
