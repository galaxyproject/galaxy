...
class RequestsGrid(grids.Grid):
    ...
    columns = [
        NameColumn(
            "Name", key="name", link=(lambda item: dict(operation="view_request", id=item.id)), attach_popup=True, filterable="advanced"
        ),
        DescriptionColumn("Description", key='desc', filterable="advanced"),
        SamplesColumn("Samples", link=(lambda item: iff(item.deleted, None, dict(operation="edit_samples", id=item.id)))),
        TypeColumn("Type", link=(lambda item: iff(item.deleted, None, dict(operation="view_type", id=item.type.id)))),
    ...
    operations = [
        grids.GridOperation(
            ...
            condition=
            (lambda item: not item.deleted and item.is_unsubmitted and item.samples
             ),
            confirm=
            "Samples cannot be added to this request after it is submitted. Click OK to submit."
        )
    ]
...
class RequestsCommon(BaseUIController, UsesFormDefinitionsMixin):
                ...
                if id and state:
                    ...
                    if sample.state.name != state:
                        rval[id] = {
                            ...
                            unicodify(
                                trans.fill_template(
                                    "requests/common/sample_state.mako",
                                    sample=sample), 'utf-8')
                        }
        return rval
                ...
                if id and number_of_datasets:
                    ...
                    if len(sample.datasets) != number_of_datasets:
                        rval[id] = {
                            ...
                            unicodify(
                                trans.fill_template(
                                    "requests/common/sample_datasets.mako",
                                    sample=sample), 'utf-8')
                        }
        return rval

    @web.json
    def dataset_transfer_status_updates(self,
                                        trans,
                                        ids=None,
                                        transfer_status_list=None):
        # Avoid caching
        trans.response.headers['Pragma'] = 'no-cache'
        trans.response.headers['Expires'] = '0'
        # Create new HTML for any that have changed
                ...
                if id and transfer_status:
                    ...
                    if sample_dataset.status != transfer_status:
                        rval[id] = {
                            ...
                            unicodify(
                                trans.fill_template(
                                    "requests/common/sample_dataset_transfer_status.mako",
    ...
    def create_request(self, trans, cntrller, **kwd):
        ...
        if user_id != 'none':
            ...
            user = None
        if params.get('create_request_button', False) or params.get(
                'add_sample_button', False):
            ...
            elif cntrller == 'requests' and not trans.app.security_agent.can_access_request_type(
                    user.all_roles(), request_type):
                message = '%s does not have access permission to the "%s" request type.' % (
                    user.email, request_type.name)
                status = 'error'
            elif not name:
        ...
        request_type_select_field = self.__build_request_type_id_select_field(
            trans, selected_value=request_type_id)
        ...
        if request_type is not None or status == 'error':
            ...
            widgets.append(
                dict(
                    ...
                    widget=TextField(
                        'name', 40, util.restore_text(params.get('name', ''))),
                    helptext='(Required)'))
            widgets.append(
                dict(
                    label='Description',
                    widget=TextField(
                        'desc', 40, util.restore_text(params.get('desc', ''))),
                    helptext='(Optional)'))
        ...
        if request_type is not None or status == 'error':
            ...
            if is_admin:
                ...
                user_widget = dict(
                    label='Select user',
                    widget=self.__build_user_id_select_field(
                        trans, selected_value=selected_user_id),
                    helptext=
                    'Submit the request on behalf of the selected user (Required)'
                )
                widgets = [user_widget] + widgets
        return trans.fill_template(
            '/requests/common/create_request.mako',
            cntrller=cntrller,
    ...
    def view_request(self, trans, cntrller, **kwd):
        ...
        try:
            request = trans.sa_session.query(trans.model.Request).get(
                trans.security.decode_id(request_id))
        except:
        ...
        displayable_sample_widgets = self.__get_sample_widgets(
            trans, request, request.samples, **kwd)
        request_widgets = self.__get_request_widgets(trans, request.id)
        return trans.fill_template(
            '/requests/common/view_request.mako',
            cntrller=cntrller,
            request=request,
    ...
    def edit_basic_request_info(self, trans, cntrller, **kwd):
        ...
        if params.get('edit_basic_request_info_button', False):
            if not name:
                ...
                request = self.__save_request(
                    trans, cntrller, request=request, **kwd)
        ...
        widgets.append(
            dict(
                ...
                helptext='(Optional)'))
        widgets = widgets + request.type.request_form.get_widgets(
            request.user, request.values.content, **kwd)
        # In case there is an error on the form, make sure to populate widget fields with anything the user
        # may have already entered.
        widgets = self.populate_widgets_from_kwd(trans, widgets, **kwd)
        return trans.fill_template(
            'requests/common/edit_basic_request_info.mako',
            cntrller=cntrller,
    ...
    def __save_request(self, trans, cntrller, request=None, **kwd):
        ...
        notification = dict(
            email=[user.email],
            sample_states=[request_type.final_sample_state.id],
            body='',
            subject='')
        values = self.get_form_values(trans, user, request_type.request_form,
                                      **kwd)
        if request is None:
            form_values = trans.model.FormValues(request_type.request_form,
                                                 values)
            trans.sa_session.add(form_values)
            # We're creating a new request
            request = trans.model.Request(name, desc, request_type, user,
                                          form_values, notification)
            ...
            event = trans.model.RequestEvent(request, request.states.NEW,
                                             comment)
    ...
    def submit_request(self, trans, cntrller, **kwd):
        ...
        if request.user != trans.user:
            comment += " on behalf of %s." % request.user.email
        event = trans.model.RequestEvent(request, request.states.SUBMITTED,
                                         comment)
        trans.sa_session.add(event)
        ...
        for sample in request.samples:
            event_comment = 'Sequencing request submitted and sample state set to %s.' % request.type.states[
                0].name
            event = trans.model.SampleEvent(
                sample, initial_sample_state_after_request_submitted,
                event_comment)
            trans.sa_session.add(event)
        trans.sa_session.add(request)
        trans.sa_session.flush()
        request.send_email_notification(
            trans, initial_sample_state_after_request_submitted)
    ...
    def edit_samples(self, trans, cntrller, **kwd):
        ...
        if params.get('cancel_changes_button', False):
            return trans.response.send_redirect(
                web.url_for(
                    ...
                    cntrller=cntrller,
                    id=request_id))
        libraries = trans.app.security_agent.get_accessible_libraries(
            trans, request.user)
        # Build a list of sample widgets (based on the attributes of each sample) for display.
        displayable_sample_widgets = self.__get_sample_widgets(
            trans, request, request.samples, **kwd)
        encoded_selected_sample_ids = self.__get_encoded_selected_sample_ids(
            trans, request, **kwd)
        ...
        if params.get('save_samples_button', False):
            if encoded_selected_sample_ids:
                ...
                for encoded_sample_id in encoded_selected_sample_ids:
                    sample = trans.sa_session.query(trans.model.Sample).get(
                        trans.security.decode_id(encoded_sample_id))
                ...
                sample_widgets = self.__get_sample_widgets(
                    trans, request, sample_widgets, **kwd)
            ...
            return self.__save_samples(
                trans,
                cntrller,
                request,
                sample_widgets,
                saving_new_samples=False,
                **kwd)
        request_widgets = self.__get_request_widgets(trans, request.id)
        sample_copy_select_field = self.__build_copy_sample_select_field(
            trans, displayable_sample_widgets)
        libraries_select_field, folders_select_field = self.__build_library_and_folder_select_fields(
            trans, request.user, 'sample_operation', libraries, None, **kwd)
        sample_operation_select_field = self.__build_sample_operation_select_field(
            trans, is_admin, request, sample_operation)
        sample_state_id = params.get('sample_state_id', None)
        sample_state_id_select_field = self.__build_sample_state_id_select_field(
            trans, request, sample_state_id)
        return trans.fill_template(
            '/requests/common/edit_samples.mako',
            cntrller=cntrller,
            request=request,
        ...
        for sample_id in sample_ids:
            try:
                ...
                if cntrller == 'api':
                    ...
                    return "Invalid sample id ( %s ) specified, unable to decode." % str(
                        sample_id)
                else:
        ...
        for id in id_list:
            ...
            if ok_for_now:
                ...
                if is_admin or not request.is_submitted:
                    ...
                    event = trans.model.RequestEvent(request, request.state,
                                                     comment)
                    trans.sa_session.add(event)
                    trans.sa_session.flush()
                    num_deleted += 1
    ...
    def view_request_history(self, trans, cntrller, **kwd):
        ...
        try:
            ...
            return invalid_id_redirect(trans, cntrller, request_id)
        return trans.fill_template(
            '/requests/common/view_request_history.mako',
            cntrller=cntrller,
            request=request)
    ...
    def edit_email_settings(self, trans, cntrller, **kwd):
        ...
        try:
            ...
            return invalid_id_redirect(trans, cntrller, request_id)
        email_address = CheckboxField.is_checked(
        ...
        for index, sample_state in enumerate(request.type.states):
            if CheckboxField.is_checked(
                    params.get('sample_state_%i' % sample_state.id, '')):
        ...
        if email_address or additional_email_addresses:
            ...
            for email_address in email_addresses:
                err_msg += validate_email(
                    trans, email_address, check_dup=False)
            if err_msg:
                status = 'error'
                message += err_msg
            else:
                request.notification = dict(
    ...
    def update_request_state(self, trans, cntrller, **kwd):
        ...
        if not common_state:
            ...
            if request.is_complete:
                ...
                event = trans.model.RequestEvent(
            ...
            retval = request.send_email_notification(trans, common_state,
                                                     final_state)
            if retval:
    ...
    def find_samples(self, trans, cntrller, **kwd):
                    ...
                    for sample in all_samples:
                        ...
                        for field in sample.request.type.sample_form.fields:
                            if field_label == field['label']:
                                ...
                                if sample.values.content[field[
                                        'name']] == field_value:
                                    samples.append(sample)
            if is_admin:
                for s in samples:
                ...
                for s in samples:
                    if s.request.user.id == trans.user.id and s.request.state in request_states and not s.request.deleted:
                        samples_list.append(s)
            results = 'There are %i samples matching the search parameters.' % len(
                samples_list)
        # Build the request_states SelectField
        selected_value = kwd.get('request_states',
                                 trans.model.Request.states.SUBMITTED)
        states = [v for k, v in trans.model.Request.states.items()]
        request_states = build_select_field(
            trans,
            states,
            'self',
            'request_states',
        ...
        search_type = build_select_field(
            trans,
            types,
            'self',
            'search_type',
        ...
        return trans.fill_template(
            '/requests/common/find_samples.mako',
            cntrller=cntrller,
    ...
    def view_sample_history(self, trans, cntrller, **kwd):
        ...
        return trans.fill_template(
            '/requests/common/view_sample_history.mako',
            cntrller=cntrller,
            sample=sample)
    ...
    def add_samples(self, trans, cntrller, **kwd):
        ...
        if params.get('import_samples_button', False):
            ...
            workflows = [
                w.latest_workflow for w in trans.user.stored_workflows
                if not w.deleted
            ]
            return self.__import_samples(trans, cntrller, request,
                                         displayable_sample_widgets, libraries,
                                         workflows, **kwd)
        elif params.get('add_sample_button', False):
            return self.add_sample(trans, cntrller, request_id, **kwd)
            ...
            return self.__save_samples(
                ...
                cntrller,
                request,
                displayable_sample_widgets,
                saving_new_samples=True,
                **kwd)
        request_widgets = self.__get_request_widgets(trans, request.id)
        sample_copy_select_field = self.__build_copy_sample_select_field(
            trans, displayable_sample_widgets)
        libraries_select_field, folders_select_field = self.__build_library_and_folder_select_fields(
            trans, request.user, 'sample_operation', libraries, None, **kwd)
        return trans.fill_template(
            '/requests/common/add_samples.mako',
            cntrller=cntrller,
            request=request,
            request_widgets=request_widgets,
            displayable_sample_widgets=displayable_sample_widgets,
        ...
        if params.get('add_sample_button', False):
            ...
            for index in range(num_samples_to_add):
                ...
                if copy_sample_index != -1:
                    ...
                    library_id = displayable_sample_widgets[copy_sample_index][
                        'library_select_field'].get_selected(
                            return_value=True)
                    folder_id = displayable_sample_widgets[copy_sample_index][
                        'folder_select_field'].get_selected(return_value=True)
                    name = displayable_sample_widgets[copy_sample_index]['name'] + '_%i' % (
                        len(displayable_sample_widgets) + 1)
                    history_id = displayable_sample_widgets[copy_sample_index][
                        'history_select_field'].get_selected(
                            return_value=True)
                    workflow_id = displayable_sample_widgets[
                        copy_sample_index]['workflow_select_field'][
                            0].get_selected(return_value=True)
                    # DBTODO Do something nicer with the workflow fieldset.  Remove [0] indexing and copy mappings as well.
                    for field_name in displayable_sample_widgets[
                            copy_sample_index]['field_values']:
                ...
                library_select_field, folder_select_field = self.__build_library_and_folder_select_fields(
                    trans,
                    user=request.user,
                    ...
                    folder_id=folder_id,
                    **kwd)
                history_select_field = self.__build_history_select_field(
                    trans=trans,
                    user=request.user,
                    sample_index=len(displayable_sample_widgets),
                    history_id=history_id,
                    **kwd)
                workflow_select_field = self.__build_workflow_select_field(
                    trans=trans,
                    user=request.user,
                    request=request,
                    sample_index=len(displayable_sample_widgets),
                ...
                displayable_sample_widgets.append(
                    dict(
                        id=None,
                        name=name,
                        bar_code='',
    ...
    def view_sample(self, trans, cntrller, **kwd):
        ...
        return trans.fill_template(
            '/requests/common/view_sample.mako',
            cntrller=cntrller,
            sample=sample,
            widgets=widgets,
    ...
    def view_sample_datasets(self, trans, cntrller, **kwd):
        ...
        return trans.fill_template(
            '/requests/common/view_sample_datasets.mako',
            cntrller=cntrller,
            title=title,
    ...
    def __import_samples(self, trans, cntrller, request,
                         displayable_sample_widgets, libraries, workflows,
                         **kwd):
        """
        ...
        try:
            ...
            for row in reader:
                ...
                library_select_field, folder_select_field = self.__build_library_and_folder_select_fields(
                    trans, request.user,
                    len(displayable_sample_widgets), libraries, None,
                    library_id, folder_id, **kwd)
                # Get the history
            ...
            if str(e) == "'unicode' object has no attribute 'file'":
                ...
                message = 'Error attempting to create samples from selected file: %s.' % str(
                    e)
        ...
        if not saving_new_samples:
            ...
            if sample_operation == trans.model.Sample.bulk_operations.CHANGE_STATE:
                ...
                if sample_state_id in [None, 'none']:
                    ...
                    handle_error(**kwd)
                sample_event_comment = util.restore_text(
                    params.get('sample_event_comment', ''))
                new_state = trans.sa_session.query(
                    trans.model.SampleState).get(
                        trans.security.decode_id(sample_state_id))
                ...
                for sample_index in range(len(sample_widgets)):
                    ...
                    if bar_code:
                        ...
                        bc_message = self.__validate_bar_code(
                            trans, request_sample, bar_code)
                        if bc_message:
                            message += bc_message
                            kwd['message'] = message
                            del kwd['save_samples_button']
                            handle_error(**kwd)
                self.update_sample_state(
                    trans,
                    cntrller,
                    encoded_selected_sample_ids,
                    new_state,
                    comment=sample_event_comment)
                return trans.response.send_redirect(
                    web.url_for(
                        controller='requests_common',
                        cntrller=cntrller,
                ...
                library, folder = self.__get_library_and_folder(
                    trans, library_id, folder_id)
                for sample_index in range(len(sample_widgets)):
                    current_sample = sample_widgets[sample_index]
                    if current_sample is None:
                        # We have a None value because the user did not select this sample
            ...
            self.__update_samples(trans, cntrller, request, sample_widgets,
                                  **kwd)
            ...
            for index in range(len(sample_widgets) - len(request.samples)):
                ...
                form_values = trans.model.FormValues(
                    request.type.sample_form, sample_widget['field_values'])
                trans.sa_session.add(form_values)
                trans.sa_session.flush()
            ...
            if request.is_submitted:
                initial_sample_state_after_request_submitted = request.type.states[
                    0]
                for sample in new_samples:
                    event_comment = 'Sample added and sample state set to %s.' % request.type.states[
                        0].name
                    event = trans.model.SampleEvent(
                        sample, initial_sample_state_after_request_submitted,
                        event_comment)
    ...
    def __update_samples(self, trans, cntrller, request, sample_widgets,
                         **kwd):
        ...
        for index, sample_widget in enumerate(sample_widgets):
            if sample_widget is not None:
                ...
                form_values = trans.sa_session.query(
                    trans.model.FormValues).get(sample.values.id)
                    ...
                    if bar_code:
                        ...
                        if not sample.bar_code:
                            ...
                            if sample.state.id == request.type.states[0].id:
                                ...
                                common_state = request.samples_have_common_state
                                if request.is_submitted and common_state and len(
                                        request.type.states) >= 2:
                                    comment = "All samples of this request are in the (%s) sample state. " % common_state.name
                                    event = trans.model.RequestEvent(
                                        request, request.states.SUBMITTED,
                                        comment)
                                    trans.sa_session.add(event)
                                    trans.sa_session.flush()
                                    request.send_email_notification(
                                        trans, next_sample_state)
    ...
    def __get_history(self, trans, history_id):
        try:
            history = trans.sa_session.query(trans.model.History).get(
    ...
    def __get_workflow(self, trans, workflow_id):
        try:
            workflow = trans.sa_session.query(trans.model.Workflow).get(
    ...
    def __get_request_widgets(self, trans, id):
        ...
        for index, field in enumerate(request.type.request_form.fields):
            ...
            if field['type'] == 'AddressField':
                if field_value:
                    request_widgets.append(
                        dict(
                            label=field['label'],
                            value=trans.sa_session.query(
                                trans.model.UserAddress).get(int(
                                    field_value)).get_html(),
    ...
    def __get_sample_widgets(self, trans, request, samples, **kwd):
        ...
        if sample_operation != 'none':
            ...
            library_id = util.restore_text(
                params.get('sample_operation_library_id', 'none'))
            folder_id = util.restore_text(
                params.get('sample_operation_folder_id', 'none'))
        ...
        for index, sample in enumerate(samples):
            if sample is None:
                ...
                if not workflow_id and sample.workflow:
                    workflow_id = trans.security.encode_id(
                        sample.workflow['id'])
                    workflow_dict = sample.workflow
                ...
                for field_index, field in enumerate(
                    ...
                    input_value = params.get('sample_%i_field_%i' %
                                             (index, field_index),
                                             sample.values.content[field_name])
        ...
        while True:
            ...
            if not name:
                break
            bar_code = util.restore_text(
                params.get('sample_%i_bar_code' % index, ''))
            library_id = util.restore_text(
                params.get('sample_%i_library_id' % index, ''))
            folder_id = util.restore_text(
                params.get('sample_%i_folder_id' % index, ''))
                ...
                if workflow:
                    ...
                    for k, v in kwd.iteritems():
                        ...
                        if k.startswith(kwd_tag):
                            ...
                            workflow_dict['mappings'][int(k[len(
                                kwd_tag):])] = {
                                    'ds_tag': v
                                }
            field_values = {}
    ...
    def __build_copy_sample_select_field(self, trans,
                                         displayable_sample_widgets):
        ...
        for index, sample_dict in enumerate(displayable_sample_widgets):
            copy_sample_index_select_field.add_option(sample_dict['name'],
                                                      index)
        return copy_sample_index_select_field

    def __build_request_type_id_select_field(self,
                                             trans,
                                             selected_value='none'):
        accessible_request_types = trans.app.security_agent.get_accessible_request_types(
            trans, trans.user)
    ...
    def __build_sample_operation_select_field(self, trans, is_admin, request,
                                              selected_value):
        ...
        if is_admin:
            if request.is_complete:
                bulk_operations = [
                    trans.model.Sample.bulk_operations.CHANGE_STATE
                ]
            if request.is_rejected:
                bulk_operations = [
                    trans.model.Sample.bulk_operations.SELECT_LIBRARY
                ]
            else:
                bulk_operations = [
                    s for i, s in trans.model.Sample.bulk_operations.items()
                ]
        else:
            if request.is_complete:
                bulk_operations = []
    ...
    def __build_library_and_folder_select_fields(self,
                                                 trans,
                                                 user,
                                                 sample_index,
                                                 libraries,
        ...
        library_select_field = build_select_field(
            trans,
            libraries,
            'name',
        ...
        if selected_library:
            folders = self.__get_active_folders(
                selected_library.root_folder,
                active_folders_list=[selected_library.root_folder])
            if folder_id:
                selected_folder_id = folder_id
            elif sample and sample.folder:
                selected_folder_id = trans.security.encode_id(sample.folder.id)
            else:
                selected_folder_id = trans.security.encode_id(
                    selected_library.root_folder.id)
        else:
        ...
        for folder in folders:
            if not folder.parent:
                ...
                break
        folder_select_field = build_select_field(
            trans,
            folders,
            'name',
    ...
    def __build_history_select_field(self,
                                     trans,
                                     user,
                                     sample_index,
                                     sample=None,
        ...
        hsf = build_select_field(
            trans, [h for h in user.histories if not h.deleted],
            'name',
        ...
        hsf.options = [("Select one", 'none',
                        'none' == str(history_id).lower())] + hsf.options[1:]
        return hsf

    def __build_workflow_select_field(self,
                                      trans,
                                      user,
                                      request,
                                      sample_index,
        ...
        if workflow_id not in [None, 'none']:
            selected_workflow = trans.sa_session.query(
                trans.model.Workflow).get(
                    trans.security.decode_id(workflow_id))
            ...
            selected_workflow = trans.sa_session.query(
                trans.model.Workflow).get(sample.workflow['id'])
        s_list = [
            w.latest_workflow for w in user.stored_workflows if not w.deleted
        ]
        if selected_workflow and selected_workflow not in s_list:
            s_list.append(selected_workflow)
        workflow_select_field = build_select_field(
            trans,
            s_list,
            'name',
            ...
            refresh_on_change=True)
        workflow_select_field.options = [
            ("Select one", 'none', 'none' == str(workflow_id).lower())
        ] + workflow_select_field.options[1:]
        ...
        if selected_workflow and request.type.external_services:
            ...
            if history_id not in [None, 'none', 'new', '']:
                hist = trans.sa_session.query(trans.model.History).get(
                    trans.security.decode_id(history_id))
                h_inputs = [("hi|%s" % trans.security.encode_id(ds.id),
                             ds.name) for ds in hist.datasets
                            if not ds.deleted]
                ds_list += h_inputs
            for step in selected_workflow.steps:
                if step.type == 'data_input':
                    if step.tool_inputs and "name" in step.tool_inputs:
                        sf_name = '%s_%s' % (workflow_select_field_name,
                                             step.id)
                        ...
                        if not sf and sample and sample.workflow:
                            if str(step.id) in sample.workflow['mappings']:
                                sf = sample.workflow['mappings'][str(step.id)][
                                    'ds_tag']
                        ...
                        wf_fieldset.append((step.tool_inputs['name'],
                                            select_field))
        return wf_fieldset

    def __build_sample_state_id_select_field(self, trans, request,
                                             selected_value):
        if selected_value == 'none':
            if request.samples:
                selected_value = trans.security.encode_id(
                    request.samples[0].state.id)
            else:
                selected_value = trans.security.encode_id(
                    request.type.states[0].id)
        return build_select_field(
            trans,
    ...
    def __validate_request(self, trans, cntrller, request):
        ...
        for index, field in enumerate(request.type.request_form.fields):
            if field['required'] == 'required' and request.values.content[field['name']] in [
                    '', None
            ]:
        ...
        for s in request.samples:
            for field in request.type.sample_form.fields:
                ...
                if field['required'] == 'required' and s.values.content[field['name']] in [
                        '', None
                ]:
        ...
        if empty_fields or empty_sample_fields:
            ...
            if empty_sample_fields:
                for sname, ef in empty_sample_fields:
                    message = message + '<b>%s</b> field of sample <b>%s</b><br/>' % (
                        ef, sname)
            return message
        return None

    def __validate_sample_names(self, trans, cntrller, request,
                                displayable_sample_widgets, **kwd):
        # Check for duplicate sample names for all samples of the request.
        message = ''
        for index in range(
                len(displayable_sample_widgets) - len(request.samples)):
            ...
            for i in range(len(displayable_sample_widgets)):
                if sample_name == displayable_sample_widgets[i]['name']:
                    count += 1
            if count > 1:
                message = "You tried to add %i samples with the name (%s).  Samples belonging to a request must have unique names." % (
                    count, sample_name)
                break
        if message:
            del kwd['save_samples_button']
    ...
    def __get_encoded_selected_sample_ids(self, trans, request, **kwd):
        ...
        for sample in request.samples:
            if CheckboxField.is_checked(
                    kwd.get('select_sample_%i' % sample.id, '')):
                encoded_selected_sample_ids.append(
                    trans.security.encode_id(sample.id))
        return encoded_selected_sample_ids


# ===== Miscellaneous utility methods outside of the RequestsCommon class =====
def invalid_id_redirect(trans,
                        cntrller,
                        obj_id,
                        item='sequencing request',
                        action='browse_requests'):
    status = 'error'
    message = "Invalid %s id (%s)" % (item, str(obj_id))
    return trans.response.send_redirect(
        web.url_for(
            controller=cntrller, action=action, status=status,
            message=message))
