...
class AdminToolshed(AdminGalaxy):
    ...
    def activate_repository(self, trans, **kwd):
        ...
        try:
            trans.app.installed_repository_manager.activate_repository(
                repository)
        except Exception as e:
            error_message = "Error activating repository %s: %s" % (
                escape(repository.name), str(e))
        ...
        if 'operation' in kwd:
            ...
            if operation == "update tool shed status":
                return trans.response.send_redirect(
                    web.url_for(
                        controller='admin_toolshed',
                        action=
                        'update_tool_shed_status_for_installed_repository',
                        **kwd))
            ...
            if operation == "activate or reinstall":
                ...
                if repository.uninstalled:
                    ...
                    current_ctx_rev = changeset_revision_dict.get(
                        'ctx_rev', None)
                    if current_changeset_revision and current_ctx_rev:
                        if current_ctx_rev == repository.ctx_rev:
                            ...
                            json_repo_info_dict = json.dumps(
                                updated_repo_info_dict)
                        ...
                        message = "Unable to get latest revision for repository <b>%s</b> from " % escape(
                            str(repository.name))
    ...
    def browse_toolsheds(self, trans, **kwd):
        app = {'jscript': "admin.toolshed"}
        return trans.fill_template(
            'galaxy.panels.mako',
            config={'title': 'Galaxy Tool Sheds',
                    'app': app})
        ...
        for tool_shed_repository in tool_shed_repositories:
            if kwd.get('deactivate_or_uninstall_repository_button', False):
                ...
                message += 'The repository named <b>%s</b> has been %s.  ' % (
                    escape(tool_shed_repository.name), action)
        ...
        if kwd.get('deactivate_or_uninstall_repository_button', False):
            return trans.response.send_redirect(
                web.url_for(
                    ...
                    message=message,
                    status=status))
        remove_from_disk_check_box = CheckboxField(
            'remove_from_disk', checked=remove_from_disk_checked)
        return trans.fill_template(
    ...
    def find_tools_in_tool_shed(self, trans, **kwd):
        ...
        url = util.build_url(
            tool_shed_url,
            pathspec=['repository', 'find_tools'],
            params=params)
        return trans.response.send_redirect(url)
    ...
    def find_workflows_in_tool_shed(self, trans, **kwd):
        ...
        url = util.build_url(
            tool_shed_url,
            pathspec=['repository', 'find_workflows'],
            params=params)
        return trans.response.send_redirect(url)

    @web.expose
    @web.require_admin
    def generate_workflow_image(self, trans, workflow_name,
                                repository_id=None):
    ...
    @web.expose
    @web.require_admin
    def get_tool_dependencies(self, trans, repository_id, repository_name,
    ...
    @web.expose
    @web.require_admin
    def get_updated_repository_information(self, trans, repository_id,
    ...
    def import_workflow(self, trans, workflow_name, repository_id, **kwd):
        ...
        if workflow_name:
            ...
            if repository:
                ...
                if workflow:
                    workflow_name = encoding_util.tool_shed_encode(
                        str(workflow.name))
                else:
        ...
        if repository_id is not None:
            ...
            if repository is not None:
                ...
                params = dict(
                    ...
                    owner=owner)
                pathspec = [
                    'repository', 'get_latest_downloadable_changeset_revision'
                ]
                raw_text = util.url_get(
                    tool_shed_url,
                ...
                if latest_downloadable_revision == hg_util.INITIAL_CHANGELOG_HASH:
                    ...
                    tmp_repo_info_tuple = (None, tmp_clone_url,
                    ...
                    if installed_repository:
                        ...
                        params = dict(
                            name=name,
                            owner=owner,
                            changeset_revisions=str(
                                latest_downloadable_revision),
                            galaxy_url=web.url_for('/', qualified=True))
                        pathspec = [
                            'repository', 'install_repositories_by_revision'
                        ]
                        url = util.build_url(
                            tool_shed_url, pathspec=pathspec, params=params)
                        return trans.response.send_redirect(url)
            else:
                message = 'Cannot locate installed tool shed repository with encoded id <b>%s</b>.' % str(
                    repository_id)
                status = 'error'
        else:
    ...
    def install_tool_dependencies_with_update(self, trans, **kwd):
        ...
        encoded_tool_dependencies_dict = kwd.get(
            'encoded_tool_dependencies_dict', None)
        ...
        encoded_relative_install_dir = kwd.get('encoded_relative_install_dir',
                                               None)
        ...
        updating_to_changeset_revision = kwd.get(
            'updating_to_changeset_revision', None)
        ...
        if view.installable_resolvers:
            install_resolver_dependencies_check_box = CheckboxField(
                'install_resolver_dependencies', checked=True)
        else:
        ...
        return trans.fill_template(
            ...
            install_resolver_dependencies_check_box=
            install_resolver_dependencies_check_box,
            ...
            if operation == 'browse':
                ...
                for repository_id in tsridslist:
                    ...
                    if repository.status in [
                            ...
                            installation_status.INSTALLED, trans.install_model.
                ...
                reinstalling = util.string_as_bool(
                    kwd.get('reinstalling', False))
    ...
    def manage_repository(self, trans, **kwd):
        ...
        if repository_id is None:
            return trans.show_error_message(
                'Missing required encoded repository id.')
            ...
            if operation == 'browse':
                ...
                for tool_dependency_id in tool_dependency_ids:
                    ...
                    if tool_dependency.status in [
                        ...
                        tool_dependencies_for_uninstallation.append(
                            tool_dependency)
                ...
                if trans.app.config.tool_dependency_dir:
                    ...
                    for tool_dependency_id in tool_dependency_ids:
                        ...
                        if tool_dependency.status in [
                                trans.install_model.ToolDependency.
                                installation_status.NEVER_INSTALLED,
                                trans.install_model.ToolDependency.
                                installation_status.UNINSTALLED
                        ]:
                            tool_dependencies_for_installation.append(
                                tool_dependency)
    ...
    def manage_tool_dependencies(self, trans, **kwd):
        ...
        if tool_dependency_ids:
            ...
            tool_shed_repository = repository_util.get_tool_shed_repository_by_id(
                trans.app, repository_id)
        self.tool_dependency_grid.title = "Tool shed repository '%s' tool dependencies" % escape(
    ...
    def monitor_repository_installation(self, trans, **kwd):
        tsridslist = common_util.get_tool_shed_repository_ids(**kwd)
        if not tsridslist:
            tsridslist = repository_util.get_ids_of_tool_shed_repositories_being_installed(
    ...
    def prepare_for_install(self, trans, **kwd):
        ...
        install_repository_dependencies = kwd.get(
        ...
        install_resolver_dependencies = kwd.get(
        ...
        if not encoded_repo_info_dicts:
            ...
            if updating:
                ...
                try:
                    ...
                    message = 'The updates available for the repository <b>%s</b> ' % escape(
                        str(repository.name))
            ...
            for encoded_repo_info_dict in repo_information_dict.get(
                ...
                if not has_repository_dependencies:
                    has_repository_dependencies = util.string_as_bool(
                        repo_information_dict.get(
                            'has_repository_dependencies', False))
                if not includes_tool_dependencies:
                    includes_tool_dependencies = util.string_as_bool(
                        repo_information_dict.get('includes_tool_dependencies',
                                                  False))
        ...
        dd = dependency_display.DependencyDisplayer(trans.app)
        install_repository_manager = install_manager.InstallRepositoryManager(
            trans.app)
            ...
            if repository:
                if repository.is_new:
                    if kwd.get('purge_repository_button', False):
                        ...
                        purge_status, purge_message = irm.purge_repository(
                            repository)
                        if purge_status == 'ok':
                            new_kwd['status'] = "done"
                        else:
                    ...
                    new_kwd['status'] = 'error'
                    new_kwd[
                        'message'] = 'Repositories must have a <b>New</b> status in order to be purged.'
            else:
                new_kwd['status'] = 'error'
                new_kwd[
                    'message'] = 'Cannot locate the database record for the repository with encoded id %s.' % str(
                        repository_id)
        else:
            new_kwd['status'] = 'error'
            new_kwd[
                'message'] = 'Invalid repository id value "None" received for repository to be purged.'
        return trans.response.send_redirect(
            web.url_for(
                controller='admin_toolshed',
                action='browse_repositories',
    ...
    def reinstall_repository(self, trans, **kwd):
        ...
        if tool_shed_repository.status == trans.install_model.ToolShedRepository.installation_status.UNINSTALLED:
            repository_type = suc.get_repository_type_from_tool_shed(
                trans.app, tool_shed_url, tool_shed_repository.name,
            ...
            tool_shed_repository = repository_util.create_or_update_tool_shed_repository(
                trans.app, tool_shed_repository.name,
                tool_shed_repository.description,
                tool_shed_repository.installed_changeset_revision,
                tool_shed_repository.ctx_rev, repository_clone_url, metadata,
                trans.install_model.ToolShedRepository.installation_status.NEW,
                tool_shed_repository.changeset_revision,
                tool_shed_repository.owner, tool_shed_repository.dist_to_shed)
        ...
        new_kwd = dict(
            ...
            includes_tools_for_display_in_tool_panel=tool_shed_repository.
            includes_tools_for_display_in_tool_panel,
            install_tool_dependencies=install_tool_dependencies,
            install_resolver_dependencies=install_resolver_dependencies,
        ...
        for tsr_id in tsr_ids:
            tsr = trans.install_model.context.query(
                trans.install_model.ToolShedRepository).get(tsr_id)
    ...
    def repair_repository(self, trans, **kwd):
        ...
        if not repository_id:
            message = 'Invalid installed tool shed repository id %s received.' % str(
                repository_id)
            status = 'error'
            return trans.response.send_redirect(
                web.url_for(
        ...
        if kwd.get('repair_repository_button', False):
            ...
            if encoded_repair_dict:
                repair_dict = encoding_util.tool_shed_decode(
                    encoded_repair_dict)
            else:
                repair_dict = None
            if not repair_dict:
            ...
            ordered_repo_info_dicts = repair_dict.get(
            ...
            if ordered_tsr_ids and ordered_repo_info_dicts:
                ...
                return self.repair_tool_shed_repositories(
                    trans, rrm, repositories_for_repair,
                    ordered_repo_info_dicts)
        tool_shed_repository = repository_util.get_installed_tool_shed_repository(
            trans.app, repository_id)
        repair_dict = rrm.get_repair_dict(tool_shed_repository)
        ...
        ordered_repo_info_dicts = repair_dict.get('ordered_repo_info_dicts',
                                                  [])
        return trans.fill_template(
            'admin/tool_shed_repository/repair_repository.mako',
            repository=tool_shed_repository,
    ...
    @web.expose
    @web.require_admin
    def repair_tool_shed_repositories(self, trans, repair_repository_manager,
                                      tool_shed_repositories, repo_info_dicts,
                                      **kwd):
        ...
        for index, tool_shed_repository in enumerate(tool_shed_repositories):
            ...
            repair_repository_manager.repair_tool_shed_repository(
                tool_shed_repository,
    ...
    @web.json
    def repository_installation_status_updates(self,
                                               trans,
                                               ids=None,
                                               status_list=None):
        # Avoid caching
    ...
    def reselect_tool_panel_section(self, trans, **kwd):
        ...
        if latest_changeset_revision and latest_ctx_rev:
            ...
            encoded_updated_repo_info_dict = kwd.get('updated_repo_info_dict',
                                                     None)
            updated_repo_info_dict = encoding_util.tool_shed_decode(
                encoded_updated_repo_info_dict)
            readme_files_dict = updated_repo_info_dict.get(
                'readme_files_dict', None)
            includes_data_managers = updated_repo_info_dict.get(
                'includes_data_managers', False)
            includes_datatypes = updated_repo_info_dict.get(
                'includes_datatypes', False)
            includes_workflows = updated_repo_info_dict.get(
        ...
        if includes_tools_for_display_in_tool_panel:
            ...
            if original_section_name:
                ...
                message += "The tools contained in your <b>%s</b> repository were last loaded into the tool panel outside of any sections.  " % escape(
                    tool_shed_repository.name)
    ...
    def reset_repository_metadata(self, trans, id):
        ...
        repository_clone_url = common_util.generate_clone_url_for_installed_repository(
            trans.app, repository)
        tool_path, relative_install_dir = repository.get_tool_relative_path(
            trans.app)
        if relative_install_dir:
            ...
            if irmm_metadata_dict != original_metadata_dict:
                ...
                message = 'Metadata did not need to be reset on repository <b>%s</b>.' % escape(
                    repository.name)
                status = 'done'
        else:
            message = 'Error locating installation directory for repository <b>%s</b>.' % escape(
                repository.name)
            status = 'error'
        return trans.response.send_redirect(
            web.url_for(
    ...
    def reset_to_install(self, trans, **kwd):
        ...
        if kwd.get('reset_repository', False):
            repository_util.set_repository_attributes(
                trans.app,
                repository,
                status=trans.install_model.ToolShedRepository.
            ...
            new_kwd = {}
            new_kwd[
                'message'] = "You can now attempt to install the repository named <b>%s</b> again." % escape(
                    str(repository.name))
            new_kwd['status'] = "done"
            return trans.response.send_redirect(
                web.url_for(
    ...
    def set_tool_versions(self, trans, **kwd):
        ...
        if text:
            ...
            message = (
                "Version information for the tools included in the <b>%s</b> repository is missing.  "
                "Reset all of this reppository's metadata in the tool shed, then set the installed tool versions "
                "from the installed repository's <b>Repository Actions</b> menu.  "
    ...
    @web.json
    def tool_dependency_status_updates(self, trans, ids=None,
                                       status_list=None):
        # Avoid caching
        trans.response.headers['Pragma'] = 'no-cache'
        trans.response.headers['Expires'] = '0'
        ...
        if ids is not None and status_list is not None:
            ...
            for tup in zip(ids, status_list):
                ...
                tool_dependency = trans.install_model.context.query(
                    trans.install_model.ToolDependency).get(
                        trans.security.decode_id(id))
            ...
            if changeset_revision == latest_changeset_revision:
                ...
                if relative_install_dir:
                    ...
                    irmm = InstalledRepositoryMetadataManager(
                        ...
                        repository_clone_url=repository_clone_url,
                        shed_config_dict=repository.get_shed_config_dict(
                            trans.app),
                        relative_install_dir=relative_install_dir,
                        repository_files_dir=None,
                        resetting_all_metadata_on_repository=False,
                    ...
                    if 'repository_dependencies' in irmm_metadata_dict or 'tool_dependencies' in irmm_metadata_dict:
                        ...
                        if new_repository_dependencies:
                            ...
                            if proceed_to_install:
                                ...
                                new_kwd = dict(
                                    tool_shed_url=tool_shed_url,
                                    updating_repository_id=trans.security.
                                    encode_id(repository.id),
                                    updating_to_ctx_rev=latest_ctx_rev,
                                    updating_to_changeset_revision=
                                    latest_changeset_revision,
                                    encoded_updated_metadata=encoding_util.
                                    tool_shed_encode(irmm_metadata_dict),
                                    updating=True)
                                return self.prepare_for_install(
                                    trans, **new_kwd)
                        ...
                        for new_key, new_val in new_tool_dependencies_dict.items(
                            ...
                            if new_key not in original_tool_dependencies_dict:
                                proceed_to_install = True
                                break
                            original_val = original_tool_dependencies_dict[
                                new_key]
                ...
                if tool_metadata_dict['id'] == tool_id:
                    ...
                    if not tool:
                        tool = trans.app.toolbox.load_tool(
                            os.path.abspath(tool_config),
                            guid=tool_metadata['guid'])
                        if tool:
                            tool._lineage = trans.app.toolbox._lineage_map.register(
                                tool)
                    if tool:
                        tool_lineage = tool.lineage.get_version_ids(
                            reverse=True)
                    break
        return trans.fill_template(
