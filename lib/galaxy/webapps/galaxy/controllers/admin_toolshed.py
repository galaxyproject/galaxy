...
class AdminToolshed(AdminGalaxy):
    ...
    def activate_repository(self, trans, **kwd):
        ...
        try:
            ...
            return trans.response.send_redirect(
                web.url_for(controller='admin_toolshed', action='manage_repository', id=repository_id, message=message, status=status))
        message = 'The <b>%s</b> repository has been activated.' % escape(repository.name)
        status = 'done'
        return trans.response.send_redirect(
            web.url_for(controller='admin_toolshed', action='browse_repositories', message=message, status=status))
    ...
    def browse_repository(self, trans, **kwd):
        ...
        return trans.fill_template(
            '/admin/tool_shed_repository/browse_repository.mako', repository=repository, message=message, status=status)
    ...
    def browse_repositories(self, trans, **kwd):
        if 'operation' in kwd:
            ...
            if operation == "manage_repository":
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='manage_repository', **kwd))
            if operation == "get updates":
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='check_for_updates', **kwd))
            if operation == "update tool shed status":
                return trans.response.send_redirect(
                    web.url_for(controller='admin_toolshed', action='update_tool_shed_status_for_installed_repository', **kwd))
            if operation == "reset to install":
                kwd['reset_repository'] = True
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='reset_to_install', **kwd))
            if operation == "purge":
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='purge_repository', **kwd))
            if operation == "activate or reinstall":
                ...
                if repository.uninstalled:
                    ...
                    return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='activate_repository', **kwd))
            if operation == "deactivate or uninstall":
                return trans.response.send_redirect(
                    web.url_for(controller='admin_toolshed', action='deactivate_or_uninstall_repository', **kwd))
            if operation == "install latest revision":
                return trans.response.send_redirect(
                    web.url_for(controller='admin_toolshed', action='install_latest_repository_revision', **kwd))
    ...
    def browse_toolsheds(self, trans, **kwd):
        ...
        return trans.fill_template('galaxy.panels.mako', config={'title': 'Galaxy Tool Sheds', 'app': app})

    @web.expose
    @web.require_admin
    def browse_tool_sheds(self, trans, **kwd):
        message = escape(kwd.get('message', ''))
        return trans.fill_template('/webapps/galaxy/admin/tool_sheds.mako', message=message, status='error')
    ...
    def initiate_tool_dependency_installation(self, trans, tool_dependencies, **kwd):
        ...
        return trans.response.send_redirect(
            web.url_for(
                controller='admin_toolshed', action='manage_tool_dependencies', tool_dependency_ids=td_ids, message=message, status=status))
    ...
    def install_tool_dependencies_with_update(self, trans, **kwd):
        ...
        install_tool_dependencies_check_box = CheckboxField(
            'install_tool_dependencies', checked=install_tool_dependencies_check_box_checked)
    ...
    def manage_repositories(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if not tsridslist:
                ...
                kwd['status'] = 'error'
                del kwd['operation']
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='manage_repositories', **kwd))
            if operation == 'browse':
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='browse_repository', **kwd))
                ...
                if repositories_for_uninstallation:
                    return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='uninstall_repositories', **kwd))
                else:
    ...
    def manage_repository(self, trans, **kwd):
        ...
        if repository.can_install and operation == 'install':
            ...
            params = dict(
                name=name, owner=owner, changeset_revisions=installed_changeset_revision, galaxy_url=web.url_for('/', qualified=True))
            pathspec = ['repository', 'install_repositories_by_revision']
            url = util.build_url(tool_shed_url, pathspec=pathspec, params=params)
            return trans.response.send_redirect(url)
        ...
        containers_dict = dd.populate_containers_dict_from_repository_metadata(
            tool_shed_url=tool_shed_url, tool_path=tool_path, repository=repository, reinstalling=False, required_repo_info_dicts=None)
        view = views.DependencyResolversView(self.app)
    ...
    def manage_repository_tool_dependencies(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if not tool_dependency_ids:
                ...
                del kwd['operation']
                return trans.response.send_redirect(
                    web.url_for(controller='admin_toolshed', action='manage_repository_tool_dependencies', **kwd))
            if operation == 'browse':
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='browse_tool_dependency', **kwd))
            elif operation == 'uninstall':
                ...
                if tool_dependencies_for_uninstallation:
                    return trans.response.send_redirect(
                        web.url_for(controller='admin_toolshed', action='uninstall_tool_dependencies', **kwd))
                else:
                ...
                if trans.app.config.tool_dependency_dir:
                    ...
                    if tool_dependencies_for_installation:
                        self.initiate_tool_dependency_installation(
                            trans, tool_dependencies_for_installation, message=message, status=status)
                    else:
    ...
    def manage_tool_dependencies(self, trans, **kwd):
        ...
        if 'operation' in kwd:
            ...
            if not tool_dependency_ids:
                ...
                kwd['status'] = 'error'
                del kwd['operation']
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='manage_tool_dependencies', **kwd))
            if operation == 'browse':
                return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='browse_tool_dependency', **kwd))
            elif operation == "install":
                if trans.app.config.tool_dependency_dir:
    ...
    def prepare_for_install(self, trans, **kwd):
        ...
        tool_requirements = suc.get_tool_shed_repo_requirements(app=trans.app, tool_shed_url=tool_shed_url, repo_info_dicts=repo_info_dicts)
        view = views.DependencyResolversView(self.app)
    ...
    def purge_repository(self, trans, **kwd):
            ...
            if repository:
                if repository.is_new:
                    if kwd.get('purge_repository_button', False):
                        ...
                        return trans.fill_template('admin/tool_shed_repository/purge_repository_confirmation.mako', repository=repository)
                else:
                    new_kwd['status'] = 'error'
        ...
        return trans.response.send_redirect(web.url_for(controller='admin_toolshed', action='browse_repositories', **new_kwd))
    ...
    def reinstall_repository(self, trans, **kwd):
        ...
        if tool_shed_repository.status == trans.install_model.ToolShedRepository.installation_status.UNINSTALLED:
            repository_type = suc.get_repository_type_from_tool_shed(trans.app, tool_shed_url, tool_shed_repository.name,
            ...
            if repository_type == rt_util.TOOL_DEPENDENCY_DEFINITION:
                ...
                # revision.
                raw_text = suc.get_tool_dependency_definition_metadata_from_tool_shed(trans.app, tool_shed_url, tool_shed_repository.name,
                                                                                      tool_shed_repository.owner)
            ...
            tool_shed_repository = repository_util.create_or_update_tool_shed_repository(
                trans.app, tool_shed_repository.name, tool_shed_repository.description, tool_shed_repository.installed_changeset_revision,
                tool_shed_repository.ctx_rev, repository_clone_url, metadata,
                trans.install_model.ToolShedRepository.installation_status.NEW, tool_shed_repository.changeset_revision,
                tool_shed_repository.owner, tool_shed_repository.dist_to_shed)
        ctx_rev = suc.get_ctx_rev(trans.app, tool_shed_url, tool_shed_repository.name, tool_shed_repository.owner,
    ...
    def repair_tool_shed_repositories(self, trans, repair_repository_manager, tool_shed_repositories, repo_info_dicts, **kwd):
        ...
        for index, tool_shed_repository in enumerate(tool_shed_repositories):
            ...
            repair_repository_manager.repair_tool_shed_repository(tool_shed_repository, encoding_util.tool_shed_encode(repo_info_dict))
        tsr_ids_for_monitoring = [trans.security.encode_id(tsr.id) for tsr in tool_shed_repositories]
        return trans.response.send_redirect(
            web.url_for(
                controller='admin_toolshed', action='monitor_repository_installation', tool_shed_repository_ids=tsr_ids_for_monitoring))
                ...
                if repository.status != status:
                    rval.append(
                        dict(
                            ...
                            html_status=unicode(
                                trans.fill_template(
                                    "admin/tool_shed_repository/repository_installation_status.mako", repository=repository), 'utf-8')))
        return rval
    ...
    def reset_repository_metadata(self, trans, id):
        ...
        if relative_install_dir:
            ...
            status = 'error'
        return trans.response.send_redirect(
            web.url_for(controller='admin_toolshed', action='manage_repository', id=id, message=message, status=status))
                ...
                if tool_dependency.status != status:
                    rval.append(
                        dict(
                            ...
                            html_status=unicode(
                                trans.fill_template(
                                    "admin/tool_shed_repository/tool_dependency_installation_status.mako", tool_dependency=tool_dependency),
                                'utf-8')))
        return rval
                ...
                if relative_install_dir:
                    ...
                    if 'tools' in irmm_metadata_dict:
                        ...
                        if 'data_manager' in irmm_metadata_dict:
                            dmh = data_manager.DataManagerHandler(trans.app)
                            dmh.install_data_managers(trans.app.config.shed_data_manager_config_file, irmm_metadata_dict,
                                                      repository.get_shed_config_dict(trans.app),
                                                      os.path.join(relative_install_dir, name), repository, repository_tools_tups)
