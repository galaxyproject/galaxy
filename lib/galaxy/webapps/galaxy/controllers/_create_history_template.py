...
def render_item_hda(trans, hda, children):
    ...
    if hda.copied_from_history_dataset_association:
        ...
        template = f'<div id="hda-{trans.security.encode_id(hda.id)}" class="dataset hda state-{hda.state}"></div>'
    return template
...
def render_hda_copied_from_history(trans, hda, children):
    ...
    url = url_for(f"/histories/view?id={history_id}")
    template = '<div class="copied-from"><div class="header"><div>'
    template += '<span class="light"> Copied from history dataset: </span>'
    template += f'<span class="bold">{hda.copied_from_history_dataset_association.name}</span>'
    template += '</div><div class="copied-from-source">'
    template += '<span class="light">History: </span>'
    template += '<span class="bold">'
    template += f'<a href="{url}">{hda.copied_from_history_dataset_association.history.name}</a>'
    template += "</span></div></div>"
    template += f'<div id="hda-{id}" class="dataset hda state-{hda.state}"></div></div>'
    return template
...
def render_hda_copied_from_library(trans, hda, children):
    ...
    template += f'<span class="bold">{hda.copied_from_library_dataset_dataset_association.name}</span>'
    template += "</div>"
    template += '<div class="copied-from-source">'
    template += '<span class="light">Library: </span>'
    template += '<span class="bold">'
    template += f'<a href="{url}">{folder.name}</a></span></div></div>'
    template += f'<div id="hda-{id}" class="dataset hda state-{hda.state}"></div></div>'
    return template
...
def render_item_job(trans, job, children):
    ...
    try:
        ...
        pass
    template += f'<div class="header"><div><span class="bold">{tool_name}</span>'
    template += f'<span class="light"> - {tool_desc}</span></div>'
...
def render_item_wf(trans, wf, children):
    ...
    """
    template = f'<div class="workflow"><div class="header"><span class="bold">{wf.workflow.name}</span>'
...
def inputs_recursive_indent(text, depth):
    ...
    """
    return f'<td style="padding-left:{str((depth - 1) * 10)}px">{text}</td>'
        ...
        if input.name in param_values:
            if input.type == "repeat":
                ...
                if is_valid:
                    ...
                    tool_parameter_template += f"<td>{input.cases[current_case].value}</td><td></td></tr>"
                    inputs_recursive(
                        trans,
                    ...
                    tool_parameter_template += inputs_recursive_indent(text=input.name, depth=depth)
                    tool_parameter_template += (
                        "<td><em> The previously used value is no longer valid </em></td><td></td></tr>"
                    )
            elif input.type == "upload_dataset":
                tool_parameter_template += "<tr>"
                tool_parameter_template += inputs_recursive_indent(text=input.group_title(param_values), depth=depth)
                tool_parameter_template += (
                    f"<td>{str(len(param_values[input.name]))} uploaded datasets</td><td></td></tr>"
                )
                ...
                for i, element in enumerate(listify(param_values[input.name])):
                    ...
                    if element.history_content_type == "dataset":
                        ...
                        tool_parameter_template += f"{str(element.hid)}:{element.name}"
                ...
                tool_parameter_template += f"<td>{input.value_to_display_text(param_values[input.name])}</td>"
                tool_parameter_template += f"<td>{upgrade_messages.get(input.name, '')}</td></tr>"
        else:
            tool_parameter_template += "<tr>"
            if input.type == "conditional":
                ...
                tool_parameter_template += inputs_recursive_indent(text=label, depth=depth)
                tool_parameter_template += (
                    "<td><em> not used (parameter was added after this job was run)</em></td><td></td></tr>"
                )
