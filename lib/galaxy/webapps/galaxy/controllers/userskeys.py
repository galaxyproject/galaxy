...
class User(BaseUIController, UsesFormDefinitionsMixin):
    ...
    def admin_api_keys(self, trans, cntrller, uid, **kwd):
        ...
        return trans.response.send_redirect(
            web.url_for(controller='userskeys', action='all_users', cntrller=cntrller, message=message, status=status))
    ...
    def all_users(self, trans, cntrller="userskeys", **kwd):
        ...
        return trans.fill_template('webapps/galaxy/user/list_users.mako', cntrller=cntrller, users=users, message=message, status=status)
