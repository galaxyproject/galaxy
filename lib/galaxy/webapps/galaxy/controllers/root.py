...
class RootController(controller.JSAppLauncher, UsesAnnotations):
    ...
    def _get_extended_config(self, trans):
        app = trans.app
        user_requests = bool(
            trans.user and (trans.user.requests
                            or app.security_agent.get_accessible_request_types(
                                trans, trans.user)))
        config = {
            'active_view':
            'analysis',
            'enable_cloud_launch':
            app.config.get_bool('enable_cloud_launch', False),
            # TODO: next two should be redundant - why can't we build one from the other?
            'toolbox':
            app.toolbox.to_dict(trans, in_panel=False),
            'toolbox_in_panel':
            app.toolbox.to_dict(trans),
            'message_box_visible':
            app.config.message_box_visible,
            'show_inactivity_warning':
            app.config.user_activation_on and trans.user
        ...
        # TODO: move to user
        stored_workflow_menu_entries = config[
        ...
        for menu_item in getattr(trans.user, 'stored_workflow_menu_entries',
            ...
            stored_workflow_menu_entries.append({
                'encoded_stored_workflow_id':
                trans.security.encode_id(menu_item.stored_workflow_id),
                'stored_workflow': {
    ...
    def index(self,
        ...
        """
        if trans.app.config.require_login and self.user_manager.is_anonymous(
                trans.user):
    ...
    def tool_search(self, trans, **kwd):
        ...
        trans.log_action(trans.get_user(), "tool_search.search", "",
                         {"query": query,
                          "tags": tags})
        results = []
        if tags:
            tags = trans.sa_session.query(trans.app.model.Tag).filter(
                trans.app.model.Tag.name.in_(tags)).all()
            ...
            if trans.app.security_agent.can_access_dataset(
                ...
                if tofile:
                    ...
                    trans.response.headers[
                        "Content-Disposition"] = 'attachment; filename="GalaxyHistoryItem-%s-[%s]%s"' % (
    ...
    def display_child(self,
        ...
        try:
            data = trans.sa_session.query(
                self.app.model.HistoryDatasetAssociation).get(parent_id)
            if data:
                child = data.get_child_by_designation(designation)
                if child:
                    current_user_roles = trans.get_current_user_roles()
                    if trans.app.security_agent.can_access_dataset(
                            current_user_roles, child):
                        return self.display(
                            trans, id=child.id, tofile=tofile, toext=toext)
                    else:
                        return "You are not privileged to access this dataset."
        except Exception:
            pass
        return "A child named %s could not be found for data %s" % (
            designation, parent_id)
    ...
    def display_as(self, trans, id=None, display_app=None, **kwd):
        ...
        if data:
            ...
            if authz_method == 'rbac' and trans.app.security_agent.can_access_dataset(
                ...
                trans.response.set_content_type(data.get_mime())
                trans.log_event("Formatted dataset id %s for display at %s" %
                                (str(id), display_app))
    ...
    def history_delete(self, trans, id):
        ...
        """
        # TODO: unused?
        return trans.webapp.controllers['history'].list(
            trans, id, operation='delete')
    ...
    def history_import(self, trans, id=None, confirm=False, **kwd):
        ...
        import_history = trans.sa_session.query(
            trans.app.model.History).get(id)
        ...
        if user:
            if import_history.user_id == user.id:
                return trans.show_error_message(
                    "You cannot import your own history.")
            ...
            try:
                ...
                association = None
            new_history.add_galaxy_session(
                galaxy_session, association=association)
            trans.sa_session.add(new_history)
            trans.sa_session.flush()
            if not user_history.datasets:
                trans.set_history(new_history)
            trans.log_event("History imported, id: %s, name: '%s': " %
    ...
    def history_new(self, trans, name=None):
        ...
        return trans.show_message(
            "New history created", refresh_frames=['history'])
    ...
    def history_add_to(self,
        ...
        try:
            ...
            return trans.show_ok_message("Dataset " + str(
                data.hid) + " added to history " + str(history_id) + ".")
        except Exception as e:
            msg = "Failed to add dataset to history: %s" % (e)
            log.error(msg)
            trans.log_event(msg)
            return trans.show_error_message(
                "Adding File to History has Failed")
    ...
    def history_set_default_permissions(self, trans, id=None, **kwd):
        ...
        if trans.user:
            if 'update_roles_button' in kwd:
                ...
                return trans.show_ok_message(
                    'Default history permissions have been changed.')
            return trans.fill_template('history/permissions.mako')
        else:
            # user not logged in, history group must be only public
            return trans.show_error_message(
                "You must be logged in to change a history's default permissions."
            )
