...
class DataManager(BaseUIController):
    ...
    def manage_data_manager(self, trans, **kwd):
        ...
        return trans.fill_template(
            "data_manager/manage_data_manager.mako",
    ...
    def view_job(self, trans, **kwd):
        ...
        if not job:
            return trans.response.send_redirect(
                web.url_for(controller="data_manager", action="index", message="Invalid job (%s) was requested" % job_id, status="error"))
        ...
        if data_table is None:
            return trans.response.send_redirect(
                web.url_for(
                    ...
                    status="error"))
        return trans.fill_template(
            "data_manager/manage_data_table.mako", data_table=data_table, view_only=not_is_admin, message=message, status=status)
    ...
    def reload_tool_data_tables(self, trans, table_name=None, **kwd):
        ...
        galaxy.queue_worker.send_control_task(trans.app, 'reload_tool_data_tables', noop_self=True, kwargs={'table_name': table_name})
        redirect_url = None
        if table_names:
            status = 'done'
            if len(table_names) == 1:
                ...
                redirect_url = web.url_for(
                    controller='data_manager', action='manage_data_table', table_name=table_names[0], message=message, status=status)
            else:
        ...
        if redirect_url is None:
            redirect_url = web.url_for(controller='admin', action='view_tool_data_tables', message=message, status=status)
