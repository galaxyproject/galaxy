...
class DataManager(BaseUIController):
        ...
        for hda in hdas:
            try:
                ...
                error_messages.append(
                    escape("Unable to obtain data_table info for hda (%s): %s"
                           % (hda.id, e)))
            values = []
        ...
        if data_table is None:
            return trans.response.send_redirect(
                web.url_for(
                    ...
                    status="error"))
        return trans.fill_template(
            "data_manager/manage_data_table.mako",
    ...
    def reload_tool_data_tables(self, trans, table_name=None, **kwd):
        ...
        table_names = self.app.tool_data_tables.reload_tables(
        ...
        if table_names:
            ...
            if len(table_names) == 1:
                message = "The data table '%s' has been reloaded." % table_names[
                    0]
                ...
                message = "The data tables '%s' have been reloaded." % ', '.join(
                    table_names)
        else:
