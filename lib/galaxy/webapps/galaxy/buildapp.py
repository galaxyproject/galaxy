...
def populate_api_routes(webapp, app):
    ...
    webapp.mapper.connect('/api/upload/resumable_upload/{session_id}', controller="uploads", action="tus", conditions=dict(method=["PATCH"]))
    webapp.mapper.connect('/api/upload/_resumable_upload', controller="uploads", action="tus", conditions=dict(method=["POST"]))
