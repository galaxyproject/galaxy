...
class ActionSuggestion(BaseModel):
    ...
    priority: int = Field(
        default=1, description="Priority level (1=high, 2=medium, 3=low)"
    )
...
class AgentResponse(BaseModel):
    ...
    reasoning: Optional[str] = Field(
        default=None, description="Explanation of the agent's reasoning"
    )
...
class AgentQueryRequest(BaseModel):
    ...
    agent_type: str = Field(
        default="auto", description="Preferred agent type ('auto' for routing)"
    )
...
class AgentQueryResponse(BaseModel):
    ...
    processing_time: Optional[float] = Field(
        default=None, description="Time taken to process the query in seconds"
    )
...
class AvailableAgent(BaseModel):
    ...
    model: Optional[str] = Field(
        default=None, description="LLM model used by the agent"
    )
...
class RoutingDecision(BaseModel):
    ...
    secondary_agents: List[str] = Field(
        default_factory=list, description="Additional agents that might help"
    )
    ...
    direct_response: str = Field(
        default="", description="Direct response if routing is not needed"
    )
...
class ErrorAnalysisRequest(BaseModel):
    ...
    job_id: Optional[int] = Field(
        default=None, description="Galaxy job ID associated with the error"
    )
    error_text: Optional[str] = Field(
        default=None, description="Specific error message text"
    )
    tool_id: Optional[str] = Field(
        default=None, description="Tool that caused the error"
    )
...
class ErrorCategory(BaseModel):
    ...
    subcategory: Optional[str] = Field(
        default=None, description="More specific error subcategory"
    )
...
class ErrorAnalysisResponse(BaseModel):
    ...
    related_documentation: List[str] = Field(
        default_factory=list, description="Relevant documentation links"
    )
    requires_admin: bool = Field(
        default=False, description="Whether admin intervention is needed"
    )
...
class ToolRecommendationRequest(BaseModel):
    ...
    input_format: Optional[str] = Field(
        default=None, description="Format of input data"
    )
    output_format: Optional[str] = Field(
        default=None, description="Desired output format"
    )
    preferences: Dict[str, Any] = Field(
        default_factory=dict, description="User preferences or constraints"
    )
...
class ToolSuggestion(BaseModel):
    ...
    suggested_parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Recommended parameter values"
    )
...
class ToolRecommendationResponse(BaseModel):
    ...
    recommended_tools: List[ToolSuggestion] = Field(
        description="List of recommended tools"
    )
    workflow_suggestions: List[str] = Field(
        default_factory=list, description="Suggestions for workflow structure"
    )
    parameter_recommendations: Dict[str, Any] = Field(
        default_factory=dict, description="General parameter guidance"
    )
...
class QualityIssue(BaseModel):
    ...
    affected_records: Optional[int] = Field(
        default=None, description="Number of affected records"
    )
...
class WorkflowOptimizationRequest(BaseModel):
    ...
    workflow_id: Optional[str] = Field(
        default=None, description="Galaxy workflow identifier"
    )
    workflow_structure: Optional[Dict[str, Any]] = Field(
        default=None, description="Workflow structure data"
    )
    performance_goals: List[str] = Field(
...
class WorkflowOptimizationResponse(BaseModel):
    ...
    estimated_time_savings: Optional[str] = Field(
        default=None, description="Estimated time savings"
    )
    confidence: ConfidenceLevel = Field(description="Confidence in the analysis")
...
class AgentStatus(BaseModel):
    ...
    health_status: str = Field(
        description="Health status (healthy, degraded, unavailable)"
    )
    ...
    model_info: Dict[str, Any] = Field(
        default_factory=dict, description="Information about the underlying model"
    )
