...
class DatabaseToolSourceStore(ToolSourceStore):
    ...
    def _read_session(self) -> Iterator[Session]:
        ...
        be called mid-request, and a ``rollback()`` issued afterwards to
        release the implicit read transaction would expire the caller's
        request-scoped objects. A private ``Session`` bound to the same
        engine sees the same committed data but its lifecycle is isolated
        from the caller's.
        """
