...
class Tree(object):
    ...
    @staticmethod
    def for_dataset_collection(dataset_collection,
    ...
    def walk_collections(self, hdca_dict):
        return self._walk_collections(
            dict_map(lambda hdca: hdca.collection, hdca_dict))
    ...
    def can_match(self, other_structure):
        ...
        for my_child, other_child in zip(self.children,
                                         other_structure.children):
            # At least one is nested collection...
            if my_child[1].is_leaf != other_child[1].is_leaf:
                return False

            if not my_child[1].is_leaf and not my_child[1].can_match(
                    other_child[1]):
                return False
    ...
    def element_identifiers_for_outputs(self, trans, outputs):
        ...
        for identifier, child in self.children:
            if isinstance(child, Tree):
                child_identifiers = child.element_identifiers_for_outputs(
                    trans, outputs[0:len(child)])
                ...
                element_identifiers.append(
                    dict(name=identifier, __object__=output_object))
                if hasattr(output_object, "collection_type"):
                    elements_collection_type = output_object.collection_type

            outputs = outputs[len(child):]

        collection_type = map_over_collection_type(
            self.collection_type_description.rank_collection_type(),
    ...
    def multiply(self, other_structure):
        ...
        for (identifier, structure) in self.children:
            new_children.append((identifier,
                                 structure.multiply(other_structure)))
...
def get_structure(dataset_collection_instance,
    ...
    return Tree.for_dataset_collection(dataset_collection_instance.collection,
                                       collection_type_description)
