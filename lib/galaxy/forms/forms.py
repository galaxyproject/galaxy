...
FORM_TYPES = dict([(f_type.lower(), f_descript)
    ...
    def from_elem(self, elem, form_definition_current=None):
        ...
        if fields_elem is not None:
            for field_elem in fields_elem.findall('field'):
                ...
                fields.append(self.field_type_factories[field_type]
                              .from_elem(field_elem, layout))
...
class FormDefinitionFieldFactory(object):
    ...
    def from_elem(self, elem, layout=None):
        ...
        if field_layout:
            assert layout and field_layout in layout, 'Invalid layout specified: %s not in %s' % (
                field_layout, layout)
            field_layout = str(
...
class FormDefinitionWorkflowMappingFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionWorkflowMappingFieldFactory,
                     self).from_elem(
                         elem, layout=layout)
        rval['type'] = self.__get_stored_field_type()
        return rval
...
field_type_factories = dict(
    [(field.type, field())
     for field in (FormDefinitionTextFieldFactory,
