...
def upgrade(migrate_engine):
    ...
    importable_column = Column(
        "importable", Boolean, default=False, index=True)
    slug_column = Column("slug", TEXT)
    published_column = Column("published", Boolean, index=True)

    try:
        # Add column.
        deleted_column.create(
            Visualiation_table, index_name="ix_visualization_deleted")
...
def downgrade(migrate_engine):
    ...
    try:
        ...
        log.exception(
            "Dropping visualization_user_share_association table failed.")
    ...
    try:
        ...
        log.exception(
            "Dropping deleted column from visualization table failed.")

    try:
        Visualiation_table.c.importable.drop()
    except Exception:
        log.exception(
            "Dropping importable column from visualization table failed.")
    ...
    try:
        ...
        log.exception(
            "Dropping published column from visualization table failed.")
    ...
    try:
        ...
        log.exception(
            "Dropping visualization_annotation_association table failed.")
