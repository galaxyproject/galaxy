...
User_table = Table("galaxy_user", metadata,
                   ...
                   Column("password", TrimmedString(40), nullable=False), Column("external", Boolean, default=False))
...
History_table = Table("history", metadata,
                      ...
                      Column("purged", Boolean, index=True, default=False), Column("genome_build", TrimmedString(40)))
...
ImplicitlyConvertedDatasetAssociation_table = Table("implicitly_converted_dataset_association", metadata,
                                                    ...
                                                    Column(
                                                        ...
                                                        nullable=True),
                                                    Column(
                                                        "hda_parent_id", Integer, ForeignKey("history_dataset_association.id"), index=True),
                                                    Column("deleted", Boolean, index=True, default=False),
...
ValidationError_table = Table("validation_error", metadata,
                              ...
                              Column("message", TrimmedString(255)), Column("err_type", TrimmedString(64)), Column("attributes", TEXT))
...
Job_table = Table("job", metadata,
                  ...
                  Column("job_runner_name", String(255)), Column("job_runner_external_id", String(255)))

JobParameter_table = Table("job_parameter", metadata,
                           Column("id", Integer, primary_key=True),
                           Column("job_id", Integer, ForeignKey("job.id"), index=True), Column("name", String(255)), Column("value", TEXT))
...
GalaxySession_table = Table("galaxy_session", metadata,
                            ...
                            Column("is_valid", Boolean, default=False), Column("prev_session_id", Integer))
...
StoredWorkflow_table = Table("stored_workflow", metadata,
                             ...
                             Column(
                                 ...
                                 index=True), Column("name", TEXT), Column("deleted", Boolean, default=False))
...
Workflow_table = Table("workflow", metadata,
                       ...
                       Column("name", TEXT), Column("has_cycles", Boolean), Column("has_errors", Boolean))
...
WorkflowStep_table = Table("workflow_step", metadata,
                           ...
                           Column("position", JSONType), Column("config", JSONType), Column("order_index", Integer))
...
WorkflowStepConnection_table = Table("workflow_step_connection", metadata,
                                     ...
                                     Column("output_name", TEXT), Column("input_name", TEXT))
...
StoredWorkflowMenuEntry_table = Table("stored_workflow_menu_entry", metadata,
                                      Column("id", Integer, primary_key=True),
                                      Column("stored_workflow_id", Integer, ForeignKey("stored_workflow.id"), index=True),
                                      Column("user_id", Integer, ForeignKey("galaxy_user.id"), index=True), Column("order_index", Integer))
