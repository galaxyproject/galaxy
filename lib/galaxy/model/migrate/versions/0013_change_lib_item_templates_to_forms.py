...
def upgrade(migrate_engine):
    ...
    try:
        ...
        log.debug(
            "Failed loading table library_item_info_template_permissions")
    try:
    ...
    try:
        ...
        log.debug(
            "Failed loading table library_folder_info_template_association")
    try:
    ...
    try:
        ...
        log.debug(
            "Failed loading table library_dataset_info_template_association")
    try:
    ...
    try:
        ...
        log.debug(
            "Failed loading table library_dataset_dataset_info_association")
    try:
    ...
    try:
        LibraryItemInfo_table = Table(
            "library_item_info", metadata, autoload=True)
    except NoSuchTableError:
    ...
    try:
        ...
        log.exception(
            "Creating library_dataset_dataset_info_association table failed.")
    # Fix index on LibraryDatasetDatasetInfoAssociation_table for mysql
    if migrate_engine.name == 'mysql':
        # Load existing tables
        metadata.reflect()
        i = Index("ix_lddaia_ldda_id",
                  LibraryDatasetDatasetInfoAssociation_table.c.
                  library_dataset_dataset_association_id)
        try:
            i.create()
        except Exception:
