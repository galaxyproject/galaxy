...
def create_sequencer_form_definition(migrate_engine):
    ...
    cmd = cmd % (nextval(migrate_engine, 'form_definition_current'), localtimestamp(migrate_engine), localtimestamp(migrate_engine), 'NULL',
    ...
    fields = [('Host', 'TextField'), ('User name', 'TextField'), ('Password', 'PasswordField'), ('Data directory', 'TextField')]
    ...
    form_definition_fields.append({
        ...
        'type':
        'SelectField',
        'selectlist': ['Do not rename', 'Preprend sample name', 'Prepend experiment name', 'Prepend experiment and sample name'],
        'layout':
        'none',
        'default':
        ''
    ...
    cmd = cmd % (nextval(migrate_engine, 'form_definition'), localtimestamp(migrate_engine), localtimestamp(migrate_engine),
                 form_definition_name, form_definition_desc, form_definition_current_id, dumps(form_definition_fields),
                 form_definition_type, form_definition_layout)
    migrate_engine.execute(cmd)
...
def add_sequencer(migrate_engine, sequencer_index, sequencer_form_definition_id, sequencer_info):
    ...
    cmd = "INSERT INTO form_values VALUES ( %s, %s, %s, %s, '%s' )" % (nextval(migrate_engine, 'form_values'),
                                                                       localtimestamp(migrate_engine), localtimestamp(migrate_engine),
                                                                       sequencer_form_definition_id, values)
    migrate_engine.execute(cmd)
    ...
    cmd = cmd % (nextval(migrate_engine, 'sequencer'), localtimestamp(migrate_engine), localtimestamp(migrate_engine), name, desc,
                 sequencer_type_id, version, sequencer_form_definition_id, sequencer_form_values_id, boolean(migrate_engine, 'false'))
    migrate_engine.execute(cmd)
