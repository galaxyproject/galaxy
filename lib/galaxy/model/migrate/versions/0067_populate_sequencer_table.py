...
def get_latest_id(migrate_engine, table):
    result = migrate_engine.execute(
        "select id from %s order by id desc" % table)
    row = result.fetchone()
    if row:
        return row[0]
    else:
...
def create_sequencer_form_definition(migrate_engine):
    ...
    cmd = cmd % (nextval(migrate_engine, 'form_definition_current'),
                 ...
                 localtimestamp(migrate_engine), 'NULL',
                 boolean(migrate_engine, 'false'))
    migrate_engine.execute(cmd)
    # get this form_definition_current id
    form_definition_current_id = get_latest_id(migrate_engine,
    ...
    fields = [('Host', 'TextField'), ('User name', 'TextField'),
              ('Password', 'PasswordField'), ('Data directory', 'TextField')]
    ...
    form_definition_fields.append({
        ...
        'field_%i' % len(fields),
        'label':
        'Prepend the experiment name and sample name to the dataset name?',
        'helptext':
        'Galaxy datasets are renamed by prepending the experiment name and sample name to the dataset name, ensuring dataset names remain unique in Galaxy even when multiple datasets have the same name on the sequencer.',
        'visible':
        True,
        'required':
        False,
        ...
        'SelectField',
        'selectlist': [
            'Do not rename', 'Preprend sample name', 'Prepend experiment name',
    ...
    cmd = cmd % (nextval(migrate_engine, 'form_definition'),
                 localtimestamp(migrate_engine),
                 localtimestamp(migrate_engine), form_definition_name,
                 form_definition_desc, form_definition_current_id,
                 dumps(form_definition_fields), form_definition_type,
    ...
    cmd = "UPDATE form_definition_current SET latest_form_id=%i WHERE id=%i" % (
        form_definition_id, form_definition_current_id)
    migrate_engine.execute(cmd)
...
def add_sequencer(migrate_engine, sequencer_index,
                  sequencer_form_definition_id, sequencer_info):
    ...
    cmd = "INSERT INTO form_values VALUES ( %s, %s, %s, %s, '%s' )" % (
        nextval(migrate_engine, 'form_values'), localtimestamp(migrate_engine),
        localtimestamp(migrate_engine), sequencer_form_definition_id, values)
    migrate_engine.execute(cmd)
    ...
    cmd = cmd % (nextval(migrate_engine, 'sequencer'),
                 localtimestamp(migrate_engine),
                 localtimestamp(migrate_engine), name, desc, sequencer_type_id,
                 version, sequencer_form_definition_id,
                 sequencer_form_values_id, boolean(migrate_engine, 'false'))
    migrate_engine.execute(cmd)
    return get_latest_id(migrate_engine, 'sequencer')


def update_sequencer_id_in_request_type(migrate_engine, request_type_id,
                                        sequencer_id):
...
def upgrade(migrate_engine):
    ...
    try:
        col = Column(
            "sequencer_id", Integer, ForeignKey("sequencer.id"), nullable=True)
            ...
            if sequencer_info and isinstance(sequencer_info, dict):
                ...
                if not sequencer_id:
                    ...
                    sequencer_id = add_sequencer(
                        migrate_engine, sequencer_index,
                        sequencer_form_definition_id, sequencer_info)
