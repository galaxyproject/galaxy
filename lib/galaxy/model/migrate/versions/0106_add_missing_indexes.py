...
indexes = (
    ...
    ("ix_lddia_deleted", 'library_dataset_dataset_info_association',
     'deleted'),  # 0036
    ...
    ("ix_lfia_inheritable", 'library_folder_info_association',
     'inheritable'),  # 0038
    ("ix_job_imported", 'job', 'imported'),  # 0051
    ("ix_request_notification", 'request', 'notification'),  # 0057
    ("ix_sd_external_service_id", 'sample_dataset',
     'external_service_id'),  # 0068
    ("ix_icda_ldda_parent_id", 'implicitly_converted_dataset_association',
     'ldda_parent_id'),  # 0073
    ("ix_library_dataset_purged", 'library_dataset', 'purged'),  # 0074
    ("ix_run_subindex", 'run', 'subindex'),  # 0075
    ("ix_history_dataset_association_purged", 'history_dataset_association',
     'purged'),  # 0078
    ("ix_galaxy_user_disk_usage", 'galaxy_user', 'disk_usage'),  # 0078
    ("ix_galaxy_session_disk_usage", 'galaxy_session', 'disk_usage'),  # 0078
    ("ix_icda_ldda_id", 'implicitly_converted_dataset_association',
     'ldda_id'),  # 0084
    ("ix_tsr_includes_datatypes", 'tool_shed_repository',
     'includes_datatypes'),  # 0086
    ("ix_dataset_object_store_id", 'dataset', 'object_store_id'),  # 0089
    ("ix_job_object_store_id", 'job', 'object_store_id'),  # 0089
    ("ix_metadata_file_object_store_id", 'metadata_file',
     'object_store_id'),  # 0089
...
def upgrade(migrate_engine):
    ...
    for ix, table, col in indexes:
        try:
            log.debug("Creating index '%s' on column '%s' in table '%s'" %
                      (ix, col, table))
            t = Table(table, metadata, autoload=True)
            if ix not in [
                    ins_ix.get('name', None)
                    for ins_ix in insp.get_indexes(table)
            ]:
