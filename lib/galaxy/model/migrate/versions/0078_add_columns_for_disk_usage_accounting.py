...
def upgrade(migrate_engine):
    ...
    try:
        ...
        log.exception(
            "Adding purged column to history_dataset_association table failed."
        )
    ...
    try:
        ...
        c = Column('disk_usage', Numeric(15, 0), index=True)
        c.create(
            GalaxySession_table, index_name="ix_galaxy_session_disk_usage")
        assert c is GalaxySession_table.c.disk_usage
    except Exception:
        log.exception(
            "Adding disk_usage column to galaxy_session table failed.")
...
def downgrade(migrate_engine):
    ...
    try:
        ...
        log.exception(
            "Dropping purged column from history_dataset_association table failed."
        )
    ...
    try:
        ...
        log.exception(
            "Dropping disk_usage column from galaxy_user table failed.")
    ...
    try:
        ...
        log.exception(
            "Dropping disk_usage column from galaxy_session table failed.")
