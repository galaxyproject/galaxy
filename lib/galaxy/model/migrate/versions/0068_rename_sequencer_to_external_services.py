...
def upgrade(migrate_engine):
    ...
    col = Column(
        "external_service_id",
        Integer,
        ForeignKey("external_service.id", name="sample_dataset_external_services_id_fk"),
        index=True,
    )
    ...
    for row in results_list:
        ...
        cmd = "INSERT INTO request_type_external_service_association VALUES ( {}, {}, {} )".format(
            nextval(migrate_engine, "request_type_external_service_association"), request_type_id, sequencer_id
        )
        migrate_engine.execute(cmd)
