...
WorkflowTagAssociation_table = Table(
    "workflow_tag_association", metadata,
    Column("id", Integer, primary_key=True),
...
def upgrade(migrate_engine):
    ...
    HistoryTagAssociation_table = Table(
        "history_tag_association", metadata, autoload=True)
    if migrate_engine.name != 'sqlite':
        c = Column(
            "user_id", Integer, ForeignKey("galaxy_user.id"), index=True)
        try:
            c.create(
                HistoryTagAssociation_table,
                index_name='ix_history_tag_association_user_id')
            assert c is HistoryTagAssociation_table.c.user_id
        except Exception:
            log.exception(
                "Adding user_id column to history_tag_association table failed."
            )
    else:
    ...
    if migrate_engine.name != 'sqlite':
        ...
        try:
            ...
            log.exception(
                "Adding user_id column to history_dataset_association_tag_association table failed."
            )
    else:
    ...
    if migrate_engine.name != 'sqlite':
        ...
        PageTagAssociation_table = Table(
            "page_tag_association", metadata, autoload=True)
        c = Column(
            "user_id", Integer, ForeignKey("galaxy_user.id"), index=True)
        try:
            c.create(
                PageTagAssociation_table,
                index_name='ix_page_tag_association_user_id')
            assert c is PageTagAssociation_table.c.user_id
        except Exception:
            log.exception(
                "Adding user_id column to page_tag_association table failed.")
    else:
        # Create user_id column in page_tag_association table.
        PageTagAssociation_table = Table(
            "page_tag_association", metadata, autoload=True)
    ...
    try:
        ...
        log.exception("Creating stored_workflow_tag_association table failed.")

    # Create workflow_tag_association table.
    try:
        WorkflowTagAssociation_table.create()
    except Exception:
        log.exception("Creating workflow_tag_association table failed.")
...
def downgrade(migrate_engine):
    ...
    try:
        ...
        log.exception(
            "Dropping column user_id from history_tag_association table failed."
        )
    ...
    try:
        ...
        log.exception(
            "Dropping column user_id from history_dataset_association_tag_association table failed."
        )
    ...
    try:
        ...
        log.exception(
            "Dropping column user_id from page_tag_association table failed.")

    # Drop stored_workflow_tag_association table.
    try:
        StoredWorkflowTagAssociation_table.drop()
    except Exception:
        log.exception("Dropping stored_workflow_tag_association table failed.")

    # Drop workflow_tag_association table.
    try:
        WorkflowTagAssociation_table.drop()
    except Exception:
        log.exception("Dropping workflow_tag_association table failed.")
