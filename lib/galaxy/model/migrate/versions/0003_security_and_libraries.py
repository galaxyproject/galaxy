...
LibraryDatasetDatasetAssociationPermissions_table = Table("library_dataset_dataset_association_permissions", metadata,
                                                          ...
                                                          Column(
                                                              ...
                                                              nullable=True), Column("role_id", Integer, ForeignKey("role.id"), index=True))
...
LibraryItemInfoPermissions_table = Table("library_item_info_permissions", metadata,
                                         ...
                                         Column("action", TEXT),
                                         Column(
                                             "library_item_info_id", Integer, ForeignKey("library_item_info.id"), nullable=True,
...
LibraryDatasetDatasetAssociation_table = Table(
    ...
    Column("deleted", Boolean, index=True, default=False),
    Column("visible", Boolean), Column("user_id", Integer, ForeignKey("galaxy_user.id"), index=True), Column("message", TrimmedString(255)))
...
Library_table = Table("library", metadata,
                      ...
                      Column("deleted", Boolean, index=True, default=False),
                      Column("purged", Boolean, index=True, default=False), Column("description", TEXT))
...
LibraryItemInfoTemplate_table = Table("library_item_info_template", metadata,
                                      ...
                                      Column("optional", Boolean, index=True, default=True),
                                      Column("deleted", Boolean, index=True, default=False),
                                      Column("name", TEXT), Column("description", TEXT), Column("item_count", Integer, default=0))
...
LibraryDatasetInfoAssociation_table = Table("library_dataset_info_association", metadata,
                                            ...
                                            Column("update_time", DateTime, default=now, onupdate=now),
                                            Column(
                                                "library_dataset_id", Integer, ForeignKey("library_dataset.id"), nullable=True, index=True),
                                            Column("library_item_info_id", Integer, ForeignKey("library_item_info.id"), index=True),
                                            Column("user_id", Integer, ForeignKey("galaxy_user.id"), nullable=True, index=True))
...
JobExternalOutputMetadata_table = Table("job_external_output_metadata", metadata,
                                        ...
                                        Column("filename_kwds", String(255)), Column("job_runner_external_pid", String(255)))
...
def upgrade(migrate_engine):
    ...
    if migrate_engine.name != 'sqlite':
        ...
        if MetadataFile_table is not None and LibraryDatasetDatasetAssociation_table is not None:
            try:
                cons = ForeignKeyConstraint(
                    [MetadataFile_table.c.lda_id], [LibraryDatasetDatasetAssociation_table.c.id], name='metadata_file_lda_id_fkey')
                # Create the constraint
                cons.create()
            except Exception:
