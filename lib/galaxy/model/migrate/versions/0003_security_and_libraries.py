...
LibraryDatasetDatasetAssociationPermissions_table = Table(
    ...
    Column(
        ...
        nullable=True),
    Column("role_id", Integer, ForeignKey("role.id"), index=True))
Index("ix_lddap_library_dataset_dataset_association_id",
      LibraryDatasetDatasetAssociationPermissions_table.c.
      library_dataset_dataset_association_id)

LibraryItemInfoPermissions_table = Table(
    "library_item_info_permissions", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column("action", TEXT),
...
LibraryItemInfoTemplatePermissions_table = Table(
    "library_item_info_template_permissions", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column("action", TEXT),
    Column(
        ...
        nullable=True),
    Column("role_id", Integer, ForeignKey("role.id"), index=True))
Index("ix_liitp_library_item_info_template_id",
...
LibraryItemInfoTemplateElement_table = Table(
    "library_item_info_template_element", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    ...
    Column("library_item_info_template_id", Integer,
           ForeignKey("library_item_info_template.id")))
Index("ix_liite_library_item_info_template_id",
      LibraryItemInfoTemplateElement_table.c.library_item_info_template_id)

LibraryItemInfoTemplate_table = Table(
    "library_item_info_template", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column("optional", Boolean, index=True, default=True),
    Column("deleted", Boolean, index=True, default=False),
    Column("name", TEXT),
    Column("description", TEXT), Column("item_count", Integer, default=0))

LibraryInfoTemplateAssociation_table = Table(
    "library_info_template_association", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
...
LibraryDatasetInfoTemplateAssociation_table = Table(
    "library_dataset_info_template_association", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
...
LibraryDatasetDatasetInfoTemplateAssociation_table = Table(
    "library_dataset_dataset_info_template_association", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
    ...
    Column("library_item_info_template_id", Integer,
           ForeignKey("library_item_info_template.id")))
Index("ix_lddita_library_dataset_dataset_association_id",
      LibraryDatasetDatasetInfoTemplateAssociation_table.c.
      library_dataset_dataset_association_id)
Index("ix_lddita_library_item_info_template_id",
      LibraryDatasetDatasetInfoTemplateAssociation_table.c.
      library_item_info_template_id)

LibraryItemInfoElement_table = Table(
    "library_item_info_element", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    ...
    Column(
        ...
        ForeignKey("library_item_info.id"),
        index=True),
    Column("library_item_info_template_element_id", Integer,
           ForeignKey("library_item_info_template_element.id")))
Index("ix_liie_library_item_info_template_element_id",
...
LibraryDatasetInfoAssociation_table = Table(
    "library_dataset_info_association", metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
...
LibraryDatasetDatasetInfoAssociation_table = Table(
    ...
    Column(
        ...
        ForeignKey("library_dataset_dataset_association.id"),
        nullable=True),
    Column("library_item_info_id", Integer,
           ForeignKey("library_item_info.id")),
    Column(
        "user_id",
        Integer,
        ...
        nullable=True,
        index=True))
Index("ix_lddia_library_dataset_dataset_association_id",
      LibraryDatasetDatasetInfoAssociation_table.c.
      library_dataset_dataset_association_id)
Index("ix_lddia_library_item_info_id",
...
Index("ix_jeom_library_dataset_dataset_association_id",
...
def upgrade(migrate_engine):
    ...
    if User_table is not None:
        try:
            ...
            log.exception(
                "Adding column 'deleted' to galaxy_user table failed.")
        try:
            ...
            log.exception(
                "Adding column 'purged' to galaxy_user table failed.")
    ...
    if MetadataFile_table is not None:
        try:
            ...
            col.create(
                MetadataFile_table, index_name='ix_metadata_file_lda_id')
    ...
    if users:
        ...
        if not roles:
            ...
            cmd = \
                ...
                "FROM galaxy_user " + \
                "ORDER BY id;"
            cmd = cmd % (nextval('role'), localtimestamp(), localtimestamp(),
            ...
            cmd = cmd % (nextval('user_role_association'), localtimestamp(),
                         localtimestamp())
            migrate_engine.execute(cmd)
            ...
            cmd = cmd % (nextval('default_history_permissions'),
                         engine_false(migrate_engine))
            migrate_engine.execute(cmd)
            ...
            cmd = cmd % (nextval('dataset_permissions'), localtimestamp(),
                         localtimestamp(), engine_false(migrate_engine))
            migrate_engine.execute(cmd)
...
def downgrade(migrate_engine):
    ...
    try:
        StoredWorkflow_table = Table(
            "stored_workflow", metadata, autoload=True)
    except NoSuchTableError:
        StoredWorkflow_table = None
        log.debug("Failed loading table stored_workflow")
    if StoredWorkflow_table is not None:
