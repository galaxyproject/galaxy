...
from sqlalchemy import (and_, asc, Boolean, Column, DateTime, desc, false,
                        ForeignKey, Integer, MetaData, not_, Numeric, select,
                        String, Table, TEXT, Text, true, Unicode,
                        UniqueConstraint)
...
model.Dataset.table = Table(
    ...
    Column('total_size', Numeric(15, 0)), Column('uuid', UUIDType()))
...
model.ValidationError.table = Table(
    ...
    Column("err_type", TrimmedString(64)), Column("attributes", TEXT))
...
model.LibraryFolder.table = Table(
    ...
    Column("description", TEXT),
    Column("order_id",
           Integer),  # not currently being used, but for possible future use
...
model.PostJobActionAssociation.table = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column(
        "job_id", Integer, ForeignKey("job.id"), index=True, nullable=False),
    Column(
        "post_job_action_id",
        Integer,
...
model.GalaxySession.table = Table(
    ...
    Column("referer", TEXT),
    Column(
        "current_history_id", Integer, ForeignKey("history.id"),
        nullable=True),
...
model.WorkflowRequestInputParameter.table = Table(
    ...
    Column("value", TEXT), Column("type", Unicode(255)))
...
model.WorkflowStepConnection.table = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column(
        "output_step_id", Integer, ForeignKey("workflow_step.id"), index=True),
    Column(
        "input_step_id", Integer, ForeignKey("workflow_step.id"), index=True),
...
model.RequestTypeExternalServiceAssociation.table = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column(
        "request_type_id", Integer, ForeignKey("request_type.id"), index=True),
    Column(
        "external_service_id",
        Integer,
...
model.FormValues.table = Table("form_values", metadata,
                               ...
                               Column(
                                   ...
                                   Integer,
                                   ForeignKey("form_definition.id"),
                                   index=True), Column("content", JSONType()))
...
model.RequestTypeRunAssociation.table = Table(
    ...
    Column(
        ...
        nullable=False),
    Column(
        "run_id", Integer, ForeignKey("run.id"), index=True, nullable=False))
...
model.VisualizationRevision.table = Table(
    ...
    Column(
        ...
        nullable=False),
    Column("title", TEXT),
    Column("dbkey", TEXT, index=True), Column("config", JSONType))
...
model.HistoryRatingAssociation.table = Table(
    "history_rating_association", metadata,
    Column("id", Integer, primary_key=True),
    Column("history_id", Integer, ForeignKey("history.id"), index=True),
    Column("user_id", Integer, ForeignKey("galaxy_user.id"), index=True),
...
mapper(
    ...
    properties=dict(
        ...
        values=relation(
            model.FormValues,
            primaryjoin=(model.Sample.table.c.form_values_id ==
                         model.FormValues.table.c.id)),
        request=relation(
            model.Request,
            primaryjoin=(
                model.Sample.table.c.request_id == model.Request.table.c.id)),
        folder=relation(
            model.LibraryFolder,
            primaryjoin=(model.Sample.table.c.folder_id ==
                         model.LibraryFolder.table.c.id)),
        library=relation(
            model.Library,
            primaryjoin=(
                model.Sample.table.c.library_id == model.Library.table.c.id)),
        history=relation(
            model.History,
            primaryjoin=(
                model.Sample.table.c.history_id == model.History.table.c.id)),
    ))
...
mapper(
    ...
    properties=dict(
        values=relation(
            model.FormValues,
            primaryjoin=(model.Request.table.c.form_values_id ==
                         model.FormValues.table.c.id)),
        type=relation(
            model.RequestType,
            primaryjoin=(model.Request.table.c.request_type_id ==
                         model.RequestType.table.c.id)),
        user=relation(
            model.User,
            primaryjoin=(
                model.Request.table.c.user_id == model.User.table.c.id),
            backref="requests"),
        samples=relation(
            model.Sample,
            primaryjoin=(
                model.Request.table.c.id == model.Sample.table.c.request_id),
...
mapper(
    ...
    properties=dict(
        states=relation(
            ...
            primaryjoin=(model.RequestType.table.c.id ==
                         model.SampleState.table.c.request_type_id),
            order_by=asc(model.SampleState.table.c.update_time)),
        request_form=relation(
            model.FormDefinition,
            primaryjoin=(model.RequestType.table.c.request_form_id ==
                         model.FormDefinition.table.c.id)),
        sample_form=relation(
            model.FormDefinition,
            primaryjoin=(model.RequestType.table.c.sample_form_id ==
                         model.FormDefinition.table.c.id)), ))
...
mapper(
    ...
    properties=dict(
        request_type=relation(
            model.RequestType,
            primaryjoin=((model.RequestTypeExternalServiceAssociation.table.c.
                          request_type_id == model.RequestType.table.c.id)),
            backref="external_service_associations"),
        external_service=relation(
            model.ExternalService,
            primaryjoin=(
                model.RequestTypeExternalServiceAssociation.table.c.
...
mapper(
    ...
    properties=dict(current=relation(
        ...
        primaryjoin=(model.FormDefinition.table.c.form_definition_current_id ==
...
mapper(
    ...
    properties=dict(
        forms=relation(
            ...
            primaryjoin=(model.FormDefinitionCurrent.table.c.id == model.
                         FormDefinition.table.c.form_definition_current_id)),
        latest_form=relation(
            model.FormDefinition,
            post_update=True,
            primaryjoin=(model.FormDefinitionCurrent.table.c.latest_form_id ==
                         model.FormDefinition.table.c.id))))
...
mapper(
    ...
    properties=dict(
        state=relation(
            model.SampleState,
            primaryjoin=(model.SampleEvent.table.c.sample_state_id ==
                         model.SampleState.table.c.id)), ))
...
mapper(
    ...
    properties=dict(external_service=relation(
        model.ExternalService,
        primaryjoin=(model.SampleDataset.table.c.external_service_id ==
...
mapper(
    ...
    properties=dict(
        sample=relation(
            model.Sample,
            backref="runs",
            order_by=desc(model.Run.table.c.update_time)),
        run=relation(
            model.Run,
            backref="samples",
            order_by=asc(model.Sample.table.c.id))))
...
mapper(
    ...
    properties=dict(
        template=relation(
            model.FormDefinition,
            primaryjoin=(model.Run.table.c.form_definition_id ==
                         model.FormDefinition.table.c.id)),
        info=relation(
            model.FormValues,
            primaryjoin=(model.Run.table.c.form_values_id ==
                         model.FormValues.table.c.id))))
...
mapper(
    ...
    properties=dict(
        user=relation(
            model.User,
            primaryjoin=(
                model.UserAddress.table.c.user_id == model.User.table.c.id),
            backref='addresses',
...
mapper(
    ...
    properties=dict(
        session=relation(
            model.GalaxySession,
            primaryjoin=(model.UserOpenID.table.c.session_id ==
        ...
        user=relation(
            model.User,
            primaryjoin=(
                model.UserOpenID.table.c.user_id == model.User.table.c.id),
            backref='openids',
            order_by=desc(model.UserOpenID.table.c.update_time))))
...
simple_mapping(
    ...
    dataset=relation(
        model.Dataset,
        primaryjoin=(model.Dataset.table.c.id ==
    ...
    copied_from_history_dataset_association=relation(
        model.HistoryDatasetAssociation,
        primaryjoin=(model.HistoryDatasetAssociation.table.c.
                     copied_from_history_dataset_association_id ==
    ...
    visible_children=relation(
        model.HistoryDatasetAssociation,
        primaryjoin=(
            (model.HistoryDatasetAssociation.table.c.parent_id ==
             model.HistoryDatasetAssociation.table.c.id) &
    ...
    annotations=relation(
        ...
        order_by=model.HistoryDatasetAssociationAnnotationAssociation.table.c.
        id,
        backref="hdas"),
    ratings=relation(
...
simple_mapping(
    ...
    library_associations=relation(
        model.LibraryDatasetDatasetAssociation,
        primaryjoin=(model.Dataset.table.c.id == model.
...
mapper(
    ...
    properties=dict(
        hda=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(model.HistoryDatasetAssociationSubset.table.c.
                         history_dataset_association_id ==
                         model.HistoryDatasetAssociation.table.c.id)),
        subset=relation(
            model.HistoryDatasetAssociation,
...
mapper(
    ...
    properties=dict(
        parent_hda=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(
                model.ImplicitlyConvertedDatasetAssociation.table.c.
                hda_parent_id == model.HistoryDatasetAssociation.table.c.id)),
        parent_ldda=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(model.ImplicitlyConvertedDatasetAssociation.table.c.
                         ldda_parent_id ==
                         model.LibraryDatasetDatasetAssociation.table.c.id)),
...
mapper(
    ...
    properties=dict(
        ...
        exports=relation(
            ...
            primaryjoin=(model.JobExportHistoryArchive.table.c.history_id ==
        ...
        active_dataset_collections=relation(
            ...
            order_by=asc(
                model.HistoryDatasetCollectionAssociation.table.c.hid),
            viewonly=True),
        visible_datasets=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=((model.HistoryDatasetAssociation.table.c.history_id ==
                          model.History.table.c.id) &
                         not_(model.HistoryDatasetAssociation.table.c.deleted)
...
mapper(
    ...
    properties=dict(
        ...
        values=relation(
            model.FormValues,
            primaryjoin=(model.User.table.c.form_values_id ==
                         model.FormValues.table.c.id)),
        api_keys=relation(
            model.APIKeys,
            backref="user",
        ...
        non_private_roles=relation(
            ...
            primaryjoin=(
                (model.User.table.c.id ==
                 model.UserRoleAssociation.table.c.user_id) &
                (model.UserRoleAssociation.table.c.role_id ==
                 model.Role.table.c.id) & not_(
...
mapper(
    model.Library,
    model.Library.table,
    properties=dict(root_folder=relation(
        model.LibraryFolder, backref=backref("library_root"))))
...
mapper(
    ...
    properties=dict(children=relation(
        ...
        primaryjoin=(model.ExtendedMetadataIndex.table.c.extended_metadata_id
                     == model.ExtendedMetadata.table.c.id),
        backref=backref(
            "parent",
            primaryjoin=(
...
mapper(
    ...
    properties=dict(
        library=relation(
            model.Library,
            primaryjoin=((model.LibraryInfoAssociation.table.c.library_id ==
        ...
        info=relation(
            model.FormValues,
            primaryjoin=(model.LibraryInfoAssociation.table.c.form_values_id ==
                         model.FormValues.table.c.id))))
...
mapper(
    ...
    properties=dict(
        folders=relation(
            model.LibraryFolder,
            primaryjoin=(model.LibraryFolder.table.c.parent_id ==
                         model.LibraryFolder.table.c.id),
        ...
        datasets=relation(
            ...
            primaryjoin=((model.LibraryDataset.table.c.folder_id ==
                          model.LibraryFolder.table.c.id)),
...
mapper(
    ...
    properties=dict(
        ...
        info=relation(
            model.FormValues,
            primaryjoin=(model.LibraryFolderInfoAssociation.table.c.
...
mapper(
    ...
    properties=dict(
        ...
        library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(model.LibraryDataset.table.c.
                         library_dataset_dataset_association_id ==
                         model.LibraryDatasetDatasetAssociation.table.c.id)),
        expired_datasets=relation(
            model.LibraryDatasetDatasetAssociation,
            foreign_keys=[
                model.LibraryDataset.table.c.id, model.LibraryDataset.table.c.
                library_dataset_dataset_association_id
            ],
            primaryjoin=(
                (model.LibraryDataset.table.c.id == model.
                 LibraryDatasetDatasetAssociation.table.c.library_dataset_id) &
                (not_(model.LibraryDataset.table.c.
                      library_dataset_dataset_association_id ==
                      model.LibraryDatasetDatasetAssociation.table.c.id))),
            viewonly=True,
            uselist=True)))
...
mapper(
    ...
    properties=dict(
        ...
        library_dataset=relation(
            model.LibraryDataset,
            primaryjoin=(
                model.LibraryDatasetDatasetAssociation.table.c.
                library_dataset_id == model.LibraryDataset.table.c.id)),
        # user=relation( model.User.mapper ),
        user=relation(model.User),
        copied_from_library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(model.LibraryDatasetDatasetAssociation.table.c.
                         copied_from_library_dataset_dataset_association_id ==
                         model.LibraryDatasetDatasetAssociation.table.c.id),
        ...
        children=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(
                model.LibraryDatasetDatasetAssociation.table.c.parent_id ==
                model.LibraryDatasetDatasetAssociation.table.c.id),
            backref=backref(
                "parent",
                primaryjoin=(
        ...
        visible_children=relation(
            ...
            primaryjoin=(
                (model.LibraryDatasetDatasetAssociation.table.c.parent_id ==
                 model.LibraryDatasetDatasetAssociation.table.c.id) &
                (model.LibraryDatasetDatasetAssociation.table.c.visible ==
                 true())),
            remote_side=[model.LibraryDatasetDatasetAssociation.table.c.id]),
        tags=relation(
            model.LibraryDatasetDatasetAssociationTagAssociation,
            order_by=model.LibraryDatasetDatasetAssociationTagAssociation.
            table.c.id,
            backref='history_tag_associations'),
        extended_metadata=relation(
            model.ExtendedMetadata,
        ...
        _metadata=deferred(
            model.LibraryDatasetDatasetAssociation.table.c._metadata)))
...
mapper(
    ...
    properties=dict(
        library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=((model.LibraryDatasetDatasetInfoAssociation.table.c.
                          library_dataset_dataset_association_id ==
                          model.LibraryDatasetDatasetAssociation.table.c.id) &
                         (not_(model.LibraryDatasetDatasetInfoAssociation.
                               table.c.deleted))),
            backref="info_association"),
        template=relation(
            model.FormDefinition,
            primaryjoin=(
                model.LibraryDatasetDatasetInfoAssociation.table.c.
...
simple_mapping(
    model.JobStateHistory, job=relation(model.Job, backref="state_history"))

simple_mapping(
    model.JobMetricText, job=relation(model.Job, backref="text_metrics"))

simple_mapping(
    model.TaskMetricText, task=relation(model.Task, backref="text_metrics"))
...
simple_mapping(
    ...
    input_dataset_collection=relation(
        ...
        primaryjoin=((model.HistoryDatasetCollectionAssociation.table.c.id ==
                      model.ImplicitlyCreatedDatasetCollectionInput.table.c.
                      input_dataset_collection_id)),
...
mapper(
    ...
    properties=dict(workflow_step=relation(
        ...
        primaryjoin=(model.WorkflowStep.table.c.id ==
...
mapper(
    ...
    properties=dict(
        ...
        output_datasets=relation(
            model.JobToOutputDatasetAssociation, lazy=True),
        ...
        input_library_datasets=relation(
            model.JobToInputLibraryDatasetAssociation),
        output_library_datasets=relation(
            model.JobToOutputLibraryDatasetAssociation, lazy=True),
        external_output_metadata=relation(
            model.JobExternalOutputMetadata, lazy=True),
...
simple_mapping(
    ...
    elements=relation(
        ...
        primaryjoin=(model.DatasetCollection.table.c.id == model.
                     DatasetCollectionElement.table.c.dataset_collection_id),
        remote_side=[
...
simple_mapping(
    ...
    copied_from_history_dataset_collection_association=relation(
        model.HistoryDatasetCollectionAssociation,
        primaryjoin=(model.HistoryDatasetCollectionAssociation.table.c.
                     copied_from_history_dataset_collection_association_id ==
                     model.HistoryDatasetCollectionAssociation.table.c.id),
    ...
    annotations=relation(
        ...
        order_by=model.HistoryDatasetCollectionAnnotationAssociation.table.c.
        id,
        backref="dataset_collections"),
    ratings=relation(
...
simple_mapping(
    ...
    annotations=relation(
        ...
        order_by=model.LibraryDatasetCollectionAnnotationAssociation.table.c.
        id,
        backref="dataset_collections"),
    ratings=relation(
...
simple_mapping(
    ...
    hda=relation(
        model.HistoryDatasetAssociation,
        primaryjoin=(model.DatasetCollectionElement.table.c.hda_id ==
                     model.HistoryDatasetAssociation.table.c.id)),
    ldda=relation(
        model.LibraryDatasetDatasetAssociation,
        primaryjoin=(model.DatasetCollectionElement.table.c.ldda_id ==
                     model.LibraryDatasetDatasetAssociation.table.c.id)),
    child_collection=relation(
        model.DatasetCollection,
        primaryjoin=(model.DatasetCollectionElement.table.c.child_collection_id
...
mapper(
    ...
    properties=dict(steps=relation(
        model.WorkflowStep,
        backref='workflow',
        primaryjoin=((model.Workflow.table.c.id ==
                      model.WorkflowStep.table.c.workflow_id)),
...
mapper(
    ...
    properties=dict(
        subworkflow=relation(
            model.Workflow,
            primaryjoin=((model.Workflow.table.c.id ==
                          model.WorkflowStep.table.c.subworkflow_id)),
...
mapper(
    ...
    properties=dict(
        input_step=relation(
            ...
            cascade="all",
            primaryjoin=(model.WorkflowStepConnection.table.c.input_step_id ==
...
mapper(
    ...
    properties=dict(
        user=relation(
            model.User,
            primaryjoin=(
                model.User.table.c.id == model.StoredWorkflow.table.c.user_id),
            backref='stored_workflows'),
        workflows=relation(
            model.Workflow,
            backref='stored_workflow',
            cascade="all, delete-orphan",
            primaryjoin=(model.StoredWorkflow.table.c.id ==
                         model.Workflow.table.c.stored_workflow_id)),
        latest_workflow=relation(
            model.Workflow,
            post_update=True,
            primaryjoin=(model.StoredWorkflow.table.c.latest_workflow_id ==
...
mapper(
    ...
    properties=dict(
        ...
        stored_workflow=relation(
            model.StoredWorkflow, backref='users_shared_with')))
...
mapper(
    ...
    properties=dict(
        history=relation(
            model.History,
            backref=backref('workflow_invocations', uselist=True)),
...
mapper(
    ...
    properties=dict(
        ...
        revisions=relation(
            ...
            cascade="all, delete-orphan",
            primaryjoin=(
                model.Page.table.c.id == model.PageRevision.table.c.page_id)),
        latest_revision=relation(
            model.PageRevision,
            post_update=True,
            primaryjoin=(model.Page.table.c.latest_revision_id ==
                         model.PageRevision.table.c.id),
            lazy=False),
        tags=relation(
...
model.Page.users_shared_with_dot_users = association_proxy(
    'users_shared_with', 'user')
...
mapper(
    ...
    properties=dict(
        ...
        revisions=relation(
            ...
            cascade="all, delete-orphan",
            primaryjoin=(model.Visualization.table.c.id == model.
                         VisualizationRevision.table.c.visualization_id)),
        latest_revision=relation(
            model.VisualizationRevision,
            post_update=True,
            primaryjoin=(model.Visualization.table.c.latest_revision_id ==
...
mapper(
    ...
    properties=dict(
        ...
        visualization=relation(
            model.Visualization, backref='users_shared_with')))
...
simple_mapping(
    ...
    children=relation(
        model.Tag,
        backref=backref('parent', remote_side=[model.Tag.table.c.id])))
...
tag_mapping(model.HistoryDatasetAssociationTagAssociation,
            "tagged_history_dataset_associations")
tag_mapping(model.LibraryDatasetDatasetAssociationTagAssociation,
...
tag_mapping(model.HistoryDatasetCollectionTagAssociation,
            "tagged_history_dataset_collections")
tag_mapping(model.LibraryDatasetCollectionTagAssociation,
...
rating_mapping(
    model.VisualizationRatingAssociation, visualizaiton=model.Visualization)
rating_mapping(
    model.HistoryDatasetCollectionRatingAssociation,
    history_dataset_collection=model.HistoryDatasetCollectionAssociation)
rating_mapping(
...
def db_next_hid(self, n=1):
    ...
    try:
        next_hid = select(
            [table.c.hid_counter], table.c.id == self.id,
...
def _workflow_invocation_update(self):
    ...
    table = self.table
    now_val = now()
    stmt = table.update().values(update_time=now_val).where(
        and_(table.c.id == self.id, table.c.update_time < now_val))
    conn.execute(stmt)
...
def init(file_path,
    ...
    engine = build_engine(url, engine_options, database_query_profiling_proxy,
                          trace_logger, slow_query_log_threshold)
