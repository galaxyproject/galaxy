...
PATH_DEFAULTS = dict(
    ...
    build_sites_config_file=[
        'config/build_sites.yml', 'config/build_sites.yml.sample'
    ],
    ...
    tool_destinations_config_file=[
        'config/tool_destinations.yml', 'config/tool_destinations.yml.sample'
    ],
    job_metrics_config_file=[
        'config/job_metrics_conf.xml', 'job_metrics_conf.xml',
        'config/job_metrics_conf.xml.sample'
    ],
    error_report_file=[
        'config/error_report.yml', 'config/error_report.yml.sample'
    ],
    dependency_resolvers_config_file=[
        'config/dependency_resolvers_conf.xml', 'dependency_resolvers_conf.xml'
    ],
    job_resource_params_file=[
        'config/job_resource_params_conf.xml', 'job_resource_params_conf.xml'
    ],
    migrated_tools_config=[
        'migrated_tools_conf.xml', 'config/migrated_tools_conf.xml'
    ],
    object_store_config_file=[
        'config/object_store_conf.xml', 'object_store_conf.xml'
    ],
    openid_config_file=[
        'config/openid_conf.xml', 'openid_conf.xml',
        'config/openid_conf.xml.sample'
    ],
    shed_data_manager_config_file=[
        'shed_data_manager_conf.xml', 'config/shed_data_manager_conf.xml'
    ],
    shed_tool_data_table_config=[
        'shed_tool_data_table_conf.xml', 'config/shed_tool_data_table_conf.xml'
    ],
    tool_sheds_config_file=[
        'config/tool_sheds_conf.xml', 'tool_sheds_conf.xml',
    ...
    user_preferences_extra_config_file=[
        'config/user_preferences_extra_conf.yml'
    ],
    containers_config_file=['config/containers_conf.yml'], )

PATH_LIST_DEFAULTS = dict(
    tool_data_table_config_path=[
        'config/tool_data_table_conf.xml', 'tool_data_table_conf.xml',
...
class Configuration(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.slow_query_log_threshold = float(
            kwargs.get("slow_query_log_threshold", 0))
        ...
        override_tempdir = string_as_bool(
            kwargs.get("override_tempdir", "True"))
        if override_tempdir:
        ...
        self.toolbox_filter_base_modules = listify(
            kwargs.get("toolbox_filter_base_modules",
                       "galaxy.tools.filters,galaxy.tools.toolbox.filters"))
        self.tool_filters = listify(
            kwargs.get("tool_filters", []), do_strip=True)
        ...
        self.has_user_tool_filters = bool(self.user_tool_filters
        ...
        self.use_remote_user = string_as_bool(
            kwargs.get("use_remote_user", "False")) or self.single_user
        ...
        self.collect_outputs_from = [
            x.strip()
            for x in kwargs.get('collect_outputs_from',
                                'new_file_path,job_working_directory').lower()
            .split(',')
        ]
        ...
        default_jobs_directory = kwargs.get("job_working_directory",
        ...
        preserve_python_environment = kwargs.get("preserve_python_environment",
                                                 "legacy_only")
        if preserve_python_environment not in [
                "legacy_only", "legacy_and_local", "always"
        ]:
            log.warn(
        ...
        self.container_image_cache_path = self.resolve_path(
            kwargs.get("container_image_cache_path",
                       "database/container_images"))
        if not os.path.exists(self.container_image_cache_path):
            self.container_image_cache_path = self.resolve_path(
                kwargs.get("container_image_cache_path",
                           "database/container_cache"))
        ...
        self.activation_grace_period = int(
            kwargs.get('activation_grace_period', 3))
        ...
        self.enable_openid = string_as_bool(
            kwargs.get('enable_openid', 'False'))
        self.enable_quotas = string_as_bool(
            kwargs.get('enable_quotas', 'False'))
        ...
        self.tool_submission_burst_threads = int(
            kwargs.get('tool_submission_burst_threads', '1'))
        self.tool_submission_burst_at = int(
            kwargs.get('tool_submission_burst_at', '10'))
        ...
        self.force_beta_workflow_scheduled_for_collections = string_as_bool(
            kwargs.get('force_beta_workflow_scheduled_for_collections',
                       'False'))
        ...
        self.real_system_username = kwargs.get('real_system_username',
                                               'user_email')
        ...
        self.sanitize_whitelist_file = resolve_path(
            kwargs.get('sanitize_whitelist_file',
                       "config/sanitize_whitelist.txt"), self.root)
        ...
        self.ftp_upload_purge = string_as_bool(
            kwargs.get('ftp_upload_purge', 'True'))
        self.ftp_upload_site = kwargs.get('ftp_upload_site', None)
        self.allow_path_paste = string_as_bool(
            kwargs.get('allow_path_paste', False))
        ...
        self.index_tool_help = string_as_bool(
            kwargs.get("index_tool_help", True))
        ...
        self.tool_enable_ngram_search = kwargs.get("tool_enable_ngram_search",
                                                   False)
        ...
        if "conda_mapping_files" in kwargs:
            ...
            self.conda_mapping_files = [
                ...
                os.path.join(self.root, "lib", "galaxy", "tools", "deps",
                             "resolvers", "default_conda_mapping.yml"),
            ]
        ...
        if "HOST_IP" in galaxy_infrastructure_url:
            galaxy_infrastructure_url = string.Template(
                galaxy_infrastructure_url).safe_substitute({
                    'HOST_IP':
        ...
        self.default_job_handlers = [
            x.strip()
            for x in kwargs.get('default_job_handlers', ','.join(
        ...
        self.gie_dirs = [
            d.strip() for d in (ie_dirs.split(",") if ie_dirs else [])
        ]
        ...
        self.proxy_session_map = self.resolve_path(
            kwargs.get("dynamic_proxy_session_map",
                       "database/session_map.sqlite"))
        self.manage_dynamic_proxy = string_as_bool(
            kwargs.get("dynamic_proxy_manage",
                       "True"))  # Set to false if being launched externally
        ...
        self.dynamic_proxy_prefix = kwargs.get("dynamic_proxy_prefix",
                                               "gie_proxy")
        ...
        self.containers_conf = parse_containers_config(
    ...
    def check(self):
        paths_to_check = [
            self.root, self.tool_path, self.tool_data_path, self.template_path
        ]
        ...
        for path in (self.new_file_path, self.template_cache,
                     self.ftp_upload_dir, self.library_import_dir,
                     self.user_library_import_dir, self.nginx_upload_store,
        ...
        for path in tool_configs:
            if not os.path.exists(path):
                raise ConfigurationError(
                    "Tool config file not found: %s" % path)
    ...
    def is_admin_user(self, user):
        ...
        """
        admin_users = [
            x.strip() for x in self.get("admin_users", "").split(",")
        ]
    ...
    def _parse_allowed_origin_hostnames(self, kwargs):
        ...
        """
        allowed_origin_hostnames = listify(
            kwargs.get('allowed_origin_hostnames', None))
...
def parse_dependency_options(kwargs, root, dependency_resolvers_config_file):
    # Location for tool dependencies.
    tool_dependency_dir = kwargs.get("tool_dependency_dir",
    ...
    if tool_dependency_dir is not None:
        ...
        tool_dependency_cache_dir = kwargs.get('tool_dependency_cache_dir',
                                               os.path.join(
                                                   tool_dependency_dir,
                                                   '_cache'))
        ...
        tool_dependency_dir = None
        use_tool_dependencies = os.path.exists(
            dependency_resolvers_config_file)
...
def configure_logging(config):
    ...
    if parser:
        paste_configures_logging = config.global_conf_parser.has_section(
            "loggers")
    else:
    ...
    if auto_configure_logging:
        ...
        disable_chatty_loggers = string_as_bool(
            config.get("auto_configure_logging_disable_chatty", "True"))
        if disable_chatty_loggers:
            # Turn down paste httpserver logging
            if level <= logging.DEBUG:
                for chatty_logger in [
                        "paste.httpserver.ThreadPool", "routes.middleware"
                ]:
...
class ConfiguresGalaxyMixin:
    ...
    def _configure_toolbox(self):
        ...
        app_info = containers.AppInfo(
            ...
            enable_beta_mulled_containers=self.config.
            enable_beta_mulled_containers,
            containers_resolvers_config_file=self.config.
