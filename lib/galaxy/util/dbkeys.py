...
class GenomeBuilds(object):
    ...
    def __init__(self, app, data_table_name="__dbkeys__", load_old_style=True):
        ...
        if load_old_style:
            self._static_dbkeys = list(
                read_dbnames(app.config.builds_file_path))
        else:
            self._static_dbkeys = []
        ...
        if trans is not None:
            if trans.history:
                ...
                for dataset in datasets:
                    rval.append((dataset.dbkey, "%s (%s) [History]" %
                                 (dataset.name, dataset.dbkey)))
            user = trans.user
            if user and hasattr(
                ...
                for key, chrom_dict in user_keys.items():
                    rval.append((key, "%s (%s) [Custom]" % (chrom_dict['name'],
                                                            key)))
