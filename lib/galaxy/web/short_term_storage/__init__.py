...
class ShortTermStorageConfiguration:
    ...
    default_storage_duration: Optional[int] = None
    maximum_storage_duration: Optional[int] = None
...
class ShortTermStorageAllocator(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def new_target(self, filename: str, mime_type: str, duration: Optional[int] = None, security: Optional[ShortTermStorageTargetSecurity] = None) -> ShortTermStorageTarget:
...
class ShortTermStorageManager(ShortTermStorageAllocator, ShortTermStorageMonitor):
    ...
    def new_target(self, filename: str, mime_type: str, duration: Optional[int] = None, security: Optional[ShortTermStorageTargetSecurity] = None) -> ShortTermStorageTarget:
        ...
        target = ShortTermStorageTarget(
            ...
            raw_path=str(target_directory / "target")
        )
    ...
    def cancel(self, target: ShortTermStorageTarget, exception: Optional[MessageException] = None):
        ...
        if exception:
            exception_json = {"status_code": exception.status_code, "exception": api_error_message(None, exception=exception)}
        else:
