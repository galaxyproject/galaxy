...
class WorkflowsManager(object):
    ...
    def build_invocations_query(self, trans, decoded_stored_workflow_id):
        try:
            ...
            raise exceptions.ObjectNotFound()
        self.check_security(
            trans, stored_workflow, check_ownership=True, check_accessible=False)
...
CreatedWorkflow = namedtuple(
    "CreatedWorkflow", ["stored_workflow", "missing_tools"])
...
class WorkflowContentsManager(UsesAnnotations):
    ...
    def build_workflow_from_dict(
        ...
        for step_dict in self.__walk_step_dicts(data):
            module, step = self.__module_from_dict(
                trans, step_dict, secure=False)
            steps.append(step)
            steps_by_external_id[step_dict['id']] = step

            if module.type == 'tool' and module.tool is None:
                # A required tool is not available in the local Galaxy
                # instance.
                missing_tool_tup = (
                    step_dict['tool_id'], step_dict['name'], step_dict['tool_version'])
        ...
        if data['annotation']:
            annotation = sanitize_html(
                data['annotation'], 'utf-8', 'text/html')
    ...
    def update_workflow_from_dict(self, trans, stored_workflow, workflow_data, from_editor=False):
        ...
        for step_dict in self.__walk_step_dicts(data):
            module, step = self.__module_from_dict(
                trans, step_dict, secure=from_editor)
            ...
            if 'workflow_outputs' in step_dict:
                for output_name in step_dict['workflow_outputs']:
                    m = model.WorkflowOutput(
                        workflow_step=step, output_name=output_name)
        ...
        if workflow.has_errors:
            errors.append(
                "Some steps in this workflow have validation errors")
    ...
    def _workflow_to_dict_editor(self, trans, stored):
        ...
        for step in workflow.steps:
            ...
            if upgrade_message:
                ...
                data['upgrade_messages'][
                    step.order_index] = upgrade_message.values()
            # Get user annotation.
            step_annotation = self.get_item_annotation_obj(
                trans.sa_session, trans.user, step)
            annotation_str = ""
            ...
            if step.type is None or step.type == 'tool':
                ...
                # Filter
                # FIXME: this removes connection without displaying a message
                # currently!
                input_connections = [
                    conn for conn in input_connections if conn.input_name in data_input_names]
                # post_job_actions
                pja_dict = {}
            ...
            for conn in input_connections:
                ...
                conn_dict = dict(id=conn.output_step.order_index,
                                 output_name=conn.output_name, input_type=input_type)
    ...
    def _workflow_to_dict_export(self, trans, stored):
        ...
        for step in workflow.steps:
            ...
            if module.type == 'tool':
                ...
                step_dict['post_job_actions'] = pja_dict
            # Data inputs
            step_dict['inputs'] = module.get_runtime_input_dicts(
                annotation_str)
            # User outputs
            ...
            if step.type is None or step.type == 'tool':
                ...
                def callback(input, value, prefixed_name, prefixed_label):
                    if isinstance(input, DataToolParameter) or isinstance(input, DataCollectionToolParameter):
                        data_input_names[prefixed_name] = True
                # FIXME: this updates modules silently right now; messages from
                ...
                # FIXME: this removes connection without displaying a message
                # currently!
                input_connections = [conn for conn in input_connections if (
                    conn.input_name in data_input_names or conn.non_data_connection)]
            ...
            for input_name in unique_input_names:
                input_conn_dict[ input_name ] = \
                    [dict(id=conn.output_step.order_index, output_name=conn.output_name)
                     for conn in input_connections if conn.input_name == input_name]
