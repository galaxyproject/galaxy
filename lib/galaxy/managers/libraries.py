...
class LibraryManager(object):
    ...
    def get(self, trans, decoded_library_id, check_accessible=True):
        ...
        try:
            library = trans.sa_session.query(trans.app.model.Library).filter(
                trans.app.model.Library.table.c.id == decoded_library_id).one()
    ...
    def check_accessible(self, trans, library):
        ...
        if not trans.app.security_agent.can_access_library(trans.get_current_user_roles(), library):
            ...
            raise exceptions.ObjectNotFound(
                'Library with the id provided is deleted.')
        else:
            return library
    ...
    def get_library_dict(self, trans, library):
        ...
        :rtype:     dictionary
        """
        library_dict = library.to_dict(view='element', value_mapper={
                                       'id': trans.security.encode_id, 'root_folder_id': trans.security.encode_id})
    ...
    def get_current_roles(self, trans, library):
        ...
        """
        access_library_role_list = [(access_role.name, trans.security.encode_id(access_role.id))
                                    for access_role in self.get_access_roles(trans, library)]
        modify_library_role_list = [(modify_role.name, trans.security.encode_id(modify_role.id))
                                    for modify_role in self.get_modify_roles(trans, library)]
        manage_library_role_list = [(manage_role.name, trans.security.encode_id(manage_role.id))
