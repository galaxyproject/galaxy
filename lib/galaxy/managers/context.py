...
class ProvidesAppContext(object):
    ...
    def log_action(self, user=None, action=None, context=None, params=None):
        ...
        if self.app.config.log_actions:
            action = self.app.model.UserAction(
                action=action, context=context, params=unicode(dumps(params)))
            try:
                if user:
                    action.user = user
                else:
    ...
    def expunge_all(self):
        ...
        context.expunge_all()
        # This is a bit hacky, should refctor this. Maybe refactor to app ->
        # expunge_all()
...
class ProvidesUserContext(object):
    ...
    def user_can_do_run_as(self):
        run_as_users = [user for user in self.app.config.get(
            "api_allow_run_as", "").split(",") if user]
        if not run_as_users:
            return False
