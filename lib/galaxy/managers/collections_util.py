    ...
    for element_identifier in element_identifiers:
        if "__object__" in element_identifier:
            message = ERROR_MESSAGE_INVALID_PARAMETER_FOUND % (
                "__model_object__", element_identifier)
...
def dictify_dataset_collection_instance(dataset_collection_instance, parent, security, view="element"):
    ...
    security.encode_all_ids(dict_value, recursive=True)
                            # TODO: Use Kyle's recusrive formulation of this.
    return dict_value
...
def dictify_element(element):
    ...
    if element.child_collection:
        ...
        object_detials["elements"] = map(
            dictify_element, child_collection.elements)
