...
class TagManager(object):
    ...
    def set_tags_from_list(self, user, item, new_tags_list):
        ...
        self.apply_item_tags(user, item, unicode(
            new_tags_str.encode('utf-8'), 'utf-8'))
    ...
    def get_community_tags(self, item=None, limit=None):
        ...
        cols_to_select = [
            item_tag_assoc_class.table.c.tag_id, func.count('*')]
        from_obj = item_tag_assoc_class.table.join(
            item_class.table).join(self.app.model.Tag.table)
        where_clause = (
    ...
    def get_tool_tags(self):
        result_set = self.app.model.context.execute(
            select(columns=[self.app.model.ToolTagAssociation.table.c.tag_id],
    ...
    def apply_item_tag(self, user, item, name, value=None):
        ...
        # Apply attributes to item-tag association. Strip whitespace from user
        # name and tag.
        lc_value = None
        if value:
    ...
    def _create_tag(self, tag_str):
        ...
        for sub_tag in tag_hierarchy:
            ...
            tag = self.app.model.context.query(
                self.app.model.Tag).filter_by(name=tag_name).first()
            if not tag:
...
class GalaxyTagManager(TagManager):
    ...
    def __init__(self, app):
        ...
        self.item_tag_assoc_info[
            "StoredWorkflow"] = ItemTagAssocInfo(model.StoredWorkflow,
                                                 model.StoredWorkflowTagAssociation,
                                                 model.StoredWorkflowTagAssociation.table.c.stored_workflow_id)
        self.item_tag_assoc_info[
            "Visualization"] = ItemTagAssocInfo(model.Visualization,
