...
class ToBasicMarkdownDirectiveHandler(GalaxyInternalMarkdownDirectiveHandler):
    ...
    def handle_history_link(self, line, history):
        if history:
            content = MarkdownFormatHelpers.literal_via_fence(history.name)
        else:
    ...
    def handle_dataset_peek(self, line, hda):
        if hda.peek:
            content = MarkdownFormatHelpers.literal_via_fence(hda.peek)
        else:
    ...
    def handle_dataset_info(self, line, hda):
        if hda.info:
            content = MarkdownFormatHelpers.literal_via_fence(hda.info)
        else:
...
"""
    ...
    def handle_generate_galaxy_version(self, line, generate_version):
        if generate_version:
            content = MarkdownFormatHelpers.literal_via_fence(generate_version)
        else:
            content = "*No Galaxy Version Available*"
        return (content, True)

    def handle_generate_time(self, line, generate_time):
        content = MarkdownFormatHelpers.literal_via_fence(generate_time.isoformat())
        return (content, True)

    def handle_invocation_time(self, line, invocation):
        content = MarkdownFormatHelpers.literal_via_fence(invocation.create_time.isoformat())
        return (content, True)

    def handle_dataset_name(self, line, hda):
        if hda.name:
            content = MarkdownFormatHelpers.literal_via_fence(hda.name)
        else:
    ...
    def handle_dataset_type(self, line, hda):
        if hda.ext:
            content = MarkdownFormatHelpers.literal_via_fence(hda.ext)
        else:
