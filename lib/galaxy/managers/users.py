...
class UserManager(base.ModelManager, deletable.PurgableManagerMixin):
    ...
    def create(self, webapp_name=None, **kwargs):
        ...
        if webapp_name == 'galaxy':
            # We set default user permissions, before we log in and set the
            # default history permissions
    ...
    def by_email(self, email, filters=None, **kwargs):
        ...
        filters = self._munge_filters(
            self.model_class.email == email, filters)
        try:
    ...
    def error_if_anonymous(self, user, msg="Log-in required", **kwargs):
        ...
        """
        if user is None:
            # TODO: code is correct (401) but should be named
            # AuthenticationRequired (401 and 403 are flipped)
    ...
    def tags_used(self, user, tag_models=None):
        ...
        if not tag_models:
            tag_models = [
                v.tag_assoc_class for v in self.app.tag_handler.item_tag_assoc_info.values()]
        # create a union of subqueries for each for this user - getting only
        # the tname and user_value
        all_tags_query = None
        ...
        # boil the tag tuples down into a sorted list of DISTINCT name:val
        # strings
    ...
    def serialize_current_anonymous_user(self, user, keys, trans=None, **kwargs):
        ...
        if not history:
            raise exceptions.AuthenticationRequired(
                'No history for anonymous user usage stats')
