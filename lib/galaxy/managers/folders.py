...
class FolderManager(object):
    ...
    def get(self, trans, decoded_folder_id, check_manageable=False, check_accessible=True):
        ...
        folder = self.secure(
            trans, folder, check_manageable, check_accessible)
        return folder
    ...
    def check_manageable(self, trans, folder):
        ...
        """
        if not trans.user:
            raise galaxy.exceptions.AuthenticationRequired(
        ...
        if not trans.app.security_agent.can_manage_library_item(current_user_roles, folder):
            raise galaxy.exceptions.InsufficientPermissionsException(
    ...
    def get_current_roles(self, trans, folder):
        ...
        modify_roles = set(trans.app.security_agent.get_roles_for_action(
            folder, trans.app.security_agent.permitted_actions.LIBRARY_MODIFY))
        manage_roles = set(trans.app.security_agent.get_roles_for_action(
            folder, trans.app.security_agent.permitted_actions.LIBRARY_MANAGE))
        add_roles = set(trans.app.security_agent.get_roles_for_action(
            folder, trans.app.security_agent.permitted_actions.LIBRARY_ADD))

        modify_folder_role_list = [(modify_role.name, trans.security.encode_id(modify_role.id))
                                   for modify_role in modify_roles]
        manage_folder_role_list = [(manage_role.name, trans.security.encode_id(manage_role.id))
                                   for manage_role in manage_roles]
        add_library_item_role_list = [(add_role.name, trans.security.encode_id(add_role.id))
    ...
    def cut_the_prefix(self, encoded_folder_id):
        ...
        if ((len(encoded_folder_id) % 16 == 1) and encoded_folder_id.startswith('F')):
            ...
            raise galaxy.exceptions.MalformedId(
                'Malformed folder id ( %s ) specified, unable to decode.' % str(encoded_folder_id))
        return cut_id
