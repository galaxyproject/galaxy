...
def security_check(trans, item, check_ownership=False, check_accessible=False):
    ...
    # Verify ownership: there is a current user and that user is the same as
    # the item's
    if check_ownership:
        if not trans.user:
        ...
        if item.user != trans.user:
            raise exceptions.ItemOwnershipException(
                "%s is not owned by the current user" % item.__class__.__name__, type='error')

    # Verify accessible:
    #   if it's part of a lib - can they access via security
    # if it's something else (sharable) have they been added to the item's
...
class ModelManager(object):
    ...
    def _munge_filters(self, filtersA, filtersB):
        ...
        """
        # TODO: there's nothing specifically filter or model-related here -
        # move to util
        if filtersA is None:
            return filtersB
        if filtersB is None:
    ...
    def _one_with_recast_errors(self, query):
        ...
        try:
            ...
            raise exceptions.ObjectNotFound(
                self.model_class.__name__ + ' not found')
    ...
    def list(self, filters=None, order_by=None, limit=None, offset=None, **kwargs):
        ...
        if not fn_filters:
            # if no fn_filtering required, we can use the 'all orm' version
            # with limit offset
            return self._orm_list(filters=orm_filters, order_by=order_by, limit=limit, offset=offset, **kwargs)

        # fn filters will change the number of items returnable by limit/offset
        # - remove them here from the orm query
        query = self.query(
...
class ModelSerializer(object):
    ...
    def __init__(self, app):
        ...
        # a map of dictionary keys to the functions (often lambdas) that create
        # the values for those keys
        self.serializers = {}
        # add subclass serializers defined there
        self.add_serializers()
        # update the keyset by the serializers (removing the responsibility
        # from subclasses)
    ...
    def add_view(self, view_name, key_list, include_keys_from=None):
        ...
        """
        key_list = list(
            set(key_list + self.views.get(include_keys_from, [])))
    ...
    def serialize(self, item, keys, **context):
        ...
        """
        # TODO: constrain context to current_user/whos_asking when that's all
        # we need (trans)
        returned = {}
        for key in keys:
    ...
    # serializing to a view where a view is a predefied list of keys to
    # serialize
    def serialize_to_view(self, item, view=None, keys=None, default_view=None, **context):
        ...
        """
        # TODO: default view + view makes no sense outside the API.index
        # context - move default view there
        all_keys = []
        keys = keys or []
...
class ModelDeserializer(object):
    ...
    # TODO:?? a larger question is: which should be first? Deserialize then
    # validate - or - validate then deserialize?
    ...
    def __init__(self, app):
        ...
        # create a generically accessible manager for the model this
        # deserializer works with/for
        self.manager = None
    ...
    def default_deserializer(self, item, key, val, **context):
        ...
        """
        # TODO: sets the item attribute to value (this may not work in all
        # instances)
    ...
    def deserialize_basestring(self, item, key, val, convert_none_to_empty=False, **context):
        val = '' if (
            convert_none_to_empty and val is None) else self.validate.basestring(key, val)
...
class ModelFilterParser(object):
    ...
    def _add_parsers(self):
        ...
        self.orm_filter_parsers.update({
            ...
            # dates can be directly passed through the orm into a filter (no
            # need to parse into datetime object)
    ...
    def parse_filter(self, attr, op, val):
        ...
        try:
            ...
            # note: have to use explicit is None here, bool( sqlalx.filter ) ==
            # False
        ...
        # TODO: send back all valid filter keys in exception for added user
        # help
        raise exceptions.RequestParameterInvalidException(
    ...
    def _parse_fn_filter(self, attr, op, val):
        ...
        """
        # fn_filter_list is a dict: fn_filter_list[ attr ] = { 'opname1' :
        # opfn1, 'opname2' : opfn2, etc. }
        ...
        if not filter_fn:
            return None
        # parse the val from string using the 'val' parser if present
    ...
    def _parse_orm_filter(self, attr, op, val):
        ...
        """
        # orm_filter_list is a dict: orm_filter_list[ attr ] = <list of allowed
        # ops>
        ...
        # op must be whitelisted: contained in the list orm_filter_list[ attr
        # ][ 'op' ]
    ...
    # --- more parsers! yay!
    # TODO: These should go somewhere central - we've got ~6 parser
    # modules/sections now
    def parse_bool(self, bool_string):
        """
        Parse a boolean from a string.
        """
        # Be strict here to remove complexity of options (but allow already
        # parsed).
    ...
    def parse_id_list(self, id_list_string, sep=','):
        ...
        id_list = [self.app.security.decode_id(id_)
