...
class RunnerParams(ParamsWithSpecs):
    ...
    def _param_map_error(self, name, value):
        raise Exception(JOB_RUNNER_PARAMETER_MAP_PROBLEM_MESSAGE % (name,
                                                                    value))
...
class BaseJobRunner(object):
    DEFAULT_SPECS = dict(recheck_missing_job_retries=dict(
        map=int, valid=lambda x: x >= 0, default=0))
    ...
    def __init__(self, app, nworkers, **kwargs):
        ...
        self.runner_params = RunnerParams(
            specs=runner_param_specs, params=kwargs)
    ...
    def _init_worker_threads(self):
        ...
        for i in range(self.nworkers):
            worker = threading.Thread(
                name="%s.work_thread-%d" % (self.runner_name, i),
    ...
    def run_next(self):
        ...
        while True:
            ...
            try:
                ...
                log.exception("(%s) Unhandled exception calling %s" % (job_id,
                                                                       name))
    ...
    def put(self, job_wrapper):
        ...
        job_wrapper.set_job_destination(
            job_wrapper.job_destination, None, flush=False, job=job)
    ...
    def shutdown(self):
        ...
        """
        log.info("%s: Sending stop signal to %s worker threads" %
                 (self.runner_name, len(self.work_threads)))
    ...
    def prepare_job(self,
        ...
        if job_state == model.Job.states.DELETED:
            ...
            log.info("(%s) Job is in state %s, skipping execution" %
                     (job_id, job_state))
        ...
        try:
            ...
            job_wrapper.fail(
                e.message
                if hasattr(e, 'message') else "Job preparation failed",
                exception=True)
            return False
    ...
    def get_work_dir_outputs(self,
                             job_wrapper,
                             job_working_directory=None,
        ...
        if tool_working_directory is None:
            ...
            tool_working_directory = os.path.join(job_working_directory,
                                                  "working")
        ...
        for (joda, dataset) in self._walk_dataset_outputs(job):
            if joda and job_tool:
                ...
                if hda_tool_output and hda_tool_output.from_work_dir:
                    ...
                    source_file = os.path.join(tool_working_directory,
    ...
    def _handle_metadata_externally(self,
                                    job_wrapper,
                                    resolve_requirements=False):
        """
        ...
        if job_wrapper.get_state() not in [
            ...
            external_metadata_script = "%s %s %s" % (lib_adjust, venv,
            ...
            if resolve_requirements:
                ...
                if dependency_shell_commands:
                    ...
                    external_metadata_script = "%s&&%s" % (
                        dependency_shell_commands, external_metadata_script)
            ...
            log.debug('execution of external set_meta for job %d finished' %
                      job_wrapper.job_id)
    ...
    def get_job_file(self, job_wrapper, **kwds):
        ...
        options = dict(
            ...
            preserve_python_environment=job_wrapper.tool.
    ...
    def fail_job(self, job_state, exception=False):
        ...
        if not job_state.runner_state_handled:
            job_state.job_wrapper.fail(
                getattr(job_state, 'fail_message', 'Job failed'),
                exception=exception)
    ...
    def mark_as_resubmitted(self, job_state, info=None):
        ...
        if not self.app.config.track_jobs_in_database:
            ...
            self.app.job_manager.job_handler.dispatcher.put(
...
class JobState(object):
    ...
    def __init__(self, job_wrapper, job_destination):
        ...
        self.cleanup_file_attributes = [
            'job_file', 'output_file', 'error_file', 'exit_code_file'
        ]
    ...
    def set_defaults(self, files_dir):
        if self.job_wrapper is not None:
            ...
            self.job_name = ''.join(
                map(lambda x: x if x in (string.ascii_letters + string.digits + '_') else '_',
                    job_name))
    ...
    def cleanup(self):
        for file in [
                getattr(self, a) for a in self.cleanup_file_attributes
                if hasattr(self, a)
        ]:
            try:
                os.unlink(file)
                ...
                if not hasattr(self, "job_id"):
                    ...
                    prefix = "(%s/%s)" % (self.job_wrapper.get_id_tag(),
                                          self.job_id)
                log.debug("%s Unable to cleanup %s: %s" % (prefix, file,
                                                           str(e)))
...
class AsynchronousJobState(JobState):
    ...
    def __init__(self,
        ...
        super(AsynchronousJobState, self).__init__(job_wrapper,
                                                   job_destination)
        self.old_state = None
        self._running = False
        self.check_count = 0
...
class AsynchronousJobRunner(BaseJobRunner):
    ...
    def _init_monitor_thread(self):
        self.monitor_thread = threading.Thread(
            name="%s.monitor_thread" % self.runner_name, target=self.monitor)
    ...
    def shutdown(self):
        ...
        log.info(
            "%s: Sending stop signal to monitor thread" % self.runner_name)
