...
from pulsar.client import (
    build_client_manager, ClientJobDescription, ClientOutputs, finish_job as
    pulsar_finish_job, PathMapper, PulsarClientTransportError, PulsarOutputs,
    submit_job as pulsar_submit_job, url_to_destination_params)
...
class PulsarJobRunner(AsynchronousJobRunner):
    ...
    def __init_pulsar_app(self, pulsar_conf_path):
        ...
        if pulsar_conf_path is None:
            ...
            log.info(
                "Loading Pulsar app configuration from %s" % pulsar_conf_path)
    ...
    def url_to_destination(self, url):
        ...
        return JobDestination(
            runner="pulsar", params=url_to_destination_params(url))
    ...
    def _populate_parameter_defaults(self, job_destination):
        ...
        for key, value in self.destination_defaults.items():
            ...
            if value is PARAMETER_SPECIFICATION_REQUIRED:
                raise Exception(
                    "Pulsar destination does not define required parameter %s"
                    % key)
    ...
    def get_client(self, job_destination_params, job_id, env=[]):
        ...
        if self.app.config.nginx_upload_job_files_path:
            endpoint_base = "%s" + \
                            ...
                            "?job_id=%s&job_key=%s"
        files_endpoint = endpoint_base % (self.galaxy_url, encoded_job_id,
                                          job_key)
        get_client_kwds = dict(
            job_id=str(job_id), files_endpoint=files_endpoint, env=env)
        return self.client_manager.get_client(job_destination_params,
                                              **get_client_kwds)
    ...
    def stop_job(self, job):
        ...
        client = self.get_client(job.destination_params,
                                 job.job_runner_external_id)
        job_ext_output_metadata = job.get_external_output_metadata()
        if not PulsarJobRunner.__remote_metadata(
                client) and job_ext_output_metadata:
            ...
            for sig in [15, 9]:
                try:
                    ...
                    log.warning(
                        ...
                        % (job.id, errno.errorcode[e.errno], sig, pid,
                           e.strerror))
                    return  # give up
                sleep(2)
                if not self.check_pid(pid):
    ...
    def __use_remote_datatypes_conf(pulsar_client):
        ...
        use_remote_datatypes = string_as_bool_or_none(
            pulsar_client.destination_params.get("use_remote_datatypes",
                                                 False))
    ...
    def __build_metadata_configuration(self, client, job_wrapper,
        ...
        if remote_metadata:
            ...
            outputs = [
                Bunch(
                    false_path=os.path.join(outputs_directory,
...
class PulsarMQJobRunner(PulsarJobRunner):
    ...
    def _monitor(self):
        ...
        self.client_manager.ensure_has_status_update_callback(
            self.__async_update)
