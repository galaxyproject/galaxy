...
def _remote_failure_message(base_message: str, full_status: dict[str, Any] | None) -> str:
...
class PulsarJobRunner(AsynchronousJobRunner[AsynchronousJobState]):
    ...
    def _client_manager_for(self, job_destination_params: dict[str, Any], job_id: int | str) -> Any:
...
class PulsarMQBYOCJobRunner(PulsarMQJobRunner):
    ...
    * ``relay_url``, ``manager`` — relay endpoint and per-user manager name.
    ...
    def __init__(
        ...
        nworkers: int,
        *,
        client_manager_factory: Callable[..., Any] | None = None,
        **kwds: Any,
    ) -> None:
    ...
    def _read_refresh_token(self, resource: "model.ComputeResource", user: Optional["model.User"]) -> str | None:
    ...
    def _get_user_for_job_id(self, job_id: int | str) -> Optional["model.User"]:
