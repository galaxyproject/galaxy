    ...
    **Record Mode Options** (some of these may work in playback mode also)
    ...
    def __init__(self, **kwargs):
        ...
        if self.mode == MODE_RECORD:
            ...
            raise Exception(
                "Invalid mode supplied to CollectlCli - %s" % self.mode)
        ...
        command_args[
            "procfilt_arg"] = "" if not procfilt else "--procfilt %s" % procfilt
    ...
    def __validate_interval_arg(self, value, multiple_of=None):
        if value and not str(value).isdigit():
            raise Exception(
                "Invalid interval argument supplied, must be integer %s" %
                value)
        if multiple_of:
    ...
    def run(self, stdout=subprocess.PIPE, stderr=subprocess.PIPE):
        ...
        proc = subprocess.Popen(
            command_line, shell=True, stdout=stdout, stderr=stderr)
