...
class CollectlFormatter(formatting.JobMetricFormatter):
    def format(self, key, value):
        if key == "pid":
            ...
            return ("Job Runtime (System+User)",
            ...
            elif resource_type in ["RSYS", "WSYS"
                                   ] and stat_type in ["count", "max", "sum"]:
                value_str = "%d (# system calls)" % int(value)
            else:
                value_str = str(value)
            resource_title = FORMATTED_RESOURCE_TITLES.get(
...
class CollectlPlugin(InstrumentPlugin):
    ...
    def __init__(self, **kwargs):
        ...
        if "app" in kwargs:
            ...
            saved_logs_path = kwargs["app"].config.resolve_path(
                saved_logs_path)
    ...
    def pre_execute_instrument(self, job_directory):
        ...
        commands.append(
            '''echo "$$" > '%s' ''' % self.__pid_file(job_directory))
    ...
    def __configure_paths(self, kwargs):
        ...
        self.remote_collectl_path = kwargs.get("remote_collectl_path",
                                               collectl_path)
        self.local_collectl_path = kwargs.get("local_collectl_path",
                                              collectl_path)
    ...
    def __summarize_process_data(self, pid, collectl_log_path):
        ...
        return processes.generate_process_statistics(playback_cli, pid,
                                                     self.process_statistics)

    def __collectl_recorder_cli(self, job_directory):
        cli_args = self.collectl_recorder_args.copy()
        cli_args["destination_path"] = self._instrument_file_path(
            job_directory, "log")
    ...
    def __collectl_record_command(self, job_directory):
        ...
        return "%s > %s 2>&1 &" % (collectl_cli.build_command_line(),
                                   redirect_to, )
