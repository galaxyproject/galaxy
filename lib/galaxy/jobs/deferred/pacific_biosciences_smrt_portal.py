...
class SMRTPortalPlugin(DataTransfer):
    ...
    def create_job(self, trans, **kwd):
        if 'secondary_analysis_job_id' in kwd:
            ...
            external_service = sample.request.type.get_external_service(
                'pacific_biosciences_smrt_portal')
            external_service.load_data_transfer_settings(trans)
            http_configs = external_service.data_transfer[
                trans.model.ExternalService.data_transfer_protocol.HTTP]
            if not http_configs['automatic_transfer']:
                raise Exception(
                    "Manual data transfer using http is not yet supported.")
            smrt_host = external_service.form_values.content['host']
            external_service_type = external_service.get_external_service_type(
                trans)
            ...
            for k, v in external_service.form_values.content.items():
                match = self.dataset_name_re.match(
                    k) or self.dataset_datatype_re.match(k)
                if match:
            ...
            for id, attrs in results.items():
                url_template = external_service_type.run_details[
                    'results_urls'].get(id + '_name')
                url = Template(url_template).substitute(
                    host=smrt_host,
                    secondary_analysis_job_id=kwd['secondary_analysis_job_id'])
                results[id]['url'] = url
                if sample.workflow:
            ...
            self._associate_untransferred_datasets_with_sample(
                sample, external_service, results)
            ...
            log.error(
                'No job was created because kwd does not include "secondary_analysis_job_id" or "transfer_job_id".'
            )
            return
        ...
        self.sa_session.flush()
        log.debug('Created a deferred job in the SMRTPortalPlugin of type: %s'
                  % params['type'])
    ...
    def _associate_untransferred_datasets_with_sample(
            self, sample, external_service, results_dict):
