    ...
    def to_xml_file( self, shed_tool_data_table_config, new_elems=None, remove_elems=None ):
        ...
        with open( full_path, 'wb' ) as out:
            ...
            for elem in out_elems:
                out.write( util.xml_util.xml_to_string( elem ) )
            out.write( '</tables>\n' )
...
class TabularToolDataTable( ToolDataTable, Dictifiable ):
    ...
    def configure_and_load( self, config_element, tool_data_path, from_shed_config=False, url_timeout=10 ):
        ...
        for file_element in config_element.findall( 'file' ):
            ...
            if file_path is None:
                log.debug( "Encountered a file element (%s) that does not contain a path value when loading tool data table '%s'.", util.xml_util.xml_to_string( file_element ), self.name )
                continue
    ...
    @property
    def xml_string( self ):
        return util.xml_util.xml_to_string( self.config_element )
