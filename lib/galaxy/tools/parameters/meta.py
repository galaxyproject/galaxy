...
def expand_workflow_inputs(inputs):
    ...
    for step_id, step in sorted(inputs.items()):
        for key, value in sorted(step.items()):
            if isinstance(
                    value, dict
            ) and 'batch' in value and value['batch'] is True and 'values' in value and isinstance(
                    value['values'], list):
    ...
    for linked_values, product_values in itertools.product(
        ...
        new_keys = []
        for (step_id,
             key), value in list(zip(linked_keys, linked_values)) + list(
...
def __expand_collection_parameter(trans,
                                  input_key,
                                  incoming_val,
                                  collections_to_match,
                                  linked=False):
...
def __collection_multirun_parameter(value):
    ...
    if len(batch_values) == 1:
        ...
        if isinstance(batch_over, dict) and ('src' in batch_over) and (
                batch_over['src'] == 'hdca'):
            return True
    return False
