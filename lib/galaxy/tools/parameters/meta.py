...
ExpandedAsyncT = Tuple[List[ToolStateJobInstanceT], List[ToolStateDumpedToJsonInternalT], Optional[matching.MatchingCollections]]


def expand_meta_parameters_async(
    app, tool, incoming: RequestInternalDereferencedToolState
) -> ExpandedAsyncT:
    ...
    expanded_job_states = build_combos(to_decoded_json(single_inputs), to_decoded_json(matched_multi_inputs), to_decoded_json(multiplied_multi_inputs), nested=True)
    if collections_to_match.has_collections():
