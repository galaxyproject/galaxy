...
class DatasetMatcher(object):
    ...
    def __init__(self, trans, param, value, other_values):
        ...
        if param.options and other_values:
            try:
                filter_value = param.options.get_options(trans,
                                                         other_values)[0][0]
            except IndexError:
    ...
    def hda_accessible(self, hda, check_security=True):
        ...
        return state_valid and (not check_security
                                or self.__can_access_dataset(dataset))

    def valid_hda_match(self,
                        hda,
        ...
        if hda.datatype.matches_any(formats):
            ...
            if not check_implicit_conversions:
                return False
            target_ext, converted_dataset = hda.find_conversion_destination(
                formats)
            if target_ext:
                original_hda = hda
                if converted_dataset:
                    hda = converted_dataset
                if check_security and not self.__can_access_dataset(
                        hda.dataset):
                    return False
    ...
    def hda_match(self,
        ...
        if accessible and (not ensure_visible or hda.visible or
                           (self.selected(hda)
                            and not hda.implicitly_converted_parent_datasets)):
    ...
    def filter(self, hda):
        ...
        """
        param = self.param
        return param.options and param.get_options_filter_attribute(
            hda) != self.filter_value
    ...
    def __can_access_dataset(self, dataset):
        ...
        return self.trans.app.security_agent.can_access_dataset(
            self.current_user_roles, dataset)
