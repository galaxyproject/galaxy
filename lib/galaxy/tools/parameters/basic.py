        ...
        if isinstance(value, list):
            ...
            for v in value:
                if v not in legal_values:
                    raise ParameterValueError("an invalid option (%r) was selected (valid options: %s)" % (v, ",".join(legal_values)), self.name, v, is_dynamic=self.is_dynamic)
                rval.append(v)
            return rval
        else:
            ...
            if value not in legal_values and require_legal_value:
                raise ParameterValueError("an invalid option (%r) was selected (valid options: %s)" % (value, ",".join(legal_values)), self.name, value, is_dynamic=self.is_dynamic)
            return value
...
class DrillDownSelectToolParameter(SelectToolParameter):
    ...
    def from_json(self, value, trans, other_values={}):
        ...
        if not legal_values and trans.workflow_building_mode:
            ...
            if self.optional:
                return None
            raise ParameterValueError("an invalid option (%r) was selected" % value, self.name, value)
        elif not legal_values:
        ...
        for val in value:
            if val not in legal_values:
                raise ParameterValueError("an invalid option (%r) was selected (valid options: %s)" % (val, ",".join(legal_values)), self.name, val)
            rval.append(val)
        return rval
        ...
        for name, conv_extension in input_source.parse_conversion_tuples():
            ...
            if self.datatypes_registry:
                ...
                if conv_type is None:
                    raise ParameterValueError("datatype class not found for extension '%s', which is used as 'type' attribute in conversion of data parameter" % conv_type, self.name)
