...
class ColumnListParameter(SelectToolParameter):
    ...
    def get_column_list(self, trans, other_values):
        ...
        for dataset in util.listify(dataset):
            ...
            if not hasattr(dataset, 'metadata_') or \
                    not hasattr(dataset.metadata_, 'columns') or \
                    not dataset.metadata_.columns:
                return []
                ...
                if self.numerical:  # If numerical was requested, filter columns based on metadata
                    if hasattr(dataset, 'metadata_') and hasattr(dataset.metadata_, 'column_types'):
                        if len(dataset.metadata_.column_types) >= len(cnames):
                            numerics = [i for i, x in enumerate(dataset.metadata_.column_types) if x in ['int', 'float']]
                ...
                if dataset:
                    for meta_key, meta_dict in filter_value.items():
                        if hasattr(dataset, 'metadata_') and hasattr(dataset.metadata_, 'spec'):
                            check_meta_val = dataset.metadata_.spec[meta_key].param.to_string(dataset.metadata_.get(meta_key))
