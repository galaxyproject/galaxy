...
def verify(
    ...
    elif attributes is not None and attributes.get("checksum",
                                                   None) is not None:
        checksum_value = attributes.get("checksum", None)
        expected_checksum_type, expected_checksum = checksum_value.split(
            "$", 1)

    if expected_checksum_type:
        try:
            _verify_checksum(output_content, expected_checksum_type,
                             expected_checksum)
        except AssertionError as err:
            errmsg = '%s different than expected\n' % (item_label)
            errmsg += str(err)
    ...
    if filename is not None:
        ...
        if keep_outputs_dir:
            ...
            try:
                ...
                log.debug('## GALAXY_TEST_SAVE=%s. saved %s' %
                          (keep_outputs_dir, ofn))
        try:
            ...
            if compare == 'diff':
                ...
                files_re_match_multiline(
                    local_name, temp_name, attributes=attributes)
...
def _verify_checksum(data, checksum_type, expected_checksum_value):
    ...
    if expected_checksum_value != actual_checksum_value:
        ...
        message = template % (actual_checksum_value, expected_checksum_value,
                              checksum_type)
...
def files_diff(file1, file2, attributes=None):
    ...
    if not filecmp.cmp(file1, file2):
        ...
        if files_differ:
            ...
            diff = list(
                difflib.unified_diff(local_file, history_data, "local_file",
                                     "history_data"))
            ...
            if diff_lines > allowed_diff_count:
                ...
                if file1.endswith('.pdf') or file2.endswith('.pdf'):
                    ...
                    # not differ we're ok.
                    valid_diff_strs = [
                        'description', 'createdate', 'creationdate', 'moddate',
                        'id', 'producer', 'creator'
                    ]
                    valid_diff = False
                    ...
                    for line in diff_slice:
                        ...
                        if (line.startswith('+') or line.startswith('-')
                            ) and line.find('local_file') < 0 and line.find(
                                'history_data') < 0:
...
def files_re_match_multiline(file1, file2, attributes=None):
    ...
    assert re.match(
        local_file, history_data,
        re.MULTILINE), "Multiline Regular expression did not match data file"
