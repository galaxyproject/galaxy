...
def _lint_exit_code(child, lint_ctx):
    for key, value in child.attrib.items():
        if key == "range":
            ...
            lint_ctx.warn(
                "Unknown attribute [%s] encountered on exit_code tag." % key)
    ...
    for key, value in child.attrib.items():
        if key == "source":
            if value not in ["stderr", "stdout", "both"]:
                lint_ctx.error(
                    "Unknown error code level encountered [%s]" % value)
        elif key == "level":
            _lint_level(value, lint_ctx)
        elif key == "match":
            # TODO: validate
            ...
            lint_ctx.warn(
                "Unknown attribute [%s] encountered on regex tag." % key)


def _lint_level(level_value, lint_ctx):
    if level_value not in ["warning", "fatal", "log"]:
        lint_ctx.error(
            "Unknown error code level encountered [%s]" % level_value)
