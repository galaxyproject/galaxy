...
def parse_tests(tool, tests_source):
    ...
    """
    default_interactor = os.environ.get('GALAXY_TEST_DEFAULT_INTERACTOR',
                                        DEFAULT_INTERACTOR)
    ...
    for i, test_dict in enumerate(tests_dict.get('tests', [])):
        test = ToolTestBuilder(
            tool, test_dict, i, default_interactor=tests_default_interactor)
        tests.append(test)
    return tests
...
class ToolTestBuilder(object):
    ...
    def __matching_case_for_value(self, cond, declared_value):
        ...
        if isinstance(test_param,
                      galaxy.tools.parameters.basic.BooleanToolParameter):
            if declared_value is None:
                # No explicit value for param in test case, determine from default
                query_value = test_param.checked
            else:
                query_value = _process_bool_param_value(
                    test_param, declared_value)

            def matches_declared_value(case_value):
                return _process_bool_param_value(test_param,
                                                 case_value) == query_value
        elif isinstance(test_param,
    ...
    def __handle_test_dict(self, test_dict, i, default_interactor):
        try:
            ...
            self.inputs = self.__process_raw_inputs(self.tool.inputs,
                                                    test_dict["inputs"])
    ...
    def __process_raw_inputs(self,
        ...
        for key, value in tool_inputs.items():
            if isinstance(value, galaxy.tools.parameters.grouping.Conditional):
                cond_context = ParamContext(
                    name=value.name, parent_context=parent_context)
                ...
                if case:
                    for input_name, input_value in case.inputs.items():
                        case_inputs = self.__process_raw_inputs(
                            {
                ...
                if raw_input:
                    ...
                    elif isinstance(value, galaxy.tools.parameters.basic.
                                    DataCollectionToolParameter):
                        assert 'collection' in param_extra
                        collection_def = param_extra['collection']
                        for (name, value,
                             extra) in collection_def.collect_inputs():
                            require_file(name, value, extra,
                                         self.required_files)
...
def _process_simple_value(param, param_value):
    if isinstance(
            param,
            galaxy.tools.parameters.basic.SelectToolParameter) and hasattr(
    ...
    for fname, extra in required_files:
        ...
        for edit_att in edit_attributes:
            if edit_att.get('type', None) == 'name':
                ...
                raise Exception('edit_attributes type (%s) is unimplemented' %
...
def require_file(name, value, extra, required_files):
    ...
    name_change = [
        att for att in extra.get('edit_attributes', [])
        if att.get('type') == 'name'
    ]
    if name_change:
        name_change = name_change[-1].get(
            'value')  # only the last name change really matters
...
class ParamContext(object):
    ...
    def for_state(self):
        name = self.name if self.index is None else "%s_%d" % (self.name,
                                                               self.index)
    ...
    def param_names(self):
        for parent_context_param in self.parent_context.param_names():
            if self.index is not None:
                yield "%s|%s_%d" % (parent_context_param, self.name,
                                    self.index)
            else:
