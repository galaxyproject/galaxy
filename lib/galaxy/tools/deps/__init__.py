...
class DependencyManager(object):
    ...
    def get_app_option(self, key, default=None):
        ...
        if value is CONFIG_VAL_NOT_FOUND and hasattr(self.__app_config,
                                                     "config_dict"):
            value = self.__app_config.config_dict.get(key,
                                                      CONFIG_VAL_NOT_FOUND)
        if value is CONFIG_VAL_NOT_FOUND:
            value = default
        return value

    def dependency_shell_commands(self, requirements, **kwds):
        requirement_to_dependency = self.requirements_to_dependencies(
            requirements, **kwds)
        return [
            dependency.shell_commands(requirement)
            for requirement, dependency in requirement_to_dependency.items()
        ]
    ...
    def _requirements_to_dependencies_dict(self, requirements, **kwds):
        ...
        for i, resolver in enumerate(self.dependency_resolvers):
            ...
            if all_unmet and hasattr(resolver, "resolve_all"):
                # TODO: Handle specs.
                dependencies = resolver.resolve_all(resolvable_requirements,
                                                    **kwds)
                if dependencies:
                    assert len(dependencies) == len(resolvable_requirements)
                    for requirement, dependency in zip(resolvable_requirements,
                                                       dependencies):
            ...
            for requirement in resolvable_requirements:
                ...
                if not isinstance(dependency, NullDependency):
                    log.debug(dependency.resolver_msg)
                    requirement_to_dependency[requirement] = dependency
                elif return_null_dependencies and (
                        resolver == self.dependency_resolvers[-1]
                        or i == index):
                    log.debug(dependency.resolver_msg)
                    requirement_to_dependency[requirement] = dependency
    ...
    def uses_tool_shed_dependencies(self):
        return any(
            map(lambda r: isinstance(r, ToolShedPackageDependencyResolver),
    ...
    def find_dep(self, name, version=None, type='package', **kwds):
        ...
        dep_dict = self._requirements_to_dependencies_dict(
            requirements, **kwds)
        if len(dep_dict) > 0:
    ...
    def __resolvers_dict(self):
        ...
        return plugin_config.plugins_dict(galaxy.tools.deps.resolvers,
                                          'resolver_type')
...
class CachedDependencyManager(DependencyManager):
    ...
    def build_cache(self, requirements, **kwds):
        resolved_dependencies = self.requirements_to_dependencies(
            requirements, **kwds)
        ...
        if os.path.exists(hashed_dependencies_dir):
            if kwds.get('force_rebuild', False):
                ...
                return
        [
            dep.build_cache(hashed_dependencies_dir)
    ...
    def dependency_shell_commands(self, requirements, **kwds):
        ...
        commands = [
            dep.shell_commands(req)
            for req, dep in resolved_dependencies.items()
        ]
        return commands

    def hash_dependencies(self, resolved_dependencies):
        """Return hash for dependencies"""
        resolved_dependencies = [(dep.name, dep.version, dep.exact,
                                  dep.dependency_type)
    ...
    def get_hashed_dependencies_path(self, resolved_dependencies):
        ...
        return os.path.abspath(
            os.path.join(self.tool_dependency_cache_dir, req_hashes))
