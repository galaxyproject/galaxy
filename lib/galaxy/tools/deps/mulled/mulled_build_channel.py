...
def _fetch_repo_data(args):
    ...
    if not os.path.exists(repo_data):
        ...
        check_output("wget --quiet https://conda.anaconda.org/%s/%s/repodata.json.bz2 -O '%s.bz2' && bzip2 -d '%s.bz2'" %
...
def add_channel_arguments(parser):
    ...
    parser.add_argument(
        '--diff-hours', dest='diff_hours', default="25", help='If finding all recently changed recipes, use this number of hours.')
