...
def main(argv=None):
    ...
    parser.add_argument(
        'files', metavar="FILES", default=".", help="Path to directory (or single file) of TSV files describing composite recipes.")
    args = parser.parse_args()
    for (targets, image_build, name_override) in generate_targets(args.files):
        try:
            mull_targets(targets, image_build=image_build, name_override=name_override, **args_to_mull_targets_kwds(args))
...
def _parse_line(line_str):
    ...
    assert len(
        line_parts) < 3, "Too many fields in line [%s], expect at most 3 - targets, image build number, and name override." % line_str
