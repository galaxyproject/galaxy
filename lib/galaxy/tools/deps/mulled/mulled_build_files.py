...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        default=".",
        help=
        "Path to directory (or single file) of TSV files describing composite recipes."
    )
...
def line_to_targets(line_str):
    ...
    return (target_str_to_targets(line.targets), line.image_build,
            line.name_override)


_Line = collections.namedtuple("_Line",
                               ["targets", "image_build", "name_override"])
...
def _parse_line(line_str):
    ...
    assert len(
        line_parts
    ) < 3, "Too many fields in line [%s], expect at most 3 - targets, image build number, and name override." % line_str
