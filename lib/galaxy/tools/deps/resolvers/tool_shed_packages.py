...
class ToolShedPackageDependencyResolver(BaseGalaxyPackageDependencyResolver,
    ...
    def _find_dep_default(self, name, type='package', **kwds):
        if type == 'set_environment' and kwds.get(
            ...
            if installed_tool_dependency:
                ...
                if has_script_dep:
                    ...
                    return ToolShedDependency(
                        dependency.script, dependency.path, None, name, True)
        return NullDependency(version=None, name=name)

    def _get_package_installed_dependency_path(self, installed_tool_dependency,
                                               name, version):
        tool_shed_repository = installed_tool_dependency.tool_shed_repository
        base_path = self.base_path
        return join(base_path, name, version, tool_shed_repository.owner,
    ...
    def _get_set_environment_installed_dependency_script_path(
        ...
        base_path = self.base_path
        path = abspath(
            join(base_path, 'environment_settings', name,
