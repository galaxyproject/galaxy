...
class ExecutionTracker:
    ...
    def precreate_output_collections(self, history, params, tool_request: Optional[ToolRequest]):
        ...
        if tool_request:
            for output_name, collection_instance in collection_instances.items():
                trica = ToolRequestImplicitCollectionAssociation()
                trica.output_name = output_name
                trica.dataset_collection = collection_instance
                trica.tool_request = tool_request
                trans.sa_session.add(trica)
