        ...
        if input.type == "repeat":
            ...
            if str(current_case) != "null":  # str because it is a wrapped...
                ...
                input_json[input_name] = simple_value(
                    case_input, case_value, cwl_type=current_case)
        else:
            input_json[input_name] = simple_value(input,
    ...
    for input_name, input in inputs.iteritems():
        ...
        if galaxy_input_type == "repeat":
            ...
            if (as_dict_value is NOT_PRESENT
                    or as_dict_value is None) and "null" in case_strings:
                cwl_type = "null"
                ...
                cwl_type = "long"
            elif isinstance(as_dict_value,
                            (int, float)) and "float" in case_strings:
                cwl_type = "float"
            elif isinstance(as_dict_value,
                            (int, float)) and "double" in case_strings:
                cwl_type = "double"
            elif isinstance(as_dict_value,
                            string_types) and "string" in case_strings:
                cwl_type = "string"
            elif isinstance(
                    as_dict_value,
                    dict) and "src" in as_dict_value and "id" in as_dict_value:
                # Bit problematic...
                cwl_type = "File"
            ...
            if cwl_type != "null":
                ...
                galaxy_value = from_simple_value(current_case_input,
                                                 as_dict_value, cwl_type)
