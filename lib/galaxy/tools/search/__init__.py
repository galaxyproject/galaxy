...
class ToolBoxSearch(object):
    ...
    def _create_doc(self, tool_id, tool, index_help=True):
        ...
        add_doc_kwds = {
            ...
            "section":
            to_unicode(tool.get_panel_section()[1] if len(
                tool.get_panel_section()) == 2 else ''),
            "help":
            to_unicode("")
        }
        if tool.name.find('-') != -1:
    ...
    def search(self, q, tool_name_boost, tool_section_boost,
        ...
        searcher = self.index.searcher(weighting=BM25F(field_B={
            'name_B':
            float(tool_name_boost),
            'section_B':
            float(tool_section_boost),
            'description_B':
            float(tool_description_boost),
            'labels_B':
            float(tool_label_boost),
            'stub_B':
            float(tool_stub_boost),
            'help_B':
            float(tool_help_boost)
        }))
