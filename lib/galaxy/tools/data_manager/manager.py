...
class DataManager:
    ...
    def process_result(self, out_data):
        ...
        for data_table_name in self.data_tables.keys():
            ...
            if not data_table_values:
                log.warning(
                    f'No values for data table "{data_table_name}" were returned by the data manager "{self.id}".'
                )
            ...
            if not isinstance(data_table, SUPPORTED_DATA_TABLE_TYPES):
                log.error(
                    f'The data manager "{self.id}" returned an unsupported data table "{data_table_name}" with type "{type(data_table)}" with new entries "{data_table_values}". These entries will not be created. Please confirm that the data table is of a supported type ({SUPPORTED_DATA_TABLE_TYPES}).'
                )
                continue  # next table name
            output_ref_values = {}
            ...
            for data_table_row in data_table_values:
                ...
                for (
                    ...
                    if name in output_ref_values:
                        self.process_move(
                            data_table_name, name, output_ref_values[name].extra_files_path, **data_table_value
                        )
        ...
        if self.undeclared_tables and data_tables_dict:
            ...
            for data_table_name, data_table_values in data_tables_dict.items():
                ...
                log.warning(
                    f'The data manager "{self.id}" returned an undeclared data table "{data_table_name}" with new entries "{data_table_values}". These entries will not be created. Please confirm that an entry for "{data_table_name}" exists in your "{self.data_managers.filename}" file.'
                )
    ...
    def process_value_translation(self, data_table_name, column_name, **kwd):
        ...
        if (
            ...
            and column_name in self.value_translation_by_data_table_column[data_table_name]
        ):
