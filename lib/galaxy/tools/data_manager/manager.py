...
class DataManagers(object):
    ...
    def load_from_xml(self, xml_filename, store_tool_path=True):
        ...
        for data_manager_elem in root.findall('data_manager'):
            self.load_manager_from_elem(
                data_manager_elem, tool_path=self.tool_path)

    def load_manager_from_elem(self,
                               data_manager_elem,
                               tool_path=None,
                               add_manager=True):
        try:
            data_manager = DataManager(
                self, data_manager_elem, tool_path=tool_path)
        except Exception as e:
    ...
    def add_manager(self, data_manager):
        if data_manager.id in self.data_managers:
            log.warning("A data manager has been defined twice: %s " %
                        (data_manager.id))
...
class DataManager(object):
    ...
    def __init__(self, data_managers, elem=None, tool_path=None):
        ...
        if elem is not None:
            self.load_from_element(elem, tool_path
                                   or self.data_managers.tool_path)
                    ...
                    for name, value in data_table_row.items():
                        if name in path_column_names:
                            data_table_value[name] = os.path.abspath(
                                os.path.join(self.data_managers.app.config.
