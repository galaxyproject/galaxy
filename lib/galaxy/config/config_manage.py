...
def _generate_api_models(args: Namespace, app_desc: App) -> None:
    ...
    f = StringIO()
    file_header = """
# This file is auto-generated by the Galaxy configuration schema tool.
...
"""
    ...
    for key, value in schema.app_schema.items():
        type = _get_schema_type(value)
        required = value.get("required", False)
        field_type = type if required else f"Optional[{type}]"
        default = _get_field_default(value, type, required)
        title = key.replace("_", " ").title()
        description = value.get("desc")
        field = f'{key}: {field_type} = Field({default}, title="{title}", description="""{description}""",)'
        f.write(f"    {field}\n")
...
def _get_field_default(value, field_type, required):
    default = None if "default" not in value or not required else value["default"]
