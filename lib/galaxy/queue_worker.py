...
def send_control_task(app, task, noop_self=False, kwargs={}):
    ...
    try:
        c = Connection(app.config.amqp_internal_connection)
        with producers[c].acquire(block=True) as producer:
            control_queues = galaxy.queues.all_control_queues_for_declare(
                app.config, app.application_stack)
            producer.publish(
                payload,
                exchange=galaxy.queues.galaxy_exchange,
...
def _get_new_toolbox(app):
    ...
    app.datatypes_registry.load_datatype_converters(
        new_toolbox, use_cached=True)
...
def reload_display_application(app, **kwargs):
    ...
    log.debug("Executing display application reload task for %s" %
              display_application_ids)
...
def recalculate_user_disk_usage(app, **kwargs):
    ...
    if user_id:
        user = sa_session.query(app.model.User).get(
            app.security.decode_id(user_id))
        if user:
            user.calculate_and_set_disk_usage()
        else:
            log.error(
                "Recalculate user disk usage task failed, user %s not found" %
                user_id)
    else:
...
def reload_tool_data_tables(app, **kwargs):
    params = util.Params(kwargs)
    log.debug("Executing tool data table reload for %s" % params.get(
        'table_names', 'all tables'))
    table_names = app.tool_data_tables.reload_tables(table_names=params.get(
        'table_name', None))
...
class GalaxyQueueWorker(ConsumerMixin, threading.Thread):
    ...
    def __init__(self,
                 app,
                 queue=None,
                 task_mapping=control_message_to_task,
                 connection=None):
        ...
        self.declare_queues = galaxy.queues.all_control_queues_for_declare(
            app.config, app.application_stack)
    ...
    def bind_and_start(self):
        log.info("Binding and starting galaxy control worker for %s",
                 self.app.config.server_name)
        self.control_queue = galaxy.queues.control_queue_from_config(
            self.app.config)
        self.start()

    def get_consumers(self, Consumer, channel):
        return [
            Consumer(queues=self.control_queue, callbacks=[self.process_task])
        ]
            ...
            if body.get('noop', None) != self.app.config.server_name:
                try:
                    ...
                    log.info(
                        ...
                        self.app.config.server_name, body['task'])
                    f(self.app, **body['kwargs'])
                except Exception:
                    # this shouldn't ever throw an exception, but...
                    log.exception("Error running control task type: %s",
                                  body['task'])
        else:
