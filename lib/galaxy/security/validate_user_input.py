...
def validate_email(trans, email, user=None, check_dup=True):
    ...
    elif check_dup and trans.sa_session.query(trans.app.model.User).filter_by(
            email=email).first():
...
def validate_publicname(trans, publicname, user=None):
    ...
    if trans.sa_session.query(trans.app.model.User).filter_by(
            username=publicname).first():
...
def transform_publicname(trans, publicname, user=None):
    ...
    if user and user.username == publicname:
        ...
        publicname = publicname.ljust(PUBLICNAME_MIN_LEN + 1,
                                      FILL_CHAR)[:PUBLICNAME_MAX_LEN]
        if not trans.sa_session.query(trans.app.model.User).filter_by(
                username=publicname).first():
            return publicname
    return ''
