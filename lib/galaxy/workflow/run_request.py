    ...
    for step in steps:
        ...
        for inputs_by_el in inputs_by.split("|"):
            if inputs_by_el == "step_id":
                ...
                possible_input_keys.append(
                    step.label or step.tool_inputs.get('name'))
            else:
...
def _get_target_history(trans, workflow, payload, param_keys=[], index=0):
    ...
    if history_id:
        ...
        if nids == 1:
            ...
            nh_name = '%s on %s and %s' % (nh_name, ', '.join(ids[0:-1]),
                                           ids[-1])
...
def build_workflow_run_configs(trans, workflow, payload):
    ...
    expanded_params, expanded_param_keys = expand_workflow_inputs(
        unexpanded_param_map)
    for index, param_map in enumerate(expanded_params):
        history = _get_target_history(trans, workflow, payload,
                                      expanded_param_keys, index)
        ...
        if inputs or not already_normalized:
            normalized_inputs = _normalize_inputs(workflow.steps, inputs,
                                                  inputs_by)
        else:
        ...
        for key, input_dict in normalized_inputs.items():
            ...
            if 'src' not in input_dict:
                raise exceptions.RequestParameterInvalidException(
                    "Not input source type defined for input '%s'." %
                    input_dict)
            ...
            try:
                if input_source == 'ldda':
                    ...
                    raise exceptions.RequestParameterInvalidException(
                        "Unknown workflow input source '%s' specified." %
                        input_source)
...
def workflow_run_config_to_request(trans, run_config, workflow):
    ...
    add_parameter("copy_inputs_to_history", "true"
                  if run_config.copy_inputs_to_history else "false",
...
def workflow_request_to_run_config(work_request_context, workflow_invocation):
    ...
    for input_association in workflow_invocation.input_step_parameters:
        inputs[input_association.
               workflow_step_id] = input_association.parameter_value
