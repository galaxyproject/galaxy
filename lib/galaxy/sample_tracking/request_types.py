...
RENAME_DATASET_OPTIONS = dict(
    [(f_type.lower(), f_descript)
     for f_type, f_descript in RequestType.rename_dataset_options.items()])
    ...
    def from_elem(self, elem, request_form, sample_form, external_service):
        ...
        if sample_states_elem:
            for sample_state_elem in sample_states_elem.findall('state'):
                self.sample_state_factory.from_elem(
                    rval,
                    sample_state_elem)  # should this be the return value??
        return rval


request_type_factory = RequestTypeFactory(sample_state_factory,
