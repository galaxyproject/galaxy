...
class RequestTypeFactory( object ):
    ...
    def new( self, name, request_form, sample_form, sequencer, description=None, sample_states = None ):
        """
        Return new RequestType.
        """
        assert name, 'RequestType requires a name'        
        return RequestType( name=name, desc=description, request_form=request_form, sample_form=sample_form, sequencer=sequencer )
    def from_elem( self, elem, request_form, sample_form, sequencer ):
        """
        Return RequestType created from an xml string.
        """
        name = elem.get( 'name', '' )
        description = elem.get( 'description', '' )
        rval = self.new( name, request_form, sample_form, sequencer=sequencer, description=description )
        #load states
