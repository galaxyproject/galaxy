...
class ExternalServiceTypesCollection(object):
    def __init__(self, config_filename, root_dir, app):
        ...
        try:
            ...
            log.exception("ExternalServiceTypesCollection error reading %s",
                          config_filename)
        ...
        for elem in root:
            try:
                if elem.tag == 'external_service_type':
                    ...
                    self.all_external_service_types[
                        external_service_type.id] = external_service_type
                    log.debug("Loaded external_service_type: %s %s" %
                              (external_service_type.name,
                               external_service_type.config_version))
                    if visible:
                        self.visible_external_service_types.append(
                            external_service_type.id)
            except:
                log.exception(
                    "error reading external_service_type from path: %s",
                    file_path)
    ...
    def reload(self, external_service_type_id):
        ...
        if external_service_type_id not in self.all_external_service_types.keys(
            ...
            raise ExternalServiceTypeNotFoundException(
                "No external_service_type with id %s" %
                external_service_type_id)
        old_external_service_type = self.all_external_service_types[
            external_service_type_id]
        new_external_service_type = self.load_external_service_type(
            old_external_service_type.config_file)
        self.all_external_service_types[
            external_service_type_id] = new_external_service_type
        log.debug("Reloaded external_service_type %s" %
                  (external_service_type_id))
...
class ExternalServiceType(object):
    ...
    def parse(self, root):
        ...
        self.actions = ExternalServiceActionsGroup.from_elem(
            root.find('actions'), parent=self)
    ...
    def parse_data_transfer_settings(self, root):
        ...
        for data_transfer_elem in data_transfer_settings_elem.findall(
                "data_transfer"):
            if data_transfer_elem.get(
                    'protocol'
            ) == model.ExternalService.data_transfer_protocol.SCP:
                scp_data_transfer = data_transfer_factories[
                    model.ExternalService.data_transfer_protocol.SCP]
                scp_data_transfer.parse(self.config_file, data_transfer_elem)
                self.data_transfer[
                    model.ExternalService.data_transfer_protocol.
                    SCP] = scp_data_transfer
            if data_transfer_elem.get(
                    'protocol'
            ) == model.ExternalService.data_transfer_protocol.HTTP:
                http_data_transfer = data_transfer_factories[
                    model.ExternalService.data_transfer_protocol.HTTP]
                http_data_transfer.parse(self.config_file, data_transfer_elem)
                self.data_transfer[
                    model.ExternalService.data_transfer_protocol.
                    HTTP] = http_data_transfer
    ...
    def parse_run_details(self, root):
        ...
        if run_details_elem is not None:
            ...
            if results_elem is not None:
                ...
                self.run_details['results'], self.run_details[
                    'results_urls'] = self.parse_run_details_results(
                        results_elem)
