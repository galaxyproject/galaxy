...
def all_control_queues_for_declare(config, application_stack):
    ...
    possible_stack_queues = [
        Queue("control.%s.%s" % (config.server_name.split('.')[0], wkr['id']), galaxy_exchange, routing_key='control')
        for wkr in application_stack.workers()
    ]
...
def control_queue_from_config(config):
    ...
    """
    return Queue("control.%s" % config.server_name, galaxy_exchange, routing_key='control.%s' % config.server_name)
