...
from . import (binary, coverage, data, images, interval, qualityscore,
               sequence, tabular, text, tracks, xml)
...
class Registry(object):
    ...
    def load_datatypes(self,
        ...
        if root_dir and config:
            ...
            if not self.converters_path:
                ...
                self.converters_path = os.path.join(root_dir,
                                                    self.converters_path_attr)
            ...
            for elem in registration.findall('datatype'):
                ...
                # in the distribution.
                make_subclass = galaxy.util.string_as_bool(
                ...
                if deactivate:
                    ...
                    for in_memory_elem in self.datatype_elems:
                        in_memory_extension = in_memory_elem.get(
                            'extension', None)
                        if in_memory_extension == extension:
                            in_memory_dtype = elem.get('type', None)
                            in_memory_type_extension = elem.get(
                                'type_extension', None)
                    ...
                    if can_process_datatype:
                        ...
                        if ok:
                            if not deactivate:
                                ...
                                self.datatypes_by_extension[
                                    extension].max_optional_metadata_filesize = elem.get(
                                ...
                                for converter in elem.findall('converter'):
                                    ...
                                    converter_config = converter.get(
                                        'file', None)
                                    target_datatype = converter.get(
                                        'target_datatype', None)
                                    depends_on = converter.get(
                                        'depends_on', None)
                                        ...
                                        if proprietary_converter_path:
                                            ...
                                            self.converters.append(
                                                (converter_config, extension,
                                                 target_datatype))
                                # Add composite files.
                                for composite_file in elem.findall(
                                        'composite_file'):
                                    ...
                                    if name is None:
                                        self.log.warning(
                                            ...
                                            % composite_file)
                                    optional = composite_file.get(
                                        'optional', False)
                                    mimetype = composite_file.get(
                                        'mimetype', None)
                                    self.datatypes_by_extension[
                                        extension].add_composite_file(
                                            name,
                                            optional=optional,
                                            mimetype=mimetype)
                                for display_app in elem.findall('display'):
                                    if proprietary_display_path:
                                        ...
                                        if elem not in self.display_app_containers:
                                            self.display_app_containers.append(
                                                elem)
    ...
    def _load_build_sites(self, root):
        def load_build_site(build_site_config):
            ...
            if not (build_site_config.get('type')
                    and build_site_config.get('file')):
                self.log.exception(
            ...
            if site_type in ('ucsc', 'gbrowse'):
                self.legacy_build_sites[
                    site_type] = galaxy.util.read_build_sites(path)
        ...
        if root.find('build_sites') is not None:
            ...
            build_sites_config_file = getattr(self.config,
                                              "build_sites_config_file", None)
            if build_sites_config_file and os.path.exists(
    ...
    def load_datatype_sniffers(self,
                               root,
                               deactivate=False,
                               handling_proprietary_datatypes=False,
                               override=False):
                        ...
                        if ok:
                            try:
                                ...
                                self.log.exception(
                                    ...
                                    str(datatype_class_name), str(module))
                                ok = False
                            if ok:
                                if deactivate:
    ...
    def get_mimetype_by_extension(self,
                                  ext,
    ...
    def load_datatype_converters(self,
                                 toolbox,
                ...
                if installed_repository_dict:
                    ...
                    for tool_dict in tool_dicts:
                        if tool_dict['id'] == converter.id:
                            ...
                            break
                if deactivate:
                    toolbox.remove_tool_by_id(
                        converter.id, remove_from_panel=False)
                    if source_datatype in self.datatype_converters:
                        if target_datatype in self.datatype_converters[
                                source_datatype]:
                    ...
                    self.datatype_converters[source_datatype][
                        target_datatype] = converter
                    if not hasattr(
                            toolbox.app, 'tool_cache'
                    ) or converter.id in toolbox.app.tool_cache._new_tool_ids:
                        self.log.debug("Loaded converter: %s", converter.id)
            except Exception:
                if deactivate:
                    self.log.exception("Error deactivating converter from (%s)"
                                       % converter_path)
                else:
                    self.log.exception(
                        "Error loading converter (%s)" % converter_path)

    def load_display_applications(self,
                                  app,
                                  installed_repository_dict=None,
                                  deactivate=False):
        """
        ...
        for elem in datatype_elems:
            ...
            for display_app in elem.findall('display'):
                ...
                if installed_repository_dict:
                    ...
                    display_file_head, display_file_tail = os.path.split(
                        display_file)
                ...
                try:
                    ...
                    display_app = DisplayApplication.from_file(
                        config_path, app)
                    if display_app:
                        if display_app.id in self.display_applications:
                            if deactivate:
                                ...
                                display_app = self.display_applications[
                                    display_app.id]
                            ...
                            tool_dicts = installed_repository_dict[
                                'tool_dicts']
                            for tool_dict in tool_dicts:
                        ...
                        if deactivate:
                            if display_app.id in self.display_applications:
                                del self.display_applications[display_app.id]
                            if extension in self.datatypes_by_extension:
                                if display_app.id in self.datatypes_by_extension[
                                        extension].display_applications:
                            ...
                            self.display_applications[
                                display_app.id] = display_app
                            self.datatypes_by_extension[
                                extension].add_display_application(display_app)
                            if inherit and (
                                    self.datatypes_by_extension[extension],
                                    display_app
                    ...
                    if deactivate:
                        self.log.exception(
                            "Error deactivating display application (%s)" %
                            config_path)
                    else:
                        self.log.exception(
                            "Error loading display application (%s)" %
                            config_path)
        ...
        for extension, d_type1 in self.datatypes_by_extension.items():
            for d_type2, display_app in self.inherit_display_application_by_class:
                current_app = d_type1.get_display_application(
                    display_app.id, None)
    ...
    def find_conversion_destination_for_dataset_by_extensions(
            self, dataset, accepted_formats, converter_safe=True):
        ...
        for convert_ext in self.get_converters_by_datatype(dataset.ext):
            ...
            if convert_ext_datatype is None:
                ...
                converted_dataset = dataset.get_converted_files_by_type(
                    convert_ext)
    ...
    def edam_formats(self):
        """
        """
        mapping = dict((k, v.edam_format)
                       for k, v in self.datatypes_by_extension.items())
        return mapping
