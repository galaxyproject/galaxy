...
def __main__():
    ...
    for i, line in enumerate(open(input_name)):
        ...
        if line and not line.startswith('#') and not line.startswith(
                'track') and not line.startswith('browser'):
            try:
                elems = line.split('\t')
                if len(elems) == 12:
                    complete_bed = True
                ...
                if complete_bed:
                    out.write('%s\tbed2gff\t%s\t%d\t%d\t%s\t%s\t.\t%s %s;\n' %
                              (chrom, feature, start, end, score, strand,
                               feature, group))
                else:
                ...
                if complete_bed:
                    ...
                    for j in range(block_count):
                        ...
                        out.write(
                            ...
                            % (chrom, exon_start, exon_end, score, strand,
                               group))
            except:
                skipped_lines += 1
            ...
            if not first_skipped_line:
                first_skipped_line = i + 1
    out.close()
    info_msg = "%i lines converted to GFF version 2.  " % (
        i + 1 - skipped_lines)
    if skipped_lines > 0:
