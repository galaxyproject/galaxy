...
class Audio(Binary):
    ...
    def set_meta(self, dataset, **kwd):
        if which('ffprobe'):
            ...
            dataset.metadata_.audio_codecs = [stream['codec_name'] for stream in streams if stream['codec_type'] == 'audio']
            dataset.metadata_.sample_rates = [stream['sample_rate'] for stream in streams if stream['codec_type'] == 'audio']
...
class Video(Binary):
    ...
    def set_meta(self, dataset, **kwd):
        if which('ffprobe'):
            ...
            dataset.metadata_.audio_codecs = [stream['codec_name'] for stream in streams if stream['codec_type'] == 'audio']
            dataset.metadata_.video_codecs = [stream['codec_name'] for stream in streams if stream['codec_type'] == 'video']
