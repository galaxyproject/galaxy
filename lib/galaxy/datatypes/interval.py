...
class Interval(Tabular):
                        ...
                        if first_line_is_header or line[0] == "#":
                            ...
                            if len(elems) > 2:
                                ...
                                try:
                                    ...
                                    if (
                                        overwrite
                                        or not dataset.metadata.element_is_set(
                                            "startCol"
                                        )
                                    ):
                                ...
                                try:
                                    ...
                                    if (
                                        overwrite
                                        or not dataset.metadata.element_is_set("endCol")
                                    ):
    ...
    def get_estimated_display_viewport(
        ...
        viewport_feature_count = (
            100  # viewport should check at least 100 features; excludes comment lines
        )
        max_line_count = max(
    ...
    def as_ucsc_display_file(self, dataset, **kwd):
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode="w") as fh:
            ...
            if t >= 0:  # strand column (should) exists
                for i, elems in enumerate(
                    compression_utils.file_iter(dataset.file_name)
                ):
                    strand = "+"
                    name = "region_%i" % i
    ...
    def ucsc_links(self, dataset, type, app, base_url):
        ...
        for site_name, site_url in valid_sites:
            ...
            link = (
                f"{internal_url}?redirect_url={redirect_url}&display_url={display_url}"
            )
    ...
    def get_estimated_display_viewport(self, dataset):
        ...
        if self.displayable(dataset):
            try:
                ...
                stop = 0
                with compression_utils.get_fileobj(dataset.file_name) as fh:
                    for line in util.iter_start_of_line(
                        fh, VIEWPORT_READLINE_BUFFER_SIZE
                    ):
                        try:
                            if line.startswith(
                                "##sequence-region"
            ...
            for site_name, site_url in app.datatypes_registry.get_legacy_sites_by_build(
                ...
                if site_name in app.datatypes_registry.get_display_sites("gbrowse"):
                    if seqid.startswith("chr") and len(seqid) > 3:
                        seqid = seqid[3:]
                    redirect_url = quote_plus(
                        f"{site_url}/?q={seqid}:{start}..{stop}&eurl=%s"
                    )
                    link = self._get_remote_call_url(
                        redirect_url, site_name, dataset, type, app, base_url
                    )
            ...
            for site_name, site_url in app.datatypes_registry.get_legacy_sites_by_build(
                ...
                if site_name in app.datatypes_registry.get_display_sites("gbrowse"):
                    if chrom.startswith("chr") and len(chrom) > 3:
                        chrom = chrom[3:]
                    redirect_url = quote_plus(
                        f"{site_url}/?q={chrom}:{start}..{stop}&eurl=%s"
                    )
                    link = self._get_remote_call_url(
                        redirect_url, site_name, dataset, type, app, base_url
                    )
...
class AGP(Interval):
    ...
    MetadataElement(
        name="object",
        default=1,
        desc="This is the identifier for the object being assembled. This can be a chromosome, scaffold or contig",
        param=metadata.ColumnParameter,
    )
    MetadataElement(
        name="objectBeg",
        default=2,
        desc="The starting coordinates of the component/gap on the object in column 1",
        param=metadata.ColumnParameter,
    )
    MetadataElement(
        name="objectEnd",
        default=3,
        desc="The ending coordinates of the component/gap on the object in column 1",
        param=metadata.ColumnParameter,
    )
    MetadataElement(
        name="partNumber",
        default=4,
        desc="The line count for the components/gaps that make up the object described in column 1",
        param=metadata.ColumnParameter,
    )
    MetadataElement(
        name="compType",
        default=5,
        desc="The sequencing status of the component",
        param=metadata.ColumnParameter,
    )
    MetadataElement(
        name="compId_gapLength",
        default=6,
