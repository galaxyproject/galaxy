...
class _BamOrSam:
    ...
    def set_meta(self, dataset, overwrite=True, **kwd):
        try:
            ...
            dataset.metadata_.read_groups = [read_group['ID'] for read_group in dataset.metadata_.bam_header.get('RG', []) if 'ID' in read_group]
        ...
        if not dataset.dataset.purged:
            ...
            if dataset.metadata_.materials:
                for m in dataset.metadata_.materials:
                    try:
                        lines.append(f'{m} {dataset.metadata_.SpaceGroupNumber[m]} {dataset.metadata_.LatticeParameters[m]}')
                    except Exception:
                        continue
            dataset.peek = '\n'.join(lines)
...
class Fast5Archive(CompressedArchive):
    ...
    def set_meta(self, dataset, overwrite=True, **kwd):
        ...
        try:
            if dataset and tarfile.is_tarfile(dataset.file_name):
                with tarfile.open(dataset.file_name, 'r') as temptar:
                    dataset.metadata_.fast5_count = sum(
...
class HexrdImagesNpz(Npz):
    ...
    def set_peek(self, dataset, is_multi_byte=False):
        if not dataset.dataset.purged:
            lines = [f"Binary Hexrd Image npz {dataset.metadata_.nfiles} files ({nice_size(dataset.get_size())})",
...
class HexrdEtaOmeNpz(Npz):
    ...
    def set_peek(self, dataset, is_multi_byte=False):
        if not dataset.dataset.purged:
            lines = [f"Binary Hexrd Eta-Ome npz {dataset.metadata_.nfiles} files ({nice_size(dataset.get_size())})",
