"""E2E tests for the GalaxyAI conversational AI interface.
...
class TestGalaxyAI(SeleniumTestCase):
    ...
    def test_chat_greeting_flow(self):
        ...
        galaxyai.activity.wait_for_visible()
        ...
        assert galaxyai.query_cell.wait_for_text() == "Hello!"

        @retry_assertion_during_transitions
        def assert_response():
            assert "Hello" in galaxyai.response_content.wait_for_text()

        assert_response()

        # Agent indicator and metadata tags present
        galaxyai.agent_indicator.wait_for_visible()
        assert len(galaxyai.meta_tag.all()) >= 1

        # Feedback
        galaxyai.feedback_up.wait_for_and_click()
        assert "Thanks" in galaxyai.feedback_ack.wait_for_text()
    ...
    def test_multi_turn_and_new_chat(self):
        ...
        self.galaxyai_send_message("Hello!")
        self.galaxyai_send_message("How do I analyze RNA-seq data?")

        @retry_assertion_during_transitions
        def assert_two_exchanges():
            assert len(galaxyai.query_cell.all()) == 2
            responses = galaxyai.response_content.all()
    ...
    def test_delete_chats_via_selection(self):
        ...
        self.navigate_to_galaxyai()
        galaxyai = self.components.galaxyai
        history = galaxyai.history_panel
        ...
        # Switch sidebar away from galaxyai then back to force ChatHistoryPanel remount
        self.components.tools.activity.wait_for_and_click()
        galaxyai.activity.wait_for_and_click()
