...
KNOWN_FORM_FAILURES: dict[str, str] = {
    ...
    "filter_multiple_splitter_0": "the data selector never renders, so its dependent select stays empty",
    "filter_multiple_splitter_1": "the data selector never renders, so its dependent select stays empty",
    "filter_param_value_ref_attribute_2": "a select filtered on dataset metadata offers nothing to pick",
    "filter_param_value_ref_attribute_4": "a select filtered on collection metadata never renders",
    "filter_param_value_ref_attribute_5": "a select filtered on collection metadata never renders",
    ...
    "pick_value_2": "an unset optional boolean submits false, so a declared null cannot be expressed",
