...
class TestWindowManager(SeleniumTestCase):
    ...
    def test_multiple_windows(self):
        ...
        for hid in [1, 2, 3]:
            ...
            # Use JS click to bypass tooltip interception
            self.execute_script_click(item.display_button.wait_for_visible())
        ...
        focused_count = len(self.find_elements_by_selector(".winbox.focus"))
    ...
    def test_close_window(self):
        ...
        # Close one window via JS (avoids overlay/click issues)
        self.execute_script("document.querySelectorAll('.winbox .wb-close')[0].click();")
        ...
        # Verify counter via JS
        counter = self.execute_script("return window.Galaxy.frame.counter;")
    ...
    def test_display_only_in_window(self):
        ...
        iframe_src = self.execute_script("return document.querySelector('.winbox iframe').src;")
    ...
    def test_focus_switching(self):
        ...
        focused_title = self.execute_script(
            "return document.querySelector('.winbox.focus .wb-title')?.textContent || '';"
        )
        ...
        # Each WinBox has an .iframe-focus-overlay with a mousedown
        # listener that calls win.focus(). On unfocused windows,
        # the overlay has pointer-events:auto to capture clicks.
        self.execute_script(
            "document.querySelectorAll('.winbox .iframe-focus-overlay')[0]"
        ...
        def assert_focus_switched():
            new_focused = self.execute_script(
                "return document.querySelector('.winbox.focus .wb-title')?.textContent || '';"
            )
