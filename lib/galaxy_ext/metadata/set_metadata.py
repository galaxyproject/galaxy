...
assert sys.version_info[:2] >= (2, 6) and sys.version_info[:2] <= (
    2, 7), 'Python version must be 2.6 or 2.7, this is: %s' % sys.version
...
def set_metadata():
    ...
    datatypes_registry.load_datatypes(
        root_dir=galaxy_root, config=datatypes_config)
    ...
    for filenames in sys.argv[1:]:
        ...
        try:
            ...
            files_path = os.path.abspath(
                os.path.join(tool_job_working_directory, "dataset_%s_files" % (
                    dataset.dataset.id)))
            ...
            if max_metadata_value_size:
                for k, v in dataset.metadata.items():
                    if total_size(v) > max_metadata_value_size:
                        log.info(
                            "Key %s too large for metadata, discarding" % k)
    ...
    for i, (filename, file_dict) in enumerate(
            new_job_metadata_dict.iteritems(), start=1):
    ...
    if existing_job_metadata_dict or new_job_metadata_dict:
        with open(job_metadata, 'wb') as job_metadata_fh:
            for value in existing_job_metadata_dict.values(
