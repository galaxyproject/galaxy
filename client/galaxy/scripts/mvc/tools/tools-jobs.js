...
return {
    submit: function(form, options, callback) {
        ...
        if (!this._validation(form, job_def)) {
            Utils.emit.debug('tools-jobs::submit()', 'Submission canceled. Validation failed.');
            callback && callback();
            return;
        }

        // debug
        Utils.emit.debug('tools-jobs::submit()', 'Validation complete.', job_def);
        ...
        Utils.request({
            ...
            error   : function(response) {
                callback && callback();
                Utils.emit.debug('tools-jobs::submit', 'Submission failed.', response);
    ...
    _validation: function(form, job_def) {
        ...
        for (var job_input_id in job_inputs) {
            ...
            if (!input_id || !input_def || !input_field) {
                Utils.emit.debug('tools-jobs::_validation()', 'Retrieving input objects failed.');
                continue;
            }
