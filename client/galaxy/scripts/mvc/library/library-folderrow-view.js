    ...
    undeleteDataset: function(event) {
        ...
        dataset.destroy({
            success: function(model, response) {
                ...
                Toastr.success("Dataset undeleted. Click this to see it.", "", {
                    onclick: function() {
            ...
            error: function(model, response) {
                if (typeof response.responseJSON !== "undefined") {
                    Toastr.error(`Dataset was not undeleted. ${response.responseJSON.err_msg}`);
                } else {
                    Toastr.error("An error occurred! Dataset was not undeleted. Please try again.");
                }
            }
        });
    },
    ...
    undeleteFolder: function(event) {
        ...
        folder.destroy({
            success: function(model, response) {
                ...
                Galaxy.libraries.folderListView.collection.sortFolder("name", "asc");
                Toastr.success("Folder undeleted.");
            },
            error: function(model, response) {
                if (typeof response.responseJSON !== "undefined") {
                    Toastr.error(`Folder was not undeleted. ${response.responseJSON.err_msg}`);
                } else {
                    Toastr.error("An error occurred! Folder was not undeleted. Please try again.");
                }
            }
        });
    },
    ...
    saveModifications: function() {
        ...
        if (typeof new_name !== "undefined" && new_name !== folder.get("name")) {
            if (new_name.length > 2) {
                ...
                Toastr.warning("Folder name has to be at least 3 characters long.");
                return;
            }
        }
        ...
        if (is_changed) {
            ...
            folder.save(null, {
                ...
                success: function(folder) {
                    ...
                    Toastr.success("Changes to folder saved.");
                },
                error: function(model, response) {
                    if (typeof response.responseJSON !== "undefined") {
                        Toastr.error(response.responseJSON.err_msg);
                    } else {
                        Toastr.error("An error occurred while attempting to update the folder.");
                    }
                }
            });
        } else {
