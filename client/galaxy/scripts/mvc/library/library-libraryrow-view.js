...
var LibraryRowView = Backbone.View.extend({
    ...
    cancel_library_modification: function() {
        // Toastr.info('Modifications canceled');
    ...
    save_library_modification: function() {
        ...
        if (typeof new_name !== "undefined" && new_name !== library.get("name")) {
            if (new_name.length > 2) {
                ...
                Toastr.warning("Library name has to be at least 3 characters long.");
                return;
            }
        }
        ...
        if (is_changed) {
            ...
            library.save(null, {
                ...
                success: function(library) {
                    ...
                    Toastr.success("Changes to library saved.");
                },
                error: function(model, response) {
                    if (typeof response.responseJSON !== "undefined") {
                        Toastr.error(response.responseJSON.err_msg);
                    } else {
                        Toastr.error("An error occurred while attempting to update the library.");
                    }
                }
            });
        } else {
            ...
            Toastr.info("Nothing has changed.");
        }
    },
    ...
    delete_library: function() {
        ...
        library.destroy({
            success: function(library) {
                ...
                Toastr.success("Library has been marked deleted.");
            },
            error: function(model, response) {
                if (typeof response.responseJSON !== "undefined") {
                    Toastr.error(response.responseJSON.err_msg);
                } else {
                    Toastr.error("An error occurred during deleting the library.");
                }
            }
        });
    },
    ...
    undelete_library: function() {
        ...
        library.destroy({
            success: function(library) {
                ...
                row_view.edit_mode = false;
                row_view.repaint(library);
                Toastr.success("Library has been undeleted.");
            },
            error: function(model, response) {
                if (typeof response.responseJSON !== "undefined") {
                    Toastr.error(response.responseJSON.err_msg);
                } else {
                    Toastr.error("An error occurred while undeleting the library.");
                }
            }
        });
    },
