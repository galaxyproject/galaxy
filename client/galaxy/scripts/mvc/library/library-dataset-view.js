...
var LibraryDatasetView = Backbone.View.extend({
    ...
    fetchVersion: function(options) {
        ...
        if (!this.options.ldda_id) {
            this.render();
            Toastr.error("Library dataset version requested but no id provided.");
        } else {
        ...
        this.histories.fetch({
            success: function(histories) {
                if (histories.length === 0) {
                    Toastr.warning("You have to create history first. Click this to do so.", "", {
                        onclick: function() {
    ...
    processImportToHistory: function(history_id) {
        ...
        historyItem.save(
            { content: this.id, source: "library" },
            {
                success: function() {
                    Galaxy.modal.hide();
                    Toastr.success("Dataset imported. Click this to start analyzing it.", "", {
                        onclick: function() {
                            window.location = getAppRoot();
                        }
                    });
                ...
                error: function(model, response) {
                    if (typeof response.responseJSON !== "undefined") {
                        Toastr.error(`Dataset not imported. ${response.responseJSON.err_msg}`);
                    } else {
                        Toastr.error("An error occurred. Dataset not imported. Please try again.");
                    }
                }
            }
        );
    ...
    showPermissions: function(options) {
        ...
        $.get(`${getAppRoot()}api/libraries/datasets/${self.id}/permissions?scope=current`)
            ...
            .fail(() => {
                Toastr.error("An error occurred while attempting to fetch dataset permissions.");
            });
        $('#center [data-toggle="tooltip"]').tooltip({ trigger: "hover" });
    ...
    saveModifications: function(options) {
        ...
        if (typeof new_name !== "undefined" && new_name !== ld.get("name")) {
            if (new_name.length > 0) {
                ...
                Toastr.warning("Library dataset name has to be at least 1 character long.");
                return;
            }
        }
    ...
    _submitModification(library_dataset) {
        library_dataset.save(null, {
            ...
            success: library_dataset => {
                this.render();
                Toastr.success("Changes to library dataset saved.");
            },
            error: function(model, response) {
                if (typeof response.responseJSON !== "undefined") {
                    Toastr.error(response.responseJSON.err_msg);
                } else {
                    Toastr.error("An error occurred while attempting to update the library dataset.");
                }
            }
        });
    },
    ...
    savePermissions: function(event) {
        ...
        $.post(`${getAppRoot()}api/libraries/datasets/${self.id}/permissions?action=set_permissions`, {
            ...
            .fail(() => {
                Toastr.error("An error occurred while attempting to set dataset permissions.");
            });
    },
