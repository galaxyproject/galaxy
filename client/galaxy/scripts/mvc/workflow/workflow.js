...
const WorkflowItemView = Backbone.View.extend({
    ...
    removeWorkflow: function() {
        ...
        if (window.confirm(`Are you sure you want to delete workflow '${wfName}'?`)) {
            this.model.destroy({
                success: function() {
                    Toastr.success(`Successfully deleted workflow '${wfName}'`);
                }
            });
            this.remove();
        }
    ...
    renameWorkflow: function() {
        ...
        if (newName) {
            this.model.save(
                ...
                {
                    success: function() {
                        Toastr.success(`Successfully renamed workflow '${oldName}' to '${newName}'`);
                    }
                }
            );
            this.render();
    ...
    copyWorkflow: function() {
        ...
        $.getJSON(`${this.model.urlRoot}/${this.model.id}/download`, wfJson => {
            ...
            this.collection.create(wfJson, {
                ...
                wait: true,
                success: function() {
                    Toastr.success(`Successfully copied workflow '${oldName}' to '${newName}'`);
                },
                error: function(model, resp, options) {
                    ...
                    Toastr.error(options.errorThrown);
                }
            });
        }).error((jqXHR, textStatus, errorThrown) => {
            Toastr.error(jqXHR.responseJSON.err_msg);
        });
    },
...
const WorkflowListView = Backbone.View.extend({
    ...
    readWorkflowFiles: function(f) {
        ...
        reader.onload = theFile => {
            ...
            try {
                ...
                Toastr.error(`Could not read file '${f.name}'. Verify it is a valid Galaxy workflow`);
                wf_json = null;
            }
            if (wf_json) {
                this.collection.create(wf_json, {
                    ...
                    wait: true,
                    success: function() {
                        Toastr.success(`Successfully imported workflow '${wf_json.name}'`);
                    },
                    error: function(model, resp, options) {
                        Toastr.error(options.errorThrown);
                    }
    ...
    _showArgErrors: _.once(() => {
        ...
        if (msg_status === "error") {
            Toastr.error(_.escape(msg_text || "Unknown Error, please report this to an administrator."));
        } else if (msg_text) {
            Toastr.info(_.escape(msg_text));
        }
    }),
