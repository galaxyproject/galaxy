...
define([ 'qunit/test-app', 'sinon', 'mvc/form/form-input', 'mvc/ui/ui-misc', 'mvc/form/form-data', 'mvc/tool/tool-form', 'utils/utils',
    ...
    QUnit.test( 'tool-form', function(assert) {
        ...
        assert.ok( form.$( '.portlet-title-text' ).html() == '<b>_name</b> _description (Galaxy Version _version)', 'Title correct' );
        var tour_ids = [];
        $( '[tour_id]' ).each( function() { tour_ids.push( $( this ).attr( 'tour_id' ) ) } );
        assert.ok( JSON.stringify( tour_ids ) == '["a","b|c","b|i","b|j","k_0|l","k_0|m|n","k_0|m|s","k_0|m|t"]', 'Tour ids correct' );
        assert.ok( JSON.stringify( form.data.create() ) == '{"a":"","b|c":"h","b|i":"i","b|j":"j","k_0|l":"l","k_0|m|n":"r","k_0|m|s":"s","k_0|m|t":"t"}', 'Created data correct' );
        var mapped_ids = [];
        form.data.matchModel( form.model.attributes, function( input, id ) { mapped_ids.push( $( '#' + id ).attr( 'tour_id' ) ) } );
        assert.ok( JSON.stringify( mapped_ids ) == '["a","b|c","b|i","b|j","k_0|l","k_0|m|n","k_0|m|s","k_0|m|t"]', 'Remapped tour ids correct' );
        ...
        this.clock.tick ( window.WAIT_FADE );
        assert.ok( form.$( '.ui-message' ).html() === '<span>This tool requires req_name_a (Version req_version_a) and req_name_b (Version req_version_b). Click <a target="_blank" href="https://galaxyproject.org/tools/requirements/">here</a> for more information.</span>', 'Check requirements message' );
        ...
        form.$( '.form-repeat-delete' ).each( function( i, d ) { assert.ok( $( d ).css( 'display' ) == 'block', 'Delete buttons ' + i + ' enabled' ) } );
        assert.ok( JSON.stringify( form.data.create() ) == '{"a":"","b|c":"h","b|i":"i","b|j":"j","k_0|l":"l","k_0|m|n":"r","k_0|m|s":"s","k_0|m|t":"t","k_1|l":"l","k_1|m|n":"o","k_1|m|p":"p","k_1|m|q":"q"}', 'Created data correct, after adding repeat' );
        form.$( '.form-repeat-delete:first' ).click();
        assert.ok( form.$( '.form-repeat-delete' ).css( 'display' ) == 'none', 'Delete button disabled' );
        this.clock.tick ( window.WAIT_FADE );
        assert.ok( JSON.stringify( form.data.create() ) == '{"a":"","b|c":"h","b|i":"i","b|j":"j","k_0|l":"l","k_0|m|n":"o","k_0|m|p":"p","k_0|m|q":"q"}', 'Created data correct, after removing first repeat' );
    });
    ...
    QUnit.test( 'input', function(assert) {
        ...
        assert.ok( input.$field.children().first().css( 'color' ) == 'rgb(255, 0, 0)', 'Shows correct new color' );
        input.model.set( 'color', null );
        assert.ok( input.$field.children().first().css( 'color' ) == 'rgb(85, 85, 85)', 'Shows correct old color' );
