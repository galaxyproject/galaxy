/* global define */
define(["qunit/test-app", "mvc/upload/upload-view"], function(testApp, GalaxyUpload) {
    "use strict";
    ...
    QUnit.test("test adding/removing paste/fetch upload item", function(assert) {
        ...
        assert.ok(
            this.app.default_view.collection.length == 1,
            "Invalid upload item collection length after adding item."
        );
        assert.ok(
            $("#btn-new")
                .find("i")
