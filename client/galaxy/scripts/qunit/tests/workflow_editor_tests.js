...
define(
    ...
    function(testApp, sinon, Utils, App, Node, NodeView, Terminals, TerminalsView, Connector) {
        "use strict";
        Utils = Utils.default;
        App = App.default;
        Node = Node.default;
        ...
        QUnit.test(
            "cannot accept incorrect datatype if some other output converted with PJA to compatible type",
            function(assert) {
        ...
        QUnit.test("cannot accept already connected inputs if input is multiple but datatypes don't match", function(
            assert
        ) {
            var other = { node: {}, datatypes: ["binary"] }; // binary is not txt
        ...
        QUnit.test("cannot accept list collection for multiple input if collection already connected", function(
            assert
        ) {
            var other = {
                node: {},
        ...
        QUnit.test("replacing terminal on data multiple input update preserves collection connections", function(
            assert
        ) {
        ...
        QUnit.test("replacing mapped terminal on data collection input update preserves connections", function(assert) {
            ...
            var newElement = $("<div class='inputs'></div>");
            this.view.addDataInput(
                { name: "TestName", extensions: ["txt"], input_type: "dataset_collection" },
                newElement
            );
            var terminal = newElement.find(".input-terminal")[0].terminal;
            assert.ok(connector.handle2 === terminal);
        ...
        QUnit.module("terminal mapping logic", {
            ...
            verifyAttachable: function(assert, inputTerminal, output) {
                ...
                assert.ok(
                    inputTerminal.attachable(outputTerminal),
        ...
        QUnit.test("unmapped input can be mapped over if matching connected input terminals map type", function(
            assert
        ) {
            var inputTerminal1 = this.newInputTerminal();
        ...
        QUnit.test("explicitly constrained input can not be mapped over by incompatible collection type", function(
            assert
        ) {
            var inputTerminal1 = this.newInputTerminal();
        ...
        QUnit.test("unconnected multiple inputs cannot be connected to rank > 1 collections (yet...)", function(
            assert
        ) {
            var inputTerminal1 = this.newInputTerminal(null, { multiple: true });
            this.verifyNotAttachable(assert, inputTerminal1, "list:paired");
        ...
        QUnit.test("resetMappingIfNeeded an input resets node outputs if they not connected to anything", function(
            assert
        ) {
            var inputTerminal1 = this.newInputTerminal("list");
            var output = this.addOutput(inputTerminal1);
