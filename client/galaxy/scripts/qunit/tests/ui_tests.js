    ...
    function(testApp, sinon, Ui, SelectContent, Drilldown, Slider, Thumbnails, Tabs) {
        ...
        QUnit.test("button-check", function(assert) {
            ...
            assert.ok(
                ...
                "Has correct " + model.get("value") + " value"
            );
            button.value(1);
            assert.ok(
        ...
        QUnit.test("options", function(assert) {
            function _test(obj, options) {
                assert.ok(
                    JSON.stringify(obj.value()) == JSON.stringify(options.value),
        ...
        QUnit.test("select-default", function(assert) {
            function _test(options) {
                ...
                assert.ok(
                    select.$el.display === options.visible ? "block" : "none",
                    options.visible ? "Visible" : "Hidden"
                );
                assert.ok(
                    select.data.length === options.count && select.length(),
                    "Found " + options.count + " option"
                );
                ...
                assert.ok(
                    Boolean(select.all_button) === Boolean(options.multiple),
                    "Visiblity of select all button correct."
                );
                options.multiple &&
                    assert.ok(
        ...
        QUnit.test("select-content", function(assert) {
            ...
            var _testSelect = function(tag, options) {
                ...
                assert.ok(
                    field.length() == options[tag + "length"],
                    tag + " one has " + options[tag + "length"] + " options"
                );
                ...
                assert.ok(
                    tooltip.indexOf("dataset") != -1 || tooltip.indexOf("collection") != -1,
                    "Basic tooltip check"
                );
            };
            var _test = function(options) {
