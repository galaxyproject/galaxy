...
function asSampleSheetType(): SampleSheetCollectionType {
    if (!props.collectionType.startsWith("sample_sheet")) {
        throw Error("Logic error, asSampleSheet should not have been called");
    }
...
<template>
    <component
        ...
        <SampleSheetCollectionCreator
            v-else-if="collectionType.startsWith('sample_sheet')"
            :history-id="props.historyId"
            :initial-elements="creatorItems || []"
            :default-hide-source-items="props.defaultHideSourceItems"
            :from-selection="fromSelection"
            :extensions="props.extensions"
            :collection-type="asSampleSheetType()"
