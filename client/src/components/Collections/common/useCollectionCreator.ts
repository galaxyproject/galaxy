...
export function useCollectionCreator(props: CommonCollectionBuilderProps) {
    ...
    function isElementInvalid(element: HistoryItemSummary): string | null {
        ...
        const validState = element.state === STATES.OK || STATES.NOT_READY_STATES.includes(element.state as string);
    ...
    return { removeExtensions, hideSourceItems, hasInvalidExtension, onUpdateHideSourceItems, isElementInvalid };
}
