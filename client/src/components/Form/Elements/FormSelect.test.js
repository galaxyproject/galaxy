...
describe("FormSelect", () => {
    ...
    it("optional values", async () => {
        ...
        expect(options[0].text()).toBe("Nothing selected");
    ...
    it("multiple values", async () => {
        ...
        expect(selectedValue[0].text()).toBe("label_1");
        expect(selectedValue[1].text()).toBe("label_3");
        expect(selectedValue[2].text()).toBe("label_4");
        selectedValue[0].trigger("click");
        const newValue = wrapper.emitted().input[0][0];
        expect(newValue).toEqual(["", 99]);
        await wrapper.setProps({ value: newValue });
        selectedValue[1].trigger("click");
        const numericValue = wrapper.emitted().input[1][0];
        expect(numericValue).toEqual([99]);
        await wrapper.setProps({ value: numericValue });
        selectedValue[2].trigger("click");
