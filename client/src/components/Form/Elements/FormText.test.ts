...
async function mountFormText(props: object) {
    return await mount(FormText as object, {
        propsData: props,
        localVue,
    });
}
...
describe("FormText", () => {
    ...
    it("should be able to render border and text color from props", async () => {
        ...
        expect((el.element as HTMLInputElement).style).toMatchObject({});

        await wrapper.setProps({ color: "green" });

        expect((el.element as HTMLInputElement).style).toMatchObject({ color: "green", "border-color": "green" });
