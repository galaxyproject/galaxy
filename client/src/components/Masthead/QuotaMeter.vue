...
<template>
    <div v-b-tooltip.noninteractive.hover.bottom class="quota-meter d-flex align-items-center" :title="quotaTitle">
        <BLink class="quota-progress" :to="quotaLink" data-description="storage dashboard link">
            ...
            <span>
                <span v-localize>Using</span>{{ " " }}<span v-if="hasQuota"><span>{{ usage.toFixed(0) }}%</span>{{ " " }}<span v-if="quotaLimit !== null">of {{ quotaLimit }}</span></span><span v-else>{{ totalUsageString }}</span>
            </span>
        </BLink>
    </div>
</template>
