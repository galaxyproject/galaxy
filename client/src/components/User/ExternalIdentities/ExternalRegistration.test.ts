/* src/components/User/ExternalIdentities/ExternalRegistration.test.ts */
...
function mountExtReg(cfg: object, disableLocalAccounts = false) {
  ...
  return shallowMount(ExternalRegistration, {
    localVue,
    pinia,
...
describe("ExternalRegistration – OIDC rendering", () => {
  it.each(cases)("%s", ({ cfg, disableLocal, wantButtons, wantHr }) => {
    ...
    expect(wrapper.findAllComponents({ name: "BButton" }).length).toBe(
      wantButtons,
    );
