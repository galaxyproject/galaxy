...
const createProvider = async (propsData, delay = 250) => {
    const wrapper = await mountRenderless(DscProvider, localVue, propsData);
    await wait(delay);
...
describe("DscProvider", () => {
    ...
    test("sanity check inputs", () => {
        ...
        expect(nestedCollection._id).toContain(fakeContentUrl);
    });
