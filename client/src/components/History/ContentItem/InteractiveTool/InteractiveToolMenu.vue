...
export default {
    ...
    methods: {
        ...
        async viewIT() {
            const it_entry = await getEntryPoint(this.dataset.creating_job);
            const it_page = it_entry.data[0].target;
            console.log(it_page)
            // if (!id) {
            //     return;
            // }
            this.iframeRedirect(it_page);
        ...
        getEntryPointTarget(){
            if (getActive(this.dataset.creating_job)) {
                return getEntryPoint(this.dataset.creating_job)
            } else {
                return false
            }
        },
