...
function onInsertWorkflow(workflowId: string, workflowName: string) {
...
function insertStep(contentId: string, name: string, type: NewStep["type"]) {
    ...
    getModule({ name, type, content_id: contentId }, stepData.id, stateStore.setLoadingState).then((response) => {
        const updatedStep = {
            ...stepData,
