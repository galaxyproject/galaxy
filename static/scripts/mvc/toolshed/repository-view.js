define([ "mvc/toolshed/toolshed-model", "libs/jquery/jstree", "utils/utils", "mvc/ui/ui-modal", "mvc/form/form-view", "mvc/toolshed/util" ], function(toolshed_model, jstree, Utils, Modal, FormView, toolshed_util) {
    var ToolShedRepositoryView = Backbone.View.extend({
        ...
        templateRepoDependency: _.template([ '<li id="metadata_<%= id %>" class="datasetRow repository_dependency_row" style="display: table-row;">', "Repository <b><%= repository.name %></b> revision <b><%= changeset_revision %></b> owned by <b><%= repository.owner %></b>", "<% if (has_repository_dependencies) { %>", '<ul class="child_dependencies">', "<% _.each(repository_dependencies, function(dependency) { %>", "<% dependency.repository_dependency_template = repository_dependency_template; %>", "<%= repository_dependency_template(dependency) %>", "<% }); %>", "</ul>", "<% } %>", "</li>" ].join("")),
