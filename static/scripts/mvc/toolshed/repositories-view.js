define([ "mvc/toolshed/toolshed-model" ], function(toolshed_model) {
    var ToolShedCategoryContentsView = Backbone.View.extend({
        ...
        initialize: function(shed, category) {
            ...
            shed = shed.replace(/\//g, "%2f"), this.model.url += "?tool_shed_url=" + shed + "&category_id=" + category, 
            this.model.tool_shed = shed, this.model.category = category, this.model.fetch();
        },
        render: function(options) {
            console.log(options), this.options = _.extend(this.options, options);
