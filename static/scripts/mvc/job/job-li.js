define("mvc/job/job-li", ["exports", "mvc/list/list-item", "mvc/dataset/dataset-list", "mvc/base-mvc", "utils/localization"], function(exports, _listItem, _datasetList, _baseMvc, _localization) {
    "use strict";
    ...
    //==============================================================================
    var _super = _listItem2.default.FoldoutListItemView;
    ...
    var JobListItemView = _super.extend(
        /** @lends JobListItemView.prototype */
        {
            ...
            foldoutPanelClass: _datasetList2.default.DatasetList,
            ...
            _labelParamMap: function _labelParamMap() {
                //ADAPTER
            ...
            _labelInputMap: function _labelInputMap() {
                //ADAPTER
                var view = this;
            ...
            _findToolInput: function _findToolInput(name) {
                //ADAPTER
                var toolInputs = this.tool.inputs;

                var exactMatch = _.findWhere(toolInputs, {
                    name: name
                });
                if (exactMatch) {
                    return exactMatch;
            ...
            _findRepeatToolInput: function _findRepeatToolInput(name, toolInputs) {
                //ADAPTER
    ...
    JobListItemView.prototype.templates = function() {
        ...
        var elTemplate = _baseMvc2.default.wrapTemplate(['<div class="list-element">', '<div class="id"><%- model.id %></div>',
            // errors, messages, etc.
        ...
        var titleBarTemplate = _baseMvc2.default.wrapTemplate([
            ...
            '<div class="title">', '<span class="name"><%- view.tool.name %></span>', "</div>", '<div class="subtitle">', '<span class="description"><%- view.tool.description %></span', '<span class="create-time">', " ", (0, _localization2.default)("Created"), ": <%= new Date( job.create_time ).toString() %>, ", "</span", "</div>", "</div>"
        ], "job");

        var subtitleTemplate = _baseMvc2.default.wrapTemplate(['<div class="subtitle">', '<span class="description"><%- view.tool.description %></span',
        ...
        var detailsTemplate = _baseMvc2.default.wrapTemplate(['<div class="details">',
            ...
            //'</div>',
            '<div class="params">', "<% _.each( view._labelInputMap(), function( input, label ){ %>", '<div class="input" data-input-name="<%- input.name %>" data-input-id="<%- input.id %>">', '<label class="prompt"><%- label %></label>',
            //TODO: input dataset name
            '<span class="value"><%- input.content.name %></span>', "</div>", "<% }) %>", "<% _.each( view._labelParamMap(), function( param, label ){ %>", '<div class="param" data-input-name="<%- param.name %>">', '<label class="prompt"><%- label %></label>', '<span class="value"><%- param %></span>', "</div>", "<% }) %>", "</div>", "</div>"
        ], "job");
