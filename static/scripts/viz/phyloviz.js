define("viz/phyloviz", ["exports", "utils/localization", "libs/d3", "viz/visualization", "mvc/dataset/data", "mvc/ui/icon-button"], function(exports, _localization, _d, _visualization, _data, _iconButton) {
    "use strict";
    ...
    var UserMenuBase = Backbone.View.extend({
         ...
         */
        isAcceptableValue: function isAcceptableValue($inputKey, min, max) {
         ...
         */
        hasIllegalJsonCharacters: function hasIllegalJsonCharacters($inputKey) {
     ...
     * -- PhyloTree Model --
     */
    var PhyloTree = _visualization2.default.Visualization.extend({
        defaults: {
            layout: "Linear",
            ...
            title: (0, _localization2.default)("Title"),
            scaleFactor: 1,
            translate: [0, 0],
        ...
        initialize: function initialize(options) {
            this.set("dataset", new _data2.default.Dataset({
    ...
    var PhylovizLayoutBase = Backbone.View.extend({
         ...
         */
        stdInit: function stdInit(options) {
            var self = this;
         ...
         */
        updateAndRender: function updateAndRender(source) {
         ...
         */
        renderLinks: function renderLinks(source) {
            var self = this;
            ...
            var calcalateLinePos = function calcalateLinePos(d) {
            ...
            linkEnter.append("svg:path").attr("class", "link").attr("d", function(d) {
                calcalateLinePos(d);
        ...
        addTooltips: function addTooltips() {
            ...
            $(".node").attr("data-original-title", function() {
                var d = this.__data__;
                var annotation = d.annotation || "None";
                return d ? (d.name ? d.name + "<br/>" : "") + "Dist: " + d.dist + " <br/>Annotation1: " + annotation + (d.bootstrap ? "<br/>Confidence level: " + Math.round(100 * d.bootstrap) : "") : "";
            }).tooltip({
                placement: "top",
                trigger: "hover"
            });
    ...
    var PhylovizLinearView = PhylovizLayoutBase.extend({
         ...
         */
        renderNodes: function renderNodes(source) {
            var self = this;
            ...
            var nodeEnter = node.enter().append("svg:g").attr("class", "node").on("dblclick", function() {
            ...
            nodeEnter.append("svg:text").attr("class", "nodeLabel").attr("x", function(d) {
            ...
            nodeUpdate.select("circle").attr("r", self.defaults.nodeRadius).style("fill", function(d) {
                return d._children ? "lightsteelblue" : "#fff";
            });

            nodeUpdate.select("text").style("fill-opacity", 1).style("font-size", fontSize).text(function(d) {
    ...
    var PhylovizView = Backbone.View.extend({
        ...
        render: function render() {
            ...
            // -- Layout viz. --
            self.mainSVG = d3.select("#PhyloViz").append("svg:svg").attr("width", self.width).attr("height", self.height).attr("pointer-events", "all").call(self.zoomFunc.on("zoom", function() {
                self.zoomAndPan();
            }));

            self.boundingRect = self.mainSVG.append("svg:rect").attr("class", "boundingRect").attr("width", self.width).attr("height", self.height).attr("stroke", "black").attr("fill", "white");
         ...
         */
        zoomAndPan: function zoomAndPan(event) {
            var zoomParams;
            var translateParams;
    ...
    var HeaderButtons = Backbone.View.extend({
        initialize: function initialize(phylovizView) {
            var self = this;
            ...
            $("#phylovizNexSelector").off().on("change", function() {
                self.phylovizView.reloadViz();
            });
        },

        initRightHeaderBtns: function initRightHeaderBtns() {
            var self = this;

            var rightMenu = _iconButton2.default.create_icon_buttons_menu([{
                icon_class: "gear",
                title: (0, _localization2.default)("PhyloViz Settings"),
                ...
                icon_class: "disk",
                title: (0, _localization2.default)("Save visualization"),
                on_click: function on_click() {
                ...
                title: (0, _localization2.default)("Phyloviz Help"),
                on_click: function on_click() {
        ...
        initNavBtns: function initNavBtns() {
            var self = this;

            var navMenu = _iconButton2.default.create_icon_buttons_menu([{
                icon_class: "zoom-in",
                title: (0, _localization2.default)("Zoom in"),
                on_click: function on_click() {
                ...
                icon_class: "zoom-out",
                title: (0, _localization2.default)("Zoom out"),
                on_click: function on_click() {
                    self.phylovizView.zoomAndPan({
                        zoom: "-"
                    });
    ...
    var SettingsMenu = UserMenuBase.extend({
        ...
        initialize: function initialize(options) {
            ...
            $("#settingsCloseBtn").off().on("click", function() {
                self.el.hide();
            });
            $("#phylovizResetSettingsBtn").off().on("click", function() {
                self.resetToDefaults();
            });
            $("#phylovizApplySettingsBtn").off().on("click", function() {
                self.apply();
            });
        },
        ...
        apply: function apply() {
            var self = this;
            if (!self.isAcceptableValue(self.inputs.separation, 50, 2500) || !self.isAcceptableValue(self.inputs.leafHeight, 5, 30) || !self.isAcceptableValue(self.inputs.fontSize, 5, 20)) {
                return;
            }
    ...
    var NodeSelectionView = UserMenuBase.extend({
        ...
        initialize: function initialize(options) {
            ...
            //init UI buttons
            $("#nodeSelCloseBtn").off().on("click", function() {
                self.el.hide();
            });
        ...
        updateNodes: function updateNodes() {
            ...
            var node = self.phyloTree.get("selectedNode");
            if (node) {
                if (!self.isAcceptableValue(self.UI.dist, 0, 1) || self.hasIllegalJsonCharacters(self.UI.name) || self.hasIllegalJsonCharacters(self.UI.annotation)) {
                    return;
                }
                $.each(self.valuesOfConcern, function(key, value) {
    ...
    var PhyloVizSearch = UserMenuBase.extend({
         ...
         */
        searchTree: function searchTree(attr, condition, val) {
