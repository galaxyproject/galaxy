define("layout/panel", ["exports", "jquery", "libs/underscore", "libs/backbone"], function(exports, _jquery, _underscore, _backbone) {
    "use strict";
    ...
    var SidePanel = Backbone.View.extend({
        ...
        _templatePanel: function _templatePanel() {
            return [this._templateHeader(), this._templateBody(), this._templateFooter()].join("");
        },

        /** panel dom template. id is 'right' or 'left' */
        _templateHeader: function _templateHeader(data) {
            return ['<div class="unified-panel-header" unselectable="on">', '<div class="unified-panel-header-inner">', '<div class="panel-header-buttons" style="float: right"/>', '<div class="panel-header-text"/>', "</div>", "</div>"].join("");
        },
        ...
        /** panel dom template. id is 'right' or 'left' */
        _templateFooter: function _templateFooter(data) {
            return ['<div class="unified-panel-footer">', '<div class="panel-collapse ', _.escape(this.id), '"/>', '<div class="drag"/>', "</div>"].join("");
        },
        ...
        _mousedownDragHandler: function _mousedownDragHandler(ev) {
            var self = this;
            ...
            (0, _jquery2.default)("#dd-helper").show().on("mousemove", move).one("mouseup", function(e) {
                (0, _jquery2.default)(this).hide().off("mousemove", move);
            });
        },
        ...
        show: function show() {
            ...
            self.$el.css(whichSide, -this.saved_size).show().animate(animation, "fast", function() {
        ...
        /**   */
        handle_minwidth_hint: function handle_minwidth_hint(hint) {
        ...
        /**   */
        force_panel: function force_panel(op) {
            if (op == "show") {
                return this.show();
            }
            if (op == "hide") {
    ...
    var CenterPanel = Backbone.View.extend({
        initialize: function initialize(options) {
            this.setElement((0, _jquery2.default)(this.template()));
        ...
        display: function display(view) {
            ...
            if (!message || confirm(message)) {
                ...
                this.$panel.empty().scrollTop(0).append(view.$el || view).show();
                Galaxy.trigger("center-panel:load", view);
            }
        },

        template: function template() {
            return '<div class="center-container">' + '<iframe id="galaxy_main" name="galaxy_main" frameborder="0" class="center-frame" />' + '<div class="center-panel" />' + "</div>";
        },
