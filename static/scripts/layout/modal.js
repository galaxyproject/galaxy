define("layout/modal", ["exports", "jquery"], function(exports, _jquery) {
    "use strict";
    ...
    $.extend(Modal.prototype, {
        ...
        show: function show(options, callback) {
            if (!this.$dialog.is(":visible")) {
                ...
                this.$body.css("max-height", $(window).height() - this.$footer.outerHeight() - this.$header.outerHeight() - parseInt(this.$dialog.css("padding-top"), 10) - parseInt(this.$dialog.css("padding-bottom"), 10));
            }
            // Callback on init
            if (callback) {
                callback();
    ...
    function show_in_overlay(options) {
        ...
        modal.setContent({
            closeButton: true,
            title: "&nbsp;",
            body: $("<div style='margin: -5px;'><iframe style='margin: 0; padding: 0;' src='" + options.url + "' width='" + width + "' height='" + height + "' scrolling='" + scroll + "' frameborder='0'></iframe></div>")
        });
        modal.show({
            backdrop: true
        });
