...
function appendStyle(path) {
    const href = APP_ROOT + path;
    if (!isTrustedUrl(href)) {
        console.warn("Blocked untrusted stylesheet URL:", href);
        return;
    }
...
function appendScript(path) {
    return new Promise((resolve, reject) => {
        const src = APP_ROOT + path;
        if (!isTrustedUrl(src)) {
            reject(new Error("Blocked untrusted script URL: " + src));
            return;
        }
...
(async function initializeAequatus() {
    ...
    kickOff();
    const json_result = setDB(AJAX_URL, get_Genes_for_family);
