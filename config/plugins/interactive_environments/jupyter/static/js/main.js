...
function message_failed_auth(password) {
    toastr.info(
        "Automatic authorization failed. You can manually login with:<br>" +
            password +
            "<br> <a href='https://github.com/bgruening/galaxy-ipython/wiki/Automatic-Authorization-Failed' target='_blank'>More details ...</a>",
...
function keep_alive() {
    ...
    var success = function() {
        ...
        if (spin_state.count >= warn_at) {
            toastr.clear();
            toastr.success("Interactive environment connection restored", {
                closeButton: true,
                timeOut: 5000,
        ...
        if (spin_state.count == warn_at) {
            toastr.warning(
                "Your browser has been unable to contact the interactive environment for " +
                    spin_state.count +
                    " consecutive attempts, if you do not reestablish " +
            ...
            IES.spin_error(
                "IE keepalive failure limit reached",
                "Lost connection to interactive environment, contact your administrator",
                false
            );
