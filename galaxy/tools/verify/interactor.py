...
def stage_data_in_history(galaxy_interactor, tool_id, all_test_data, history, allow_path_paste=False):
    ...
    if UPLOAD_ASYNC:
        for test_data in all_test_data:
            upload_waits.append(galaxy_interactor.stage_data_async(test_data, history, tool_id, allow_path_paste))
        for upload_wait in upload_waits:
            upload_wait()
    else:
        for test_data in all_test_data:
            upload_wait = galaxy_interactor.stage_data_async(test_data, history, tool_id, allow_path_paste)
            upload_wait()
...
class GalaxyInteractorApi(object):
    ...
    def stage_data_async(self, test_data, history_id, tool_id, async=True, allow_path_paste=False):
...
def verify_tool(tool_id, galaxy_interactor, resource_parameters={}, register_job_data=None, test_index=0, tool_version=None, quiet=False, allow_path_paste=False):
    ...
    stage_data_in_history(galaxy_interactor, tool_id, testdef.test_data(), test_history, allow_path_paste)
