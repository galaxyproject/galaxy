#!/usr/bin/env python
bwaexe = 'bwa' # normally bwa but we need a patched svn (r47) for read groups
"""
...
def __main__():
    ...
    if options.fileSource == 'history' and not options.do_not_build_index:
        ...
        cmd1 = '%s index %s %s' % ( bwaexe, indexing_cmds, ref_file_name )
        try:
    ...
    read_group_cmds = ['@RG',]
    if options.rgID > '':
        read_group_cmds.append('ID:%s' % options.rgID)
        if options.rgSM > '':
            read_group_cmds.append('SM:%s' % options.rgSM)
        if options.rgLB > '': # the rest are optional but meaningless if no id
            read_group_cmds.append('LB:%s' % options.rgLB)
        if options.rgPL > '':
            read_group_cmds.append('PL:%s' % options.rgPL)
        rgparam = '-r "%s"' % '\t'.join(read_group_cmds)
    else:
        rgparam = ''
    # set up aligning and generate aligning command options
    ...
    cmd2 = '%s aln %s %s %s > %s' % ( bwaexe, aligning_cmds, ref_file_name, options.fastq, tmp_align_out_name )
    cmd2b = ''
    if options.genAlignType == 'paired':
        cmd2b = '%s aln %s %s %s > %s' % ( bwaexe, aligning_cmds, ref_file_name, options.rfastq, tmp_align_out2_name )
        cmd3 = '%s sampe %s %s %s %s %s %s %s >> %s' % (bwaexe, gen_alignment_cmds, rgparam, ref_file_name, tmp_align_out_name, tmp_align_out2_name, options.fastq, options.rfastq, options.output )
    else:
        cmd3 = '%s samse %s %s %s %s %s >> %s' % ( bwaexe, gen_alignment_cmds, rgparam, ref_file_name, tmp_align_out_name, options.fastq, options.output )
    # perform alignments
    buffsize = 1048576
    try:
