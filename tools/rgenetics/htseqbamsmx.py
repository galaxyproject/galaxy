...
def htseqMX gffin,bam_list,halfwindow,bam_filenames,ignoreDupes,countBetween,opts):
# htseq is accessible via
# python -m HTSeq.scripts.count [options] <sam_file> <gff_file>
    stranded = opts.stranded # default no, yes or reverse
    mode = opts.mode # default union  "union", "intersection-strict", "intersection-nonempty"
    featuretype = exon # 3rd col of gff
    quiet = True
    minaqual = 0
    if opts.mapqMin and int(opts.mapqMin) > 0:
        minaqual = int(opts.mapqMin)
    samout = False
    idattr = 'gene_id' 
    htcount.count_reads_in_features( samin, gffin, stranded, mode, featuretype, idattr, quiet, minaqual, samout )
