...
class concordRank:
    ...
    def addMaxfrac(self,f,maxFrac,maxSamp):
        """ bump files and max frac in any one sample
        """
        if not self.fileMaxfracs.get(f,None):
             self.fileMaxfracs[f] = maxFrac
             self.fileMaxsamps[f] = maxSamp
             
    def getTops(self,n=5):
         """count samples with most reads
         """
         samps = set(self.fileMaxsamps)
         sd = dict(zip(samps,[0 for x in len(samps)])
         for s in self.fileMaxsamps:
             sd[s] += 1
         k = sd.keys()
	 v = sd.values()
         sddecor = [(v[i],k[i]) for i in len(samps)]
         sddecor.sort()
         sdres = [x[1] for x in sddecor] # undecorate
         return sddec[:n] 
...
class tableRanker:
    ...
    def addTerm(self,t=None,f=None,frank=None,maxFrac=None,maxSamp=None):
        ...
        if maxFrac:
           self.termdict[t].addMaxfrac(f,maxFrac,maxSamp)
    ...
    def readOne(self,fnum,f,fname):
        ...
        if self.doMaxfrac:
            ...
            for fracs in normFracs:
                m = max(fracs)
                mpos = fracs.index(m)
                maxFs.append(m)
                maxFNames.append(self.fnames[mpos])
        else:
            maxFs = [None for x in terms]
            maxFNames = maxFs
        for i,(t,frank) in enumerate(terms):
            self.addTerm(t,fname,frank,maxFs[i],maxFNames[i])
    ...
    def report(self):
        ...
        if self.doMaxfrac:
           res.insert(0,'Term\tMedianRank\tSDRank\tNfiles\tMeanMaxfrac\tSDMaxfrac\tMaxMax\tMaxMaxFile\ttopSamps')
        else:
