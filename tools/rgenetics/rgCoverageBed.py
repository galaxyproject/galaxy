<tool name="Feature Coverage:" id="rgcoverageBed" version="0.01">
  <description>BAMTools coverageBed</description>
  <command>
coverageBed
#if $strand=='true':
-s
...
-a $input_filea -b $input_fileb > $out_file 
  ...
  <outputs>
    <data format="interval" name="out_file" label="${title}coverageBed.xls"/>
  </outputs>
  <tests>
    <test>
...
This tool relies on aaron quinlan's bedtools_

Please see his site bedtools_ for all documentation and code - this Galaxy tool runs his code with your settings
without you needing to install anything on your own workstation. No muss. No fuss. (tm)
...
Source code in this wrapper for a Galaxy tool is a component of the rgenetics toolkit.

Written by and copyright Ross Lazarus, ross.lazarus at gmail etc, November 2010

This and all other rgenetics artifacts are licensed under the LGPL_
