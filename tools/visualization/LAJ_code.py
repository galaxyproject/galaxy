...
def exec_after_process(app, inp_data, out_data, param_dict, tool, stdout, stderr):
    ...
    for name,data in inp_data.items():
        ...
        elif name == "seq_file1" and data.state != data.states.FAKE:
            seq_file1 = "/display?id="+str(data.id)
        elif name == "seq_file2" and data.state != data.states.FAKE:
            seq_file2 = "/display?id="+str(data.id)
        elif name == "exonfile" and data.state != data.states.FAKE:
            exonfile = "/display?id="+str(data.id)
        elif name == "repeatfile" and data.state != data.states.FAKE:
            repeatfile = "/display?id="+str(data.id)
        elif name == "annotationfile" and data.state != data.states.FAKE:
            annotationfile = "/display?id="+str(data.id)
        elif name == "underlayfile" and data.state != data.states.FAKE:
            underlayfile = "/display?id="+str(data.id)
        elif name == "highlightfile" and data.state != data.states.FAKE:
