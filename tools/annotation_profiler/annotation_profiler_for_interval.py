...
class RegionCoverage:
    ...
    def get_coverage_regions_overlap( self, start, end ):
        ...
        if self._total_coverage and start <= self._coverage[0][0] and end >= self._coverage[-1][1]:
            return self._total_coverage, len( self._coverage )
        coverage = 0
        region_count = 0
...
class CachedCoverageReader:
    ...
    def iter_table_coverage_regions_by_region( self, chrom, start, end ):
        for tablename, chromosomes in self._coverage.iteritems():
            ...
            coverage, regions = chromosomes[chrom].get_coverage_regions_overlap( start, end )
...
class TableCoverageSummary:
    ...
    def get_nr_coverage( self ):
        ...
        for chrom, chromosome_bitset in self.chromosome_coverage.iteritems():
            ...
            while True:
                ...
                interval_count += 1
                for table_name, coverage, region_count in self.coverage_reader.iter_table_coverage_regions_by_region( chrom, start, end ):
        ...
        return interval_count, table_coverage, table_regions_overlap_count, interval_table_overlap_count
    ...
    def iter_table_coverage( self ):
        nr_interval_count, nr_table_coverage, nr_table_regions_overlap_count, nr_interval_table_overlap_count = self.get_nr_coverage()
...
def profile_summary( interval_filename, chrom_col, start_col, end_col, out_filename, keep_empty, coverage_reader ):
    out = open( out_filename, 'wb' )
    out.write( "#tableName\ttableChromosomeCoverage\ttableChromosomeCount\tallIntervalCount\tallIntervalSize\tallCoverage\tallTableRegionsOverlaped\tallIntervalsOverlapingTable\tnrIntervalCount\tnrIntervalSize\tnrCoverage\tnrTableRegionsOverlaped\tnrIntervalsOverlapingTable\n" )#\tstatistic\n" )
    ...
    for table_name, table_chromosome_size, table_chromosome_count, total_interval_count, total_interval_size, total_coverage, table_regions_overlaped_count, interval_region_overlap_count, nr_interval_count, nr_region_size, nr_coverage, nr_table_regions_overlaped_count, nr_interval_table_overlap_count in table_coverage_summary.iter_table_coverage():
        if keep_empty or total_coverage:
            #only output tables that have atleast 1 base covered unless empty are requested
            out.write( "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % ( table_name, table_chromosome_size, table_chromosome_count, total_interval_count, total_interval_size, total_coverage, table_regions_overlaped_count, interval_region_overlap_count, nr_interval_count, nr_region_size, nr_coverage, nr_table_regions_overlaped_count, nr_interval_table_overlap_count ) )
    out.close()
