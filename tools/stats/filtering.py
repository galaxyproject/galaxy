...
used_cols = sorted({int(match.group()[1:]) for match in re.finditer(r"c(\d)+", cond_text)})
...
if valid_filter:
    ...
    if valid_lines > 0:
        print(
            "kept %4.2f%% of %d valid lines (%d total lines)."
            % (100.0 * lines_kept / valid_lines, valid_lines, total_lines)
        )
    else:
        print(
    ...
    if invalid_lines:
        print(
            'Skipped %d invalid line(s) starting at line #%d: "%s"' % (invalid_lines, first_invalid_line, invalid_line)
        )
    if skipped_lines:
