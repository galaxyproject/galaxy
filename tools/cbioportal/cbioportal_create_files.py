...
def get_stable_id_case_list_name_desc(options, logger):
	...
	for i in range(0, len(options.data_file)):
		if options.genetic_alteration_type[i] == 'COPY_NUMBER_ALTERATION':
			...
			if options.datatype[i] == 'CONTINUOUS_rnaseq':
				stable_id_suffix = '_rna_seq'
				case_list_name = 'Tumors with mRNA data'
				case_list_description = 'All tumors with mRNA data (RNA Seq)'
			elif options.datatype[i] == 'Z-SCORE_rnaseq':
				stable_id_suffix = '_rna_seq_Zscores'
				case_list_name = 'Tumors with mRNA data (Z-SCORE)'
				case_list_description = 'All tumors with mRNA data (RNA Seq) (Z-SCORE)'
			elif options.datatype[i] == 'CONTINUOUS_microarray':
				stable_id_suffix = '_mrna_expression'
				case_list_name = 'Tumors with mRNA data (microarray)'
				case_list_description = 'All tumors with mRNA data (microarray)'
			elif options.datatype[i] == 'Z-SCORE_microarray':
				stable_id_suffix = '_mrna_expression_Zscores'
				case_list_name = 'Tumors with mRNA data (microarray) (Z-SCORE)'
				case_list_description = 'All tumors with mRNA data (microarray) (Z-SCORE)'
		elif options.genetic_alteration_type[i] == 'MUTATION_EXTENDED':
...
def get_show_profile_in_analysis_tab(options, logger):
	...
	for i in range(0, len(options.data_file)):
		if options.datatype[i] in cbp_common.show_datatypes_list:
...
def _write_case_list_files(study_directory, filename, cancer_study_identifier, stable_id, name, description, set_of_ids, logger):
...
def _get_case_list_ids(data_file, genetic_alteration_type, logger):
		...
		try:
			...
			try:
				...
				# CLINICAL data does not need a case list file
					...
					sys.exit(-5)
				# return set of case list ids
				return set(case_list_ids)
			except ValueError:
...
def create_case_list_files(options, logger):
	...
	logger.debug('Number of data files: %d' % len(options.data_file))
	for i in range(0, len(options.data_file)):
		path, datafile = os.path.split(options.data_file[i])
		...
		if options.genetic_alteration_type[i] == 'MUTATION_EXTENDED':
			options.stable_id[i] = options.cancer_study_identifier + '_sequenced'
		elif options.genetic_alteration_type[i] == 'COPY_NUMBER_ALTERATION' and options.datatype[i] == 'DISCRETE':
			options.stable_id[i] = options.cancer_study_identifier + '_cna'
		elif options.genetic_alteration_type[i] == 'MRNA_EXPRESSION':
			if options.datatype[i] == 'Z-SCORE_rnaseq':
				options.stable_id[i] = options.cancer_study_identifier + '_rna_seq_v2_mrna'
			elif options.datatype[i] == 'Z-SCORE_microarray':
				options.stable_id[i] = options.cancer_study_identifier + '_mrna'


		# get set of case list ids (sample names)
		case_list_ids = _get_case_list_ids(options.data_file[i], options.genetic_alteration_type[i], logger)
		...
		# write case list file for each data file (unless it's CLINICAL attribute data)
		if options.genetic_alteration_type[i] != 'CLINICAL':
			_write_case_list_files(options.directory, case_list_file, options.cancer_study_identifier, options.stable_id[i], options.case_list_name[i], options.case_list_description[i], case_list_ids, logger) 

	# write cases_all.txt file 
	_write_case_list_files(options.directory, 'cases_all.txt', options.cancer_study_identifier, (options.cancer_study_identifier + '_all'), 'All tumors', 'All tumor samples', cases_all, logger)
	# if we have mutation AND gistic data, write cases_sequencing_and_cna.txt case list file	
	if ('MAF' in options.datatype) and ('DISCRETE' in options.datatype):
		mutation_index = options.datatype.index('MAF')
		gistic_index = options.datatype.index('DISCRETE')
		# get case_list_ids for each data file
		cases_mutation = set(_get_case_list_ids(options.data_file[mutation_index], options.genetic_alteration_type[mutation_index], logger))
		cases_gistic = (set(_get_case_list_ids(options.data_file[gistic_index], options.genetic_alteration_type[gistic_index], logger)))
		# get intersection set of both case lists
		cases_mutation_and_gistic = cases_mutation & cases_gistic
		logger.debug('len(mutation): %d \t len(gistic): %d \t len(both): %d' % (len(set(cases_mutation)), len(set(cases_gistic)), len(set(cases_mutation_and_gistic))))
		# only write case list file if we have more than 0 sample ids
		if len(cases_mutation_and_gistic) > 0:
			_write_case_list_files(options.directory, 'cases_sequenced_and_cna.txt', options.cancer_study_identifier, (options.cancer_study_identifier + '_sequenced_and_cna'), 'All tumors with sequencing and CNA', 'All tumor samples with sequencing and GISTIC copy number data', cases_mutation_and_gistic, logger)
		# if we have mutation, gistic, AND expression data (a 'complete' profile) write cases_complete.txt file
		if ('MRNA_EXPRESSION' in options.genetic_alteration_type) and ('Z-SCORE_rnaseq' in options.datatype or 'Z-SCORE_microarray' in options.datatype):
			if options.genetic_alteration_type.index('MRNA_EXPRESSION') == options.datatype.index('Z-SCORE_rnaseq') or options.genetic_alteration_type.index('MRNA_EXPRESSION') == options.datatype.index('Z-SCORE_microarray'):
				mrna_zscore_index = options.genetic_alteration_type.index('MRNA_EXPRESSION')
				cases_mrna_zscore = (set(_get_case_list_ids(options.data_file[mrna_zscore_index], options.genetic_alteration_type[mrna_zscore_index], logger)))
				cases_complete = cases_mutation_and_gistic & cases_mrna_zscore
				if len(cases_complete) > 0:
					_write_case_list_files(options.directory, 'cases_complete.txt', options.cancer_study_identifier, (options.cancer_study_identifier + '_3way_complete'), 'All tumors with sequencing, CNA, & expression', 'All tumor samples with sequencing, GISTIC copy number, and expression data', cases_complete, logger)
