...
def render(line,civic_variant,host,entrez_id_column):
  ...
  if (render_header):
     print( '<html>\n<head>\n<center><title>Galaxy - CIViC Query Report Tool Summary </center>\n<link rel="stylesheet" href="/static/style/base.css" type="text/css" />\n</head>\n<body>\n  <p/>\n  <table border="1">\n    <CAPTION>Summary of genes/variants with links to CIViC webpage</CAPTION><thead>\n      <tr>\n' )
     print("<th>{}</th><th>{}</th><th>{}</th><th>{}</th><th>{}</th></tr></thead><tbody>\n"
           .format('hugo_symbol','entrez_gene_id','CIViC variant name','drugs','link') )
     render_header = False
  print(  "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(line[0],line[1],variant_name,drugs,link) )
...
def civic_properties(line,response,host,entrez_id_column):
  ...
  if (response):    
    link = '<a href="http://{}/#/events/genes/{}/summary">civic gene</a>'.format(host,entrez_id) 
    #link = '<a href="http://192.168.100.103:3001/#/events/genes/{}/summary">civic gene</a>'.format(entrez_id) 
  else:
    link = "<i>gene not found in civic</i>"
  ...
  for variant in response:
    ...
    for evidence_item in variant.get('evidence_items'):           
      if evidence_item.get('variant_hgvs') == expected_hgvs:
        ...
        #url = "http://192.168.100.103:3001/#/events/genes/{}/summary/variants/{}/summary/evidence/{}/summary".format(entrez_id,variant.get('id'),evidence_item.get('id'))
        link = '<a href="{}">civic variant</a>'.format(url)
...
def civic_lookup(entrez_id, host):
  ...
  #overriding system proxy settings using the 3 lines below
   #proxies = {'http': ''}
  if not host == 'civic.genome.wustl.edu':
