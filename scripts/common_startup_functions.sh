...
find_server() {
    ...
    case "$server_app" in
        ...
        reports)
            ...
            GUNICORN_CMD_ARGS=${GUNICORN_CMD_ARGS:-"--bind=[::1]:9001 --bind=localhost:9001 --config lib/galaxy/web_stack/gunicorn_config.py"}
            ;;
        tool_shed)
            GUNICORN_CMD_ARGS=${GUNICORN_CMD_ARGS:-"--bind=[::1]:9009 --bind=localhost:9009 --config lib/galaxy/web_stack/gunicorn_config.py"}
    esac
