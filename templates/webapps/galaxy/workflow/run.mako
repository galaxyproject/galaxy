                ...
                % if step.post_job_actions:
                    ...
                    for pja_ss in [ActionBox.get_short_str(pja) for pja in step.post_job_actions]:
                        for rematch in re.findall('\$\{.+?\}', pja_ss):
                            pja_ss = pja_ss.replace(rematch, '<span style="background-color:%s" class="wfpspan wf_parm__%s pja_wfp">%s</span>' % (h.escape(wf_parms[rematch[2:-1]]),
                                                                                                                                                  h.escape(rematch[2:-1]),
                                                                                                                                                  h.escape(rematch[2:-1])))
