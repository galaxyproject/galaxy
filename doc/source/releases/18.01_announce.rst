...
**ES6 + Vue**
  The client code that powers the Galaxy user interface has been converted to `ES6 <http://es6-features.org/>`__ that is significantly more concise than plain JavaScript. Other major client changes include
  integration of the   `VueJS <https://vuejs.org/>`__ framework and improved client build and development
  tooling. Taken together these changes will enable Galaxy developers to write usable, responsive
  client code TODO...
