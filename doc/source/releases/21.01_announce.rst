...
**Offload Zip Archive Creation to NGINX**
  ...
  This method is more efficient and can create large archives without affecting the Galaxy server process. The required setup is described in the admin documentation for `proxying Galaxy with NGINX <https://docs.galaxyproject.org/en/release_21/admin/nginx.html#creating-archives-with-mod-zip>`__.

**Workflow Enhancements and Fixes**
  This release contains many new features, enhancements and bugfixes for creating, running and reporting workflows described in greater details in the `21.01 user release notes <21.01_announce_user.html>`__
...
**Deprecation of support for Python 3.5**
  Galaxy requires Python 3.6 or newer. Instruction for updating Python can be found in the `Supported Python versions <https://docs.galaxyproject.org/en/release_21.01/admin/python.html#supported-python-versions>`__ section of the documentation.
  Note that if you update to Python 3.7.2 or newer you need to set py-call-safterfork to ``false`` in the uwsgi section of galaxy.yml.
...
**Removal of IUC conda channel from default configuration**
  ...
  If you have specified alternative channels in your Galaxy configuration we recommend remving the iuc channel from the `conda_ensure_channels <https://github.com/galaxyproject/galaxy/blob/283ec1772fcc0e574997dc2c5ad12dc86e3e1d37/lib/galaxy/config/sample/galaxy.yml.sample#L324>`__ variable.

**Removal of the communication server**
  This feature had not seen greater adoption and was now removed. Most communication has moved to Gitter or other chat services.
