...
**Enhanced workflow functionality** 
  Workflows can now now make use of optional datasets and optional parameters,
  ...
  for more flexible and universally re-usable workflows (`Pull Request 9086`_, `Pull Request 9073`_, `Pull Request 8264`_). 
...
**Extended job metadata collection**
  ...
  Galaxy job. Previously the transfer was done by Galaxy under many production
  scenarios.  This reduces IO load on Galaxy instances and should allow running
  many more jobs in parallel. This work will also make jobs running on remote
  compute resources much more efficient if the remote resources can write to
  Galaxy'so object store. To benefit from this new metadata setting Galaxy needs
  to use object stores that use uuids for storing datasets and set the
  ``metadata_strategy`` configuration value to ``extended``. This feature is in beta
  testing, but work is ongoing to mix existing object store configuration and
  choose the most efficient way of transfering datasets automatically (`Pull
  Request 8930`_ ).
