...
- All user-defined tools select their image with the top-level `container` key.
...
and see https://github.com/galaxyproject/galaxy/blob/release_26.1/test/integration/embedded_pulsar_job_conf.yml#L29 for a simple example that uses embedded pulsar to isolate mounts and disables network access.
...
| ------------------------------------------------ | -------------------------------------------------------- | ------- |
| `cores_min` / `cores_max`                        | Requested CPU cores. May be fractional for schedulers that support sharing cores. | `1` / unset |
...
Write literal numbers or numeric strings. Expression evaluation for resource requirements is
not implemented; other string values fail the lint checks performed when a tool is created.
...
[`test/integration/embedded_pulsar_job_conf.yml`](https://github.com/galaxyproject/galaxy/blob/release_26.1/test/integration/embedded_pulsar_job_conf.yml)
and
[`test/integration/embedded_pulsar_tpv_job_conf.yml`](https://github.com/galaxyproject/galaxy/blob/release_26.1/test/integration/embedded_pulsar_tpv_job_conf.yml).
...
- **Schema validation.** Unknown top-level keys and unsupported input fields are rejected, as
  are values of the wrong type and missing required fields.
...
- **Linting.** `POST /api/unprivileged_tools` runs Galaxy's tool linters and refuses to create
  the tool if any warning or error is reported. This includes a check that the container
  reference has a recognizable shape. Linters that call external services (bio.tools, EDAM)
  are skipped so that authoring does not depend on third-party availability.
...
[`test/functional/tools/cat_user_defined.yml`](https://github.com/galaxyproject/galaxy/blob/release_26.1/test/functional/tools/cat_user_defined.yml).
...
of one tool. Creating and running a tool requires the `Custom Tool Execution` role. Stored
tools are private, and the management endpoints operate on the calling user's own tools.
...
[`ToolSourceSchema.json`](https://github.com/galaxyproject/galaxy/blob/release_26.1/client/src/components/Tool/ToolSourceSchema.json).
For worked examples of every endpoint, see
[`lib/galaxy_test/api/test_unprivileged_tools.py`](https://github.com/galaxyproject/galaxy/blob/release_26.1/lib/galaxy_test/api/test_unprivileged_tools.py).
...
- Nonnumeric expressions in resource requirement values are not supported
