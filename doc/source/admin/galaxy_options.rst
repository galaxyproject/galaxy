...
:Description:
    ...
    database. Defining this URI does not make Galaxy's eager toolbox
    startup use the store; it is opened by code paths that explicitly
    use tool source storage, such as the population script and lazy
    toolbox consumers.
    By default, Galaxy uses a SQLite database at
