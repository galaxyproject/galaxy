...
:Description:
    ...
    supplies their own Pulsar daemon; Galaxy dispatches jobs to it via
    the relay). When true, the /api/compute_resources endpoints are
    exposed and the multi-tenant ``compute_resource`` job runner
